! THIS VERSION: GALAHAD 2.6 - 09/06/2014 AT 08:30 GMT.
   ...
   INTEGER, PARAMETER :: len_iw = 5, len_w = 5
   ...
   DO mode = 1, 2
     ...
     DO type = 1, 5
       ...
       CASE ( 3 ) ! sparse by rows
         ...
         A%ptr = (/ 1, 5, 8, 10, 12 /)
         A%col = (/ 1, 5, 3, 1, 2, 4, 4, 3, 2, 4, 5 /)
         A%val = (/ 10.0_wp, 15.0_wp, 13.0_wp, 1.0_wp, 22.0_wp, 2.0_wp,       &
                    22.0_wp, 33.0_wp, 32.0_wp, 44.0_wp, 45.0_wp /)
       CASE ( 4 ) ! sparse by columns
         CALL SMT_put( A%type, 'SPARSE_BY_COLUMNS', s )
         A%ne = A%ne + 2
         ALLOCATE( A%ptr( A%n + 1 ), A%row( A%ne ), A%val( A%ne ) ) 
         A%ptr = (/ 1, 3, 5, 7, 10, 12 /)
         A%row = (/ 1, 1, 3, 2, 1, 3, 2, 4, 2, 4, 1 /)
         A%val = (/ 1.0_wp, 10.0_wp, 32.0_wp, 22.0_wp, 13.0_wp, 33.0_wp,       &
                    22.0_wp, 44.0_wp, 2.0_wp, 45.0_wp, 15.0_wp /)
       CASE ( 5 ) ! sparse co-ordinate
         CALL SMT_put( A%type, 'COORDINATE', s )
         A%ne = A%ne + 2
         ALLOCATE( A%row( A%ne ), A%col( A%ne ), A%val( A%ne ) ) 
         A%row = (/ 4, 1, 1, 3, 2, 1, 3, 4, 2, 1, 2 /)
         A%col = (/ 5, 1, 1, 2, 2, 3, 3, 4, 4, 5, 4 /)
         A%val = (/ 45.0_wp, 10.0_wp, 1.0_wp, 32.0_wp, 22.0_wp,                &
                    13.0_wp, 33.0_wp, 44.0_wp, 22.0_wp, 15.0_wp, 2.0_wp /)
       END SELECT
       CALL CONVERT_to_column_format( A, A_by_cols, control, inform,           &
                                      IW, len_iw, W, len_w ) ! convert
       WRITE( 6, "( /, ' convert from ', A, ' to column format ')" )           &
         SMT_get( A%type )
       IF ( inform%duplicates > 0 ) WRITE( 6, "( 1X, I0,                       &
      &   ' duplicates summed, now ', I0, ' entries')" ) inform%duplicates,    &
           A_by_cols%ne
       DO i = 1, A_by_cols%n
