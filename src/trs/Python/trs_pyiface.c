 ...
 *
 *  History -
 *   originally released GALAHAD Version 4.1. March 12th 2023
 *
...
static struct trs_control_type control;  // control struct
static struct trs_inform_type inform;    // inform struct
...
bool trs_update_control(struct trs_control_type *control,
    ...
    while(PyDict_Next(py_options, &pos, &key, &value)) {
        ...
        if(strcmp(key_name, "force_Newton") == 0){
            if(!parse_bool_option(value, "force_Newton",
...
PyObject* trs_make_options_dict(const struct trs_control_type *control){
    ...
    PyDict_SetItemString(py_options, "force_Newton",
                         PyBool_FromLong(control->force_Newton));
...
static PyObject* trs_make_time_dict(const struct trs_time_type *time){
...
/* Take the time struct from C and turn it into a python dictionary */
static PyObject* trs_make_history_dict(const struct trs_history_type *history){
...
PyObject* trs_make_inform_dict(const struct trs_inform_type *inform){
    ...
    PyDict_SetItemString(py_inform, "time",
                         trs_make_time_dict(&inform->time));
...
static PyObject* py_trs_initialize(PyObject *self){

    // Call trs_initialize
    trs_initialize(&data, &control, &status);

    // Record that trs has been initialised
    init_called = true;
...
static PyObject* py_trs_load(PyObject *self, PyObject *args, PyObject *keywds){
    ...
    trs_reset_control(&control, &data, &status);

    // Update trs control options
    if(!trs_update_control(&control, py_options))
        return NULL;
    ...
    // Record that trs structures been initialised
    load_called = true;
...
static PyObject* py_trs_load_m(PyObject *self, PyObject *args,
    ...
    // Record that trs M structure been initialised
    load_m_called = true;
...
static PyObject* py_trs_load_a(PyObject *self, PyObject *args,
    ...
    // Record that trs A structure been initialised
    load_a_called = true;
...
static PyObject* py_trs_information(PyObject *self){

    // Check that package has been initialised
    if(!check_init(init_called))
        return NULL;

    // Call trs_information
...
static PyObject* py_trs_terminate(PyObject *self){
    ...
    // require future calls start with trs_initialize
    init_called = false;
    load_called = false;
    load_m_called = false;
    load_a_called = false;
...
/* trs python module documentation */

PyDoc_STRVAR(trs_module_doc,

"The trs package uses matrix factorization to find the global minimizer \n"
...
"l_2-trust-region case for which M = I, the n by n identity matrix.\n"
"\n"
...
"\n"
"See $GALAHAD/html/Python/trs.html for argument lists, call order\n"
"and other details.\n"
"\n"
);

/* trs python module definition */
static struct PyModuleDef module = {
   PyModuleDef_HEAD_INIT,
   "trs",               /* name of module */
   trs_module_doc,      /* module documentation, may be NULL */
   -1,                  /* size of per-interpreter state of the module,or -1
                           if the module keeps state in global variables */
   trs_module_methods   /* module methods */
};

/* Python module initialization */
PyMODINIT_FUNC PyInit_trs(void) { // must be same as module name above
