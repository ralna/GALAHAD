...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ Q P F  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  June 25th 2012

   MODULE GALAHAD_USEQPF_precision

!  This is the driver program for running QPF for a variety of computing
!  systems. It opens and closes all the files, allocate arrays, reads and
!  checks data, and calls the appropriate minimizers

     USE GALAHAD_KINDS_precision
!    USE GALAHAD_CLOCK
     USE GALAHAD_QPF_precision
...
!-*-*-*-*-*-*-*-*-*-  U S E _ Q P F   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_QPF( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNQPF'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNQPF.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'QPF.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'QPFRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'QPFSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'QPFSAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'QPF   '

     IF ( .NOT. QPF_available ) THEN
       WRITE( 6, "( ' GALAHAD QPF is not available, stopping' )" )
       STOP
     END IF

!  ------------------ Open the specfile for QPF ----------------
...
!  Set up control parameters prior to the next solution
     ...
     CALL QPF_initialize( data, control, inform )
     IF ( is_specfile ) CALL QPF_read_specfile( control, input_specfile )
...
!    CALL CPU_TIME( timeo ) ; CALL CLOCK_time( clocko )
     CALL QPF_solve( nlp, control, inform, data, userdata,                     &
...
!  End of subroutine USE_QPF

     END SUBROUTINE USE_QPF

!  End of module USEQPF

   END MODULE GALAHAD_USEQPF_precision
