...
#include "galahad_modules.h"
   PROGRAM GALAHAD_QPF_test_program
...
!    IF ( s == - GALAHAD_error_cpu_limit ) CYCLE
     ...
     CALL QPF_initialize( data, control, inform )  ! Initialize controls
!    control%print_level = 1
     ...
     ELSE
       WRITE( 6, "( I2, ': QPF_solve exit status = ', I6 ) " ) s, inform%status
     END IF

     CALL QPF_terminate( data, control, inform )  ! delete internal workspace
   END DO
...
!    control%print_level = 1
     ...
     &    F6.1, ' status = ', I6 )" ) i, inform%iter, inform%obj, inform%status
     ELSE
       WRITE( 6, "( I2, ': QPF_solve exit status = ', I6 ) " ) i, inform%status
     END IF
...
!    control%print_level = 1
     ...
     ELSE IF ( i == 3 ) THEN
       control%hessian_available = .FALSE.      ! Hessian products will be used
       control%preconditioner = - 3             ! User's preconditioner
       CALL QPF_solve( nlp, control, inform, data, userdata, eval_F = FUN,    &
   ...
   END PROGRAM GALAHAD_QPF_test_program
