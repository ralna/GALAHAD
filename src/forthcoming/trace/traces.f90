   PROGRAM GALAHAD_TRACE_EXAMPLE  !  GALAHAD 2.6 - 23/10/2014 AT 10:00 GMT.
   USE GALAHAD_TRACE_double                       ! double precision version
   IMPLICIT NONE
...
! problem data complete   
   ...
   CALL TRACE_initialize( data, control, inform )! Initialize control parameters
...
!  control%print_level = 1
   ...
   IF ( inform%status == 0 ) THEN               ! Successful return
     WRITE( 6, "( ' TRACE: ', I0, ' iterations -',                             &
    &     ' optimal objective value =',                                        &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     inform%iter, inform%obj, nlp%X
   ELSE                                         ! Error returns
     WRITE( 6, "( ' TRACE_solve exit status = ', I6 ) " ) inform%status
   END IF
   ...
   END PROGRAM GALAHAD_TRACE_EXAMPLE
