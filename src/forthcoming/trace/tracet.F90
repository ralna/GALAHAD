...
#include "galahad_modules.h"
   PROGRAM GALAHAD_TRACE_test_deck
   USE GALAHAD_PRECISION
   USE GALAHAD_TRACE_precision
   USE GALAHAD_SYMBOLS
   IMPLICIT NONE
...
!    control%print_level = 1
     ...
     &       F6.1, ' status = ', I6 )" ) i, inform%iter, inform%obj,           &
         inform%status
     ELSE
       WRITE( 6, "( I2, ': TRACE_solve exit status = ', I6 ) " ) s,            &
         inform%status
     END IF
...
!      control%print_level = 5
     ...
     &       F6.1, ' status = ', I6 )" ) i, inform%iter, inform%obj,           &
         inform%status
     ELSE
       WRITE( 6, "( I2, ': TRACE_solve exit status = ', I6 ) " ) i,            &
         inform%status
     END IF
