...
!  control%reorthogonalize = .FALSE.
   CALL CPU_time( cpu_start )
   CALL TREK_solve( n, H, C, RADIUS( 1 ), X1, data, control, inform )
   CALL CPU_time( cpu_end )
   IF ( inform%status == 0 ) THEN
     WRITE( 6, "( 1X, I0, ' vectors required, error = ', ES11.4 )" )           &
       inform%n_vec, inform%error
     WRITE( 6, "( ' radius, ||x||, shift, obj = ', 3ES11.4, ES12.4 )" )        &
       RADIUS( 1 ), TWO_NORM( X1( 1 : n ) ), inform%multiplier, inform%obj
   ELSE
     WRITE( 6, "( ' error exit, status = ', I0 )" ) inform%status
   END IF
   WRITE( 6, "( ' elapsed cpu time = ', F0.2 )" ) cpu_end - cpu_start
   ...
   DO i = 2, m
     ...
     CALL CPU_time( cpu_start )
     CALL TREK_resolve( n, H, C, RADIUS( i ), X1, data, control, inform )
     CALL CPU_time( cpu_end )
     IF ( inform%status == 0 ) THEN
       WRITE( 6, "( 1X, I0, ' vectors required, error = ', ES11.4 )" )         &
         inform%n_vec, inform%error
       WRITE( 6, "( ' radius, ||x||, shift, obj = ', 3ES11.4, ES12.4 )" )      &
         RADIUS( i ), TWO_NORM( X1( 1 : n ) ), inform%multiplier, inform%obj
     ELSE
       WRITE( 6, "( ' error exit, status = ', I0 )" ) inform%status
     END IF
     WRITE( 6, "( ' elapsed cpu time = ', F0.2 )" ) cpu_end - cpu_start
...
!  control%reorthogonalize = .FALSE.
   CALL CPU_time( cpu_start )
   CALL TREK_solve( n, H, C, RADIUS( 2 ), X1, data, control, inform )
   CALL CPU_time( cpu_end )
   IF ( inform%status == 0 ) THEN
     WRITE( 6, "( 1X, I0, ' vectors required, error = ', ES11.4 )" )           &
       inform%n_vec, inform%error
     WRITE( 6, "( ' radius, ||x||, shift, obj = ', 3ES11.4, ES12.4 )" )        &
       RADIUS( 2 ), TWO_NORM( X1( 1 : n ) ), inform%multiplier, inform%obj
   ELSE
     WRITE( 6, "( ' error exit, status = ', I0 )" ) inform%status
   END IF
   WRITE( 6, "( ' elapsed cpu time = ', F0.2 )" ) cpu_end - cpu_start
