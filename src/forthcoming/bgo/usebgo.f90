...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ B G O  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  June 20th 2016

   MODULE GALAHAD_USEBGO_double

!  This is the driver program for running BGO for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ B G O   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_BGO( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNBGO'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNBGO.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'BGO.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'BGORES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'BGOSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'BGOSAVE.d'
...
!  Output file characteristics
     ...
     INTEGER :: out  = 6
     INTEGER :: errout = 6
     CHARACTER ( LEN =  6 ) :: solv = 'BGO   '

!  ------------------ Open the specfile for bgo ----------------
...
!  Set up control parameters prior to the next solution
     ...
     IF ( is_specfile ) CALL BGO_read_specfile( control, input_specfile )
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from usebgo: re-entry requested with',   &
       ...
       IF ( i /= nlp%n ) THEN
         WRITE( out, "( /, ' *** Exit from usebgo: number of variables'      , &
        &        ' changed from ', I0,' to ', I0,' on re-entry ' )" ) nlp%n, i
         STOP
       END IF
       READ( wfiledevice, "( I8 )" )i
       IF ( i /= nlp%m ) THEN
         WRITE( out, "( /, ' *** Exit from usebgo: number of residuals',       &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from usebgo: variable named ',         &
...
!  End of subroutine USE_BGO

     END SUBROUTINE USE_BGO

!  End of module USEBGO_double

   END MODULE GALAHAD_USEBGO_double
