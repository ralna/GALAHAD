...
!-*-*-*-*-*-*-*-*-*-  U S E _ T R A L   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_TRAL( input )
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'TRAL.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'TRALRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'TRALSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'TRALSAVE.d'
...
!  Set up control parameters prior to the next solution
     ...
     CALL TRAL_initialize( data, control, inform )
     IF ( is_specfile ) CALL TRAL_read_specfile( control, input_specfile )
