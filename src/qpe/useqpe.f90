! THIS VERSION: GALAHAD 2.5 - 08/02/2013 AT 15:15 GMT.

!-*-*-*-*-*-*-*-  G A L A H A D   U S E Q P E   M O D U L E  -*-*-*-*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
    ...
    MODULE GALAHAD_USEQPE_double

!    ----------------------------------------------------------
!    | CUTEst/AMPL interface to QPE, a primal-dual active-set  |
...
!-*-*-*-*-*-*-*-*-*-   U S E _ Q P E  S U B R O U T I N E   -*-*-*-*-*-*-*-*-

     SUBROUTINE USE_QPE( input )
...
!                    x_l <=  x <= x_u
!
!  using the GALAHAD package GALAHAD_QPE
!
!  --------------------------------------------------------------------
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 16 ) :: specname = 'RUNQPE'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNQPE.SPC'

!  The default values for QPE could have been set as:

! BEGIN RUNQPE SPECIFICATIONS (DEFAULT)
!  write-problem-data                        NO
!  problem-data-file-name                    QPE.data
...
!  write-solution                            NO
!  solution-file-name                        QPESOL.d
!  solution-file-device                      62
!  write-result-summary                      NO
!  result-summary-file-name                  QPERES.d
!  result-summary-file-device                47
!  perturb-bounds-by                         0.0
! END RUNQPE SPECIFICATIONS
...
!  Default values for specfile-defined parameters
      ...
      CHARACTER ( LEN = 30 ) :: dfilename = 'QPE.data'
      CHARACTER ( LEN = 30 ) :: ifilename = 'INITIAL.SIF'
      CHARACTER ( LEN = 30 ) :: pfilename = 'PRESOLVE.SIF'
      CHARACTER ( LEN = 30 ) :: rfilename = 'QPERES.d'
      CHARACTER ( LEN = 30 ) :: sfilename = 'QPESOL.d'
...
!  Arrays
      ...
      TYPE ( QPE_data_type ) :: data
      TYPE ( QPE_control_type ) :: QPE_control
      TYPE ( QPE_inform_type ) :: QPE_inform
...
!  Set all default values, and override defaults if requested
      ...
      CALL QPE_initialize( data, QPE_control, QPE_inform )
      IF ( is_specfile )                                                       &
        CALL QPE_read_specfile( QPE_control, input_specfile )

      control%print_level = QPE_control%print_level
      control%out         = QPE_control%out
      control%out_error   = QPE_control%error
...
!       Overide some defaults
        ...
        PRE_control%infinity = QPE_control%infinity
...
!       WRITE( 6, "( ' z ', /, (5ES12.4) )" ) prob%Z
        ...
        solv = ' QPE'
        IF ( printo ) WRITE( out, " ( ' ** QPE solver used ** ' ) " )
        CALL QPE_solve( prob, data, QPE_control, QPE_inform )

        IF ( printo ) WRITE( out, " ( /, ' ** QPE solver used ** ' ) " )
        qfval = QPE_inform%obj

        CALL CPU_TIME( timet )

!  Deallocate arrays from the minimization

        status = QPE_inform%status
        iter = QPE_inform%iter
        CALL QPE_terminate( data, QPE_control, QPE_inform )
...
!  Compare the variants used so far
      ...
      WRITE( out, 2080 ) solv, iter, qfval, status

      IF ( write_result_summary ) THEN
        BACKSPACE( rfiledevice )
        WRITE( rfiledevice, 2190 )                                             &
           pname, n, m, iter, qfval, status, timet
      END IF
...
!  Non-executable statements
 ...
 2070 FORMAT( /, ' Problem: ', A10, //,                                        &
                 '         iterations  objective status' )
 2080 FORMAT( A5, I7, 6X, ES12.4, I6 )
...
!  End of subroutine USE_QPE

     END SUBROUTINE USE_QPE

!  End of module USEQPE_double

   END MODULE GALAHAD_USEQPE_double
