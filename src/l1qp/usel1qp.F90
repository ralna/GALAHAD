! THIS VERSION: GALAHAD 4.1 - 2022-12-21 AT 14:50 GMT.
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
    ...
    MODULE GALAHAD_USEL1QP_precision

!     ----------------------------------------------------------
!    | CUTEst/AMPL interface to L1QP, an infeasible primal-dual |
!    | interior-point to dual gradient-projection crossover     |
!    |  method for linear or convex quadratic programming       |
!     ----------------------------------------------------------
...
!-*-*-*-*-*-*-*-*-*-   U S E _ L 1 Q P  S U B R O U T I N E   -*-*-*-*-*-*-*-*-

     SUBROUTINE USE_L1QP( input )

!  --------------------------------------------------------------------
!
!  Solve the l_1 quadratic program from CUTEst
!
!     minimize 1/2 x(T) H x + g(T) x + rho || min( A x - c_l, c_u - A x, 0 )||_1
!
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 7 ) :: specname = 'RUNL1QP'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 11 ) :: runspec = 'RUNL1QP.SPC'
...
!  Arrays
      ...
      TYPE ( L1QP_control_type ) :: L1QP_control
      TYPE ( L1QP_inform_type ) :: L1QP_inform
...
!   Interpret the result
        ...
        CALL SPECFILE_assign_integer( spec( 7 ), problem_type, errout )
...
!  find the leftmost eigenvalue of H by minimizing x^T H x : || x ||_2 = 1
        ...
        CALL SLS_initialize(                                                   &
          L1QP_control%CQP_control%SBLS_control%symmetric_linear_solver,       &
...
!  refactorize H
          ...
          CALL SLS_initialize(                                                 &
            L1QP_control%CQP_control%SBLS_control%definite_linear_solver,      &
            sls_data, sls_control, sls_inform )
          CALL SLS_analyse( prob%H, sls_data, sls_control, sls_inform )
          IF ( sls_inform%status < 0 ) THEN
            WRITE( 6, '( A, I0 )' )                                            &
...
!STOP
      ...
      ELSE IF ( problem_type == 2 ) THEN
        ...
        prob%Hessian_kind = 1
        prob%target_kind = 2
!       ALLOCATE( prob%X0( prob%n ), STAT = alloc_stat )
        prob%X0 = 2.0_rp_
      END IF

!  Set all default values, and override defaults if requested

      CALL L1QP_initialize( data, L1QP_control, L1QP_inform )

      IF ( is_specfile )                                                       &
        CALL L1QP_read_specfile( L1QP_control, input_specfile )
      IF ( scale /= 0 )                                                        &
        CALL SCALE_read_specfile( SCALE_control, input_specfile )
!     SCALE_control%print_level = L1QP_control%print_level

      printo = out > 0 .AND. L1QP_control%print_level > 0
      printe = out > 0 .AND. L1QP_control%print_level >= 0
      WRITE( out, 2200 ) n, m, A_ne, H_ne

      IF ( printo ) CALL COPYRIGHT( out, '2017' )

      C_stat = 0 ; X_stat = 0 ; prob%C = zero
...
!       Overide some defaults
        ...
        PRE_control%infinity = L1QP_control%infinity
        PRE_control%c_accuracy = ten * L1QP_control%CQP_control%stop_abs_p
        PRE_control%z_accuracy = ten * L1QP_control%CQP_control%stop_abs_d
...
!  If required, scale the problem
        ...
        IF ( problem_type /= 2 ) DEALLOCATE( prob%X0 )
...
!  =================
        ...
        CALL L1QP_solve( prob, data, L1QP_control, L1QP_inform, C_stat, X_stat )

        IF ( printo ) WRITE( out, " ( /, ' ** L1QP solver used ** ' ) " )
        qfval = L1QP_inform%obj

        CALL CPU_TIME( timet ) ; CALL CLOCK_time( clockt )

!  Deallocate arrays from the minimization

        status = L1QP_inform%status
        iter = L1QP_inform%CQP_inform%iter + L1QP_inform%DQP_inform%iter
        stopr = L1QP_control%stop_abs_d
        CALL L1QP_terminate( data, L1QP_control, L1QP_inform )
...
!  Compute maximum KKT residual
      ...
      ELSE IF ( prob%Hessian_kind == 1 ) THEN
        ...
        ELSE IF ( prob%target_kind == 1 ) THEN
          HX = HX + prob%X - one
        ELSE
          HX = HX + prob%X - prob%X0
        END IF
      ELSE IF ( prob%Hessian_kind == 2 ) THEN
        IF ( prob%target_kind == 0 ) THEN
          HX = HX + prob%WEIGHT * prob%X
        ELSE IF ( prob%target_kind == 1 ) THEN
          HX = HX + prob%WEIGHT * ( prob%X - one )
        ELSE
          HX = HX + prob%WEIGHT * ( prob%X - prob%X0 )
        END IF
      ELSE
        DO i = 1, n
...
!  If required, write the solution to a file
        ...
        IF ( write_solution ) THEN
          ...
          DO i = 1, n
            ...
            WRITE( sfiledevice, 2050 ) i, VNAME( i ), STATE, prob%X( i ),      &
              prob%X_l( i ), prob%X_u( i ), prob%Z( i ), X_stat( i )
          END DO
      ...
      sls_solv = L1QP_control%CQP_control%SBLS_control%symmetric_linear_solver
    ...
    &                ' entries in matrix, factors' )" )                        &
        L1QP_inform%CQP_inform%SBLS_inform%SLS_inform%entries,                 &
        L1QP_inform%CQP_inform%SBLS_inform%SLS_inform%entries_in_factors
      WRITE( out, "( ' Analyse, factorize & solve CPU   times =',              &
     &  3( 1X, F8.3 ), /, ' Analyse, factorize & solve clock times =',         &
     &  3( 1X, F8.3 ) )" ) L1QP_inform%time%analyse,                           &
        L1QP_inform%time%factorize,                                            &
        L1QP_inform%time%solve, L1QP_inform%time%clock_analyse,                &
        L1QP_inform%time%clock_factorize, L1QP_inform%time%clock_solve
      ...
      WRITE( out, "( ' number of threads = ', I0 )" ) L1QP_inform%threads
      WRITE( out, 2070 ) pname

!  Compare the variants used so far

      WRITE( out, 2080 ) solv, L1QP_inform%CQP_inform%iter,                    &
                         L1QP_inform%DQP_inform%iter, qfval, status, clocks,   &
                         L1QP_inform%CQP_inform%time%clock_total,              &
                         L1QP_inform%DQP_inform%time%clock_total,              &
                         clocks + clockt
...
!     DEALLOCATE( RES )
      ...
      DEALLOCATE( prob%X, prob%X_l, prob%X_u, prob%G, VNAME, X_stat,           &
                  prob%C, prob%C_l, prob%C_u, prob%Y, CNAME, EQUATN,           &
                  prob%Z_l, prob%Z_u, prob%Y_l, prob%Y_u, C,                   &
                  prob%X0, prob%Z, prob%X_status, prob%C_status,               &
                  prob%A%row, prob%A%col, prob%A%val, prob%A%ptr, prob%A%type, &
                  prob%H%row, prob%H%col, prob%H%val, prob%H%ptr, prob%H%type, &
                  STAT = alloc_stat )
      IF ( problem_type == 2 ) DEALLOCATE( prob%X0 )
...
!  End of subroutine USE_L1QP

     END SUBROUTINE USE_L1QP

!  End of module USEL1QP

   END MODULE GALAHAD_USEL1QP_precision
