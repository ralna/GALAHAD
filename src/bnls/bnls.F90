...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D _ B N L S   M O D U L E  *-*-*-*-*-*-*-*-*-*-
...
!     --------------------------------------------------------------------
     ...
     PUBLIC :: BNLS_initialize, BNLS_read_specfile, BNLS_solve, BNLS_terminate,    &
               BNLS_subproblem_initialize, BNLS_subproblem_read_specfile,        &
               BNLS_subproblem_solve, BNLS_subproblem_terminate,                 &
               ...
               BNLS_reset_control, NLPT_problem_type, GALAHAD_userdata_type,    &
               SMT_type, SMT_put
...
!----------------------
     ...
     INTERFACE BNLS_initialize
       MODULE PROCEDURE BNLS_initialize, BNLS_full_initialize
     END INTERFACE BNLS_initialize

     INTERFACE BNLS_terminate
       MODULE PROCEDURE BNLS_terminate, BNLS_full_terminate
     END INTERFACE BNLS_terminate
...
!  models
     ...
     INTEGER ( KIND = ip_ ), PARAMETER  :: tensor_gauss_newton_model = 6
     INTEGER ( KIND = ip_ ), PARAMETER  :: tensor_newton_model = 7
     INTEGER ( KIND = ip_ ), PARAMETER  :: tensor_gauss_to_newton_model = 8
...
!  - - - - - - - - - - - - - - - - - - - - - - -
     ...
     TYPE, PUBLIC :: BNLS_subproblem_control_type
...
!  Newton solve may be called as an internal subroutine to solve subproblems
     ...
     TYPE, PUBLIC, EXTENDS( BNLS_subproblem_control_type ) :: BNLS_control_type
...
!  - - - - - - - - - - - - - - - - - - - - - - -
     ...
     TYPE, PUBLIC :: BNLS_subproblem_inform_type
...
!  Newton solve may be called as an internal subroutine to solve subproblems
     ...
     TYPE, PUBLIC, EXTENDS( BNLS_subproblem_inform_type ) :: BNLS_inform_type
...
!  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     TYPE, PUBLIC :: BNLS_regularization_data_type
...
!  the stabilisation scaling matrix, S
     ...
     END TYPE BNLS_regularization_data_type

!  - - - - - - - - - -
!   data derived type
!  - - - - - - - - - -

     TYPE, PUBLIC :: BNLS_subproblem_data_type
       ...
       TYPE ( BNLS_regularization_data_type ) :: regularization
...
!  data for ROOTS
     ...
     END TYPE BNLS_subproblem_data_type

!  extend newton_data_type so that it contains a copy of itself so that the
!  Newton solve may be called as an internal subroutine to solve subproblems

     TYPE, PUBLIC, EXTENDS( BNLS_subproblem_data_type ) :: BNLS_data_type
       TYPE ( BNLS_subproblem_data_type ) :: subproblem_data
...
! G A L A H A D - N E W T O N _ B N L S _ I N I T I A L I Z E  S U B R O U T I N E

     SUBROUTINE BNLS_subproblem_initialize( data, control, inform )

!  *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*

!   Provide default values for BNLS controls
...
!-----------------------------------------------
     ...
     TYPE ( BNLS_subproblem_control_type ), INTENT( OUT ) :: control
     TYPE ( BNLS_subproblem_inform_type ), INTENT( OUT ) :: inform
     TYPE ( BNLS_subproblem_data_type ), INTENT( INOUT ) :: data
...
!  End of subroutine BNLS_subproblem_initialize

     END SUBROUTINE BNLS_subproblem_initialize
...
!-----------------------------------------------
     ...
     TYPE ( BNLS_control_type ), INTENT( OUT ) :: control
     TYPE ( BNLS_inform_type ), INTENT( OUT ) :: inform
     TYPE ( BNLS_data_type ), INTENT( INOUT ) :: data

!  control parameters for the main iteration

     CALL BNLS_subproblem_initialize( data%BNLS_subproblem_data_type,            &
                                     control%BNLS_subproblem_control_type,      &
                                     inform%BNLS_subproblem_inform_type )

!  control parameters for the tensor subproblem solves

     CALL BNLS_subproblem_initialize( data%subproblem_data,                     &
...
!  End of subroutine BNLS_initialize

     END SUBROUTINE BNLS_initialize

!- G A L A H A D -  B N L S _ F U L L _ I N I T I A L I Z E  S U B R O U T I N E -

     SUBROUTINE BNLS_full_initialize( data, control, inform )
...
!-----------------------------------------------
     ...
     CALL BNLS_initialize( data%bnls_data, control, inform )

     RETURN

!  End of subroutine BNLS_full_initialize

     END SUBROUTINE BNLS_full_initialize

!-*-  B N L S _ N E W T O N _ R E A D _ S P E C F I L E  S U B R O U T I N E  -*-

     SUBROUTINE BNLS_subproblem_read_specfile( control, device, alt_specname )

!  Reads the content of a specification file, and performs the assignment of
!  values associated with given keywords to the corresponding control parameters

!  The default values as given by BNLS_initialize could (roughly)
!  have been set as:

! BEGIN BNLS SPECIFICATIONS (DEFAULT)
...
! END BNLS SPECIFICATIONS

!-----------------------------------------------
!   D u m m y   A r g u m e n t s
!-----------------------------------------------

     TYPE ( BNLS_subproblem_control_type ), INTENT( INOUT ) :: control
...
!-----------------------------------------------
     ...
     CHARACTER( LEN = 6 ), PARAMETER :: specname = 'BNLS'
...
!  End of subroutine BNLS_subproblem_read_specfile

     END SUBROUTINE BNLS_subproblem_read_specfile

!-*-*-*-*-   B N L S _ R E A D _ S P E C F I L E  S U B R O U T I N E  -*-*-*-*-

     SUBROUTINE BNLS_read_specfile( control, device, alt_specname )
...
!  reset control parameters for the main iteration
     ...
     CALL BNLS_subproblem_read_specfile( control%BNLS_subproblem_control_type,   &
                                        device, alt_specname )

!  reset control parameters for the tensor subproblem solves

     IF ( PRESENT( alt_specname ) ) THEN
       CALL BNLS_subproblem_read_specfile( control%subproblem_control, device,  &
            alt_specname = TRIM( alt_specname ) // '-BNLS-INNER' )
     ELSE
       CALL BNLS_subproblem_read_specfile( control%subproblem_control, device,  &
            alt_specname = 'BNLS-INNER' )
     END IF

     RETURN

!  End of subroutine BNLS_read_specfile
...
!  *-*-*-*-*-*-*-*-*-*-*-*-  A R G U M E N T S  -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
!
!  For full details see the specification sheet for GALAHAD_BNLS.
!
!  ** NB. default real/complex means double precision real/complex in
!  ** GALAHAD_BNLS_precision
!
...
!   not allocated.
!
! control is a scalar variable of type BNLS_control_type. See
!  BNLS_initialize for details
!
! inform is a scallar variable of type BNLS_inform_type. On initial entry,
...
!
!  data is a scalar variable of type BNLS_data_type used for internal data.
!
...
!   present, BNLS_solve will return to the user with inform%status = 7
...
!   If the evaluation is impossible at X, status should be set to a nonzero
!   value. If eval_SCALE is not present, BNLS_solve will return to the user
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ) :: i, j, ic, ir, l, ll, nroots
     REAL ( KIND = rp_ ) :: ared, prered, rounding, root1, root2, root3, alpha
     REAL ( KIND = rp_ ) :: c0, c1, c2, c3, sths, reg, val

     LOGICAL :: alive
     CHARACTER ( LEN = 6 ) :: char_iter, char_facts
     CHARACTER ( LEN = 80 ) :: array_name
!    REAL ( KIND = rp_ ), DIMENSION( nlp%n ) :: V
!    REAL ( KIND = rp_ ) :: H_dense( nlp%n, nlp%n )
...
!  branch to special code for Newton or Gauss-Newton variants
       ...
       IF (  control%model == first_order_model .OR.                           &
         ...
         data%subproblem_control = control%BNLS_subproblem_control_type
         data%n_or_gn = .TRUE.
...
!  s is stored in tm%X
     ...
     array_name = 'bnls: data%tensor_model%X'
...
!  r(s) is stored in tm%C
     ...
     array_name = 'bnls: data%tensor_model%C'
...
!  in tensor_model%J
     ...
     IF ( data%control%subproblem_control%subproblem_direct ) THEN
       ...
       array_name = 'bnls: data%tensor_model%J%row'
       ...
       array_name = 'bnls: data%tensor_model%J%col'
       CALL SPACE_resize_array( data%tensor_model%J%ne,                        &
       ...
       array_name = 'bnls: data%tensor_model%J%val'
       CALL SPACE_resize_array( data%tensor_model%J%ne,                        &
...
!  record its row and column indices
     ...
     IF ( data%map_h_to_jtj ) THEN
       ...
       array_name = 'bnls: data%tensor_model%H%val'
       ...
       SELECT CASE ( SMT_get( nlp%H%type ) )
       CASE ( 'SPARSE_BY_ROWS' )
         array_name = 'bnls: data%tensor_model%H%ptr'
         ...
         array_name = 'bnls: data%tensor_model%H%col'
       ...
       CASE ( 'COORDINATE' )
         array_name = 'bnls: data%tensor_model%H%row'
...
!  of HS into J and JT. Firstly, order JT by columns
     ...
     IF ( data%control%subproblem_control%subproblem_direct ) THEN
       CALL BNLS_set_map( nlp%P, data%JT, data%IW, data%PTR, data%ROW,          &
                         data%ORDER, .FALSE.,                                  &
                         data%control%deallocate_error_fatal,                  &
                         data%control%space_critical,  data%control%error,     &
...
!  compute the gradient stopping tolerance
          ...
          &   I0, ')', /, A, '  BNLS stopping tolerances (c,J''c/c) =',         &
...
!  local minimizer of phi(s) = 1/2||r(s)||^2_2 + 1/p weight ||s||_S^p
       ...
       IF ( .NOT. data%successful ) THEN
         IF ( data%inner_weight == zero ) THEN
           data%inner_weight = 0.0001_rp_
         ELSE
           data%inner_weight = data%inner_weight * 10.0_rp_
         END IF
       END IF
...
!write(6,*) ' into BNLS_subproblem_solve'
         CALL BNLS_subproblem_solve( data%tensor_model,                         &
                                    data%subproblem_control,                   &
                                    inform%subproblem_inform,                  &
                                    data%subproblem_data,                      &
                                    data%subproblem_userdata, W = W,           &
                                    stabilisation = data%regularization )
!write(6,*) ' out of BNLS_subproblem_solve, status = ', &
...
!  increase the regularization weight and try again
         ...
         IF ( inform%weight == zero ) THEN
           inform%weight = 0.0001_rp_
         ELSE
           inform%weight = inform%weight * 10.0_rp_
...
!  ==================================
 ...
 800 CONTINUE
     ...
     data%BNLS_subproblem_data_type%regularization_type = control%norm

 810 CONTINUE
     data%branch = data%branch_newton
!    WRITE(6,*) ' in data%branch ', data%branch
     CALL BNLS_subproblem_solve( nlp, data%subproblem_control,                  &
                            inform%BNLS_subproblem_inform_type,                 &
                            data%BNLS_subproblem_data_type, userdata, W = W,    &
                            eval_C = eval_C, eval_J = eval_J, eval_H = eval_H, &
                            eval_JPROD = eval_JPROD, eval_HPROD = eval_HPROD,  &
                            eval_SCALE = eval_SCALE )
!    WRITE( 6, * ) ' out data%branch ', &
!      data%branch, inform%BNLS_subproblem_inform_type%status
     data%branch_newton = data%branch
     SELECT CASE ( inform%BNLS_subproblem_inform_type%status )
     CASE ( 2 : 6, 8 )
       inform%status = inform%BNLS_subproblem_inform_type%status
...
!  -------------
 ...
 990 CONTINUE
     ...
     inform%time%clock_total = data%clock_record - data%clock_start
     IF ( data%printi ) THEN
       CALL SYMBOLS_status( inform%status, data%out, prefix, 'BNLS_solve' )
       WRITE( data%out, "( ' ' )" )
     END IF
     RETURN
...
!   not allocated.
!
! controls is a scalar variable of type BNLS_control_type. See
!  BNLS_initialize for details
!
! inform is a scalar variable of type BNLS_inform_type. On initial entry,
!  inform%status should be set to 1. On exit, the following components will
...
!   absent, weights of one will be used.
!
!  stabilisation is an optional scalar variable of type BNLS_regularization_type
...
!-----------------------------------------------
     ...
     TYPE ( NLPT_problem_type ), INTENT( INOUT ) :: nlp
     TYPE ( BNLS_subproblem_control_type ), INTENT( IN ) :: control
     TYPE ( BNLS_subproblem_inform_type ), INTENT( INOUT ) :: inform
     TYPE ( BNLS_subproblem_data_type ), INTENT( INOUT ) :: data
     TYPE ( GALAHAD_userdata_type ), INTENT( INOUT ) :: userdata
     REAL ( KIND = rp_ ), INTENT( IN ), OPTIONAL, DIMENSION( : ) :: W
     TYPE ( BNLS_regularization_data_type ), INTENT( IN ),                      &
...
!  record controls and ensure that data is consistent
     ...
     data%control%BNLS_subproblem_control_type = control
...
!   within that of J^T J
       ...
       IF ( data%stabilised ) THEN
         CALL BNLS_set_map( data%H, stabilisation%matrix, data%IW, data%PTR,    &
...
!  Non-executable statements
 ...
 !  End of subroutine BNLS_subproblem_solve

     END SUBROUTINE BNLS_subproblem_solve

!! G A L A H A D - B N L S _ u p d a t e _ h i s t o r y  S U B R O U T I N E
!
!     SUBROUTINE BNLS_update_history( history, max_hist, F_hist, F_ref, f )
!
!!-----------------------------------------------
!!   D u m m y   A r g u m e n t s
!!-----------------------------------------------
...
!-*-*-  G A L A H A D -  B N L S _ t e r m i n a t e  S U B R O U T I N E -*-*-
     ...
     SUBROUTINE BNLS_subproblem_terminate( data, control, inform )
...
!  real arrays
     ...
     array_name = 'bnls: data%tensor_model%G'
     ...
     array_name = 'bnls: data%tensor_model%H%type'
...
!  End of subroutine BNLS_terminate

     END SUBROUTINE BNLS_subproblem_terminate
...
!  Deallocate all arrays allocated within calls to the main algorithm
     ...
     CALL BNLS_subproblem_terminate( data%BNLS_subproblem_data_type,             &
                                    control%BNLS_subproblem_control_type,       &
                                    inform%BNLS_subproblem_inform_type )
     IF ( inform%BNLS_subproblem_inform_type%status /= GALAHAD_ok ) THEN
       inform%status = inform%BNLS_subproblem_inform_type%status
       RETURN
     END IF
...
!  Deallocate all arrays allocated within calls to subproblem solutions
     ...
     CALL BNLS_subproblem_terminate( data%subproblem_data,                      &
                                    control%subproblem_control,                &
                                    inform%subproblem_inform )
     RETURN
...
! -  G A L A H A D -  B N L S _ f u l l _ t e r m i n a t e  S U B R O U T I N E -

     SUBROUTINE BNLS_full_terminate( data, control, inform )
...
!  deallocate workspace
     ...
     CALL BNLS_terminate( data%bnls_data, control, inform )
...
!  deallocate any internal problem arrays
     ...
     array_name = 'bnls: data%nlp%H%type'
...
!  End of subroutine BNLS_full_terminate

     END SUBROUTINE BNLS_full_terminate
...
!  End of subroutine BNLS_import

     END SUBROUTINE BNLS_import

!-  G A L A H A D -  B N L S _ r e s e t _ c o n t r o l   S U B R O U T I N E  -

     SUBROUTINE BNLS_reset_control( control, data, status )

!  reset control parameters after import if required.
!  See BNLS_solve for a description of the required arguments
...
!  end of subroutine BNLS_reset_control
...
!-  G A L A H A D -  B N L S _ s o l v e _ reverse _ M A T  S U B R O U T I N E -
     ...
     SUBROUTINE BNLS_solve_reverse_with_mat( data, status, eval_status,         &
...
!-  G A L A H A D -  B N L S _ s o l v e _ reverse _ no _ mat  S U B R O U T I N E
...
!-  G A L A H A D -  B N L S _ i n f o r m a t i o n   S U B R O U T I N E  -
     ...
     SUBROUTINE BNLS_information( data, inform, status )

!  return solver information during or after solution by BNLS
!  See BNLS_solve for a description of the required arguments
...
!  end of subroutine BNLS_information

     END SUBROUTINE BNLS_information

!  End of module GALAHAD_BNLS

   END MODULE GALAHAD_BNLS_precision
