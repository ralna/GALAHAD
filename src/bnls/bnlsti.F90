...
#include "galahad_modules.h"
   ...
   REAL ( KIND = rp_ ), ALLOCATABLE, DIMENSION( : ) :: C, Y, W, U, V
...
! set up problem data
   ...
   ALLOCATE( X( n ), X_l( n ), X_u( n ), G( n ), H_diag( n ) )
   ALLOCATE( C( m ), Y( m ), W( m ) )
...
!    control%print_level = 1
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       st = ' C'
       CALL BNLS_import( control, data, status, n, m,                          &
                        'coordinate', J_ne, J_row, J_col, null,                &
                        'coordinate', H_ne, H_row, H_col, null,                &
                        'sparse_by_columns', P_ne, P_row, null, P_ptr,         &
                        W = W )
       CALL BNLS_solve_with_mat( data, userdata, status, X, C, G,              &
...
!                       'absent', P_ne, P_row, P_col, P_ptr,                   &
                         W = W )
       CALL BNLS_solve_without_mat( data, userdata, status, X, C, G,           &
     ...
     ELSE
       WRITE( 6, "( A2, ': BNLS_solve exit status = ', I0 )" ) st, inform%status
     END IF
!    WRITE( 6, "( ' X ', 3ES12.5 )" ) X
!    WRITE( 6, "( ' G ', 3ES12.5 )" ) G
...
!    control%print_level = 1
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       ...
       DO ! reverse-communication loop
         CALL BNLS_solve_reverse_with_mat( data, status, eval_status,          &
     ...
     CASE ( 5 ) ! access by products
       ...
       CALL BNLS_import( control, data, status, n, m,                          &
                        ...
                        'absent', P_ne, P_row, P_col, P_ptr,                   &
                         W = W )
       DO ! reverse-communication loop
         CALL BNLS_solve_reverse_without_mat( data, status, eval_status,       &
                                             X, C, G, transpose, U, V,         &
                                             Y, P_val )
...
!      control%maxit = 1
       CALL BNLS_import( control, data, status, n, m,                          &
                        'sparse_by_rows', J_ne, J_row, J_col, J_ptr,           &
                        W = W )
       CALL BNLS_solve_with_mat( data, userdata, status, X, C, G, RES, JAC )
     CASE ( 4 ) ! Newton model
       st = ' 4'
       CALL BNLS_import( control, data, status, n, m,                          &
                        'sparse_by_rows', J_ne, J_row, J_col, J_ptr,           &
                        'sparse_by_rows', H_ne, H_row, H_col, H_ptr,           &
                        W = W )
       CALL BNLS_solve_with_mat( data, userdata, status, X, C, G, RES,         &
                                JAC, HESS )
