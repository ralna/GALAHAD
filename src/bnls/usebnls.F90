...
!  This is the driver program for running BNLS for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ B N L S   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_BNLS( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNBNLS'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNBNLS.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'BNLS.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'BNLSRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'BNLSSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'BNLSSAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'BNLS '

!  ------------------ Open the specfile for bnls ----------------
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from usebnls: re-entry requested with',  &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from usebnls: variable named ',        &
...
!  end of function STRING_trim_integer_8
     ...
     END SUBROUTINE USE_BNLS

!  End of module USEBNLS

   END MODULE GALAHAD_USEBNLS_precision
