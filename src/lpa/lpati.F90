! THIS VERSION: GALAHAD 4.1 - 2022-12-21 AT 08:50 GMT.
#include "galahad_modules.h"
   PROGRAM GALAHAD_LPA_interface_test
...
!  =====================================
   ...
   DO data_storage_type = 1, 3
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       st = ' C'
       CALL LPA_import( control, data, status, n, m,                           &
                        'coordinate', A_ne, A_row, A_col, A_ptr )
       CALL LPA_solve_lp( data, status, G, f, A_val, C_l, C_u,                 &
     ...
     CASE ( 3 ) ! dense
       ...
       CALL LPA_solve_lp( data, status, G, f, A_dense, C_l, C_u,               &
                          X_l, X_u, X, C, Y, Z, X_stat, C_stat )
     END SELECT
     CALL LPA_information( data, inform, status )
     IF ( inform%status == 0 ) THEN
       WRITE( 6, "( A2, ':', I6, ' iterations. Optimal objective value = ',    &
     &    F5.2, ' status = ', I0 )" ) st, inform%iter, inform%obj, inform%status
     ELSE
       WRITE( 6, "( A2, ': LPA_solve exit status = ', I0 ) " ) st, inform%status
     END IF
   ...
   END PROGRAM GALAHAD_LPA_interface_test
