! THIS VERSION: GALAHAD 5.2 - 2025-04-29 AT 13:45 GMT.

#include "galahad_modules.h"

!-*-*-*-*-*-*-*-*-  G A L A H A D   R U N L P A _ Q P L I B  -*-*-*-*-*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
   ...
   PROGRAM RUNLPA_QPLIB_precision
   USE GALAHAD_KINDS_precision

!     -----------------------------------------
!    | Main program for the QPLIB problem-data |
!    | file interface to LPA, a simplex method |
...
!     INTEGER ( KIND = ip_ ) :: np1, npm
      ...
      INTEGER ( KIND = ip_ ) :: alloc_stat, newton, A_ne, iter
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 16 ) :: specname = 'RUNLPA'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNLPA.SPC'
...
!  If required, write a SIF file containing the presolved problem
        ...
        IF ( write_presolved_sif ) THEN
          CALL QPT_write_to_sif( prob, pname, pfilename, pfiledevice,          &
                                 .FALSE., .FALSE., LPA_control%infinity )
        END IF
      END IF
...
!       WRITE( 6, "( ' z ', /, (5ES12.4) )" ) prob%Z
        ...
        CALL LPA_solve( prob, data, LPA_control, LPA_inform, C_stat, B_stat )

        IF ( printo ) WRITE( out, " ( /, ' ** LPA solver used ** ' ) " )
        qfval = LPA_inform%obj ; newton = 0
...
!  End of RUNLPA_QPLIB_precision

   END PROGRAM RUNLPA_QPLIB_precision
