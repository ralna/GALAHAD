...
!   update released with GALAHAD Version 2.0. February 19th 2005
!
...
!        ---------------------------------------------
         ...
         INTEGER, ALLOCATABLE, DIMENSION( : ) :: group ! group(i) is the index of
         ...
         INTEGER, ALLOCATABLE, DIMENSION( : ) :: g_status ! .TRUE. if the group
                                        ! is sign restricted.  Only allocated
                                        ! if s%filter_sign = MIXED

         INTEGER, ALLOCATABLE, DIMENSION( : ) :: iw   ! integer worspace of size
                                        ! problem%m + 1

         INTEGER, ALLOCATABLE, DIMENSION( : ) :: row  ! integer worspace of size
                                        ! problem%J_ne

         INTEGER, ALLOCATABLE, DIMENSION( : ) :: perm ! integer worspace of size
                                        ! problem%J_ne
         ...
         REAL( KIND = wp ), ALLOCATABLE, DIMENSION( : ) :: fnorm_1, fnorm_2
                                        ! the filter (two hooks)

         REAL( KIND = wp ), ALLOCATABLE, DIMENSION( :, : ) :: filter_1, filter_2
                                        ! the filter (two hooks)

         REAL( KIND = wp ), ALLOCATABLE, DIMENSION( :, : ) :: offdiag ! workspace
...
!     Return codes
      ...
      INTEGER, PRIVATE, PARAMETER :: UNALLOCATED_INPUT           = -23
...
!------------------------------------------------------------------------------
      ...
      IF ( .NOT. ALLOCATED(  problem% x ) ) THEN
         inform%status = UNALLOCATED_INPUT
...
!        Allocate the group index.
         ...
         IF ( ALLOCATED( s%aut_group ) ) DEALLOCATE( s%aut_group )
...
!*****************************************************
      ...
      IF ( inform%status == GET_JTC_0 ) inform%status = GET_JTC
...
!------------------------------------------------------------------------------
     ...
     IF ( s%level >= TRACE ) THEN
        ...
        WRITE( s%out, 1100 )
        WRITE( s%out, 1099 )
        WRITE( s%out, 1001 ) inform%nbr_iterations, problem%f, s%g_norm,       &
...
!                Add the terms collected in s%r, if any.
                 ...
                 IF ( problem%m > 0 ) THEN
                    IF ( s%model_used == NEWTON .OR. s%has_bounds ) THEN
                       s%w( 1:problem%n ) = s%w( 1:problem%n ) + s%r
                    END IF
                 ELSE
                    s%w( 1:problem%n ) = s%r
                 END IF
...
!********************************************************
        ...
        IF ( s%acceptable ) THEN
           IF ( inform%status == GET_JTC_A ) inform%status = GET_JTC
           CALL FILTRANE_compute_grad_f
           SELECT CASE ( inform%status )
           CASE ( :-1 )
              GO TO 999
...
!*****************************************************
     ...
     IF ( s%best_x_is_past ) THEN
        IF ( inform%status == GET_JTC_F ) inform%status = GET_JTC
        CALL FILTRANE_compute_grad_f
...
!    Print the final messages.
     ...
     CALL FILTRANE_say_goodbye( control, inform, s, inform%status == 0 )
...
1001 FORMAT(1x,i4,1x,1pE11.3,1x,1pE10.3,22x,1E11.3,1x,i6,6x,i3)
1002 FORMAT(1x,i4,1x,1pE11.3,4(1x,1pE10.3),1x,i6,1x,a4,1x,i3 )
1003 FORMAT(4x,'============================================================')
...
1021 FORMAT(4x,'REVERSE COMMUNICATION REENTRY (inform%status = unknown)')
...
1001  FORMAT(2x,'now start printing at iteration',i6)
1002  FORMAT(2x,'start printing at iteration',i6)
...
1009  FORMAT(2x,'now stop printing at iteration',i6)
1010  FORMAT(2x,'stop printing at iteration',i6)
...
!     Local variables
      ...
      INTEGER, DIMENSION( : ), ALLOCATABLE :: fnext
      REAL ( KIND = wp ), DIMENSION( : ),   ALLOCATABLE :: fnorm
      REAL ( KIND = wp ), DIMENSION( :,: ), ALLOCATABLE :: filter
...
!     Verify acceptability for the current filter.
...
!     Clean up the various workspace arrays.
      ...
      IF ( ALLOCATED( s%step      ) ) DEALLOCATE( s%step      )
      IF ( ALLOCATED( s%theta     ) ) DEALLOCATE( s%theta     )
      IF ( ALLOCATED( s%group     ) ) DEALLOCATE( s%group     )
      IF ( ALLOCATED( s%g_status  ) ) DEALLOCATE( s%g_status  )
      IF ( ALLOCATED( s%iw        ) ) DEALLOCATE( s%iw        )
      IF ( ALLOCATED( s%row       ) ) DEALLOCATE( s%row       )
      IF ( ALLOCATED( s%perm      ) ) DEALLOCATE( s%perm      )
      IF ( ALLOCATED( s%diag      ) ) DEALLOCATE( s%diag      )
      IF ( ALLOCATED( s%offdiag   ) ) DEALLOCATE( s%offdiag   )
      IF ( ALLOCATED( s%vote      ) ) DEALLOCATE( s%vote      )
      IF ( ALLOCATED( s%best_x    ) ) DEALLOCATE( s%best_x    )
      IF ( ALLOCATED( s%filter_1  ) ) DEALLOCATE( s%filter_1  )
      IF ( ALLOCATED( s%filter_2  ) ) DEALLOCATE( s%filter_2  )
      IF ( ALLOCATED( s%active_1  ) ) DEALLOCATE( s%active_1  )
      IF ( ALLOCATED( s%active_2  ) ) DEALLOCATE( s%active_2  )
...
!==============================================================================
      ...
      SUBROUTINE FILTRANE_say_goodbye( control, inform, s, quiet )
...
!              if present and .TRUE., suppress all output, except
!              for error messages. It superseeds s%level and
!              control%print_level.
...
!     Terminate GLTR, if needed
      ...
      IF ( s%gltr_initialized ) THEN
         ...
         s%gltr_initialized = .FALSE.
         IF ( level >= DEBUG ) WRITE( s%out, 1004 )
      END IF
