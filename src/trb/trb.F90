! THIS VERSION: GALAHAD 5.3 - 2025-10-01 AT 13:50 GMT.
...
!    USE HSL_MI13
     ...
     PUBLIC :: TRB_initialize, TRB_read_specfile, TRB_solve,                   &
               TRB_projection, TRB_terminate, TRB_information,                 &
               ...
               NLPT_problem_type, USERDATA_type, SMT_type, SMT_put
...
#else
       REAL ( KIND = rp_ ) :: stop_pg_absolute = tenm5
       REAL ( KIND = rp_ ) :: stop_pg_relative = tenm8
#endif
...
!   required relative reduction in the resuiduals from CG
       ...
       REAL ( KIND = rp_ ) :: stop_rel_cg = 0.01_rp_
...
!   should an ellipsoidal trust-region be used rather than an infinity norm one?
       ...
       LOGICAL :: two_norm_tr = .FALSE.   !! no longer used
...
!  - - - - - - - - - -
     ...
     TYPE, PUBLIC :: TRB_data_type
       ...
       INTEGER ( KIND = ip_ ) :: nprec, nskip_lbfgs, nskip_prec, jumpto
       INTEGER ( KIND = ip_ ) :: h_ne, n_prods, more_toraldo_its, nfree_cp
...
!      INTEGER ( KIND = ip_ ), POINTER :: nnz_hp => NULL( )
       ...
       REAL ( KIND = rp_ ) :: f_ref, f_trial, f_best, m_best, ratio
       REAL ( KIND = rp_ ) :: radius, old_radius, radius_trial, etat, ometat
...
!      LOGICAL, POINTER :: got_h => NULL( )
       ...
       INTEGER ( KIND = ip_ ), ALLOCATABLE, DIMENSION( : ) :: INDEX_used_hp
       INTEGER ( KIND = ip_ ), ALLOCATABLE, DIMENSION( : ) :: POSITION_diagonal
       INTEGER ( KIND = ip_ ), ALLOCATABLE, DIMENSION( : ) :: FIX
       ...
       REAL ( KIND = rp_ ), ALLOCATABLE, DIMENSION( : ) :: X_cauchy
       ...
       REAL ( KIND = rp_ ), ALLOCATABLE, DIMENSION( : ) :: DX_bqp
       ...
       TYPE ( SMT_type ) :: H_by_cols, H_sub, M_sub
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ), PARAMETER :: more_toraldo = alive_unit + 1
     ...
     INTEGER ( KIND = ip_ ), PARAMETER :: infinity = advanced_start + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: stop_pg_absolute = infinity + 1
     ...
     INTEGER ( KIND = ip_ ), PARAMETER :: stop_rel_cg = maximum_radius + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: eta_successful = stop_rel_cg + 1
     ...
     INTEGER ( KIND = ip_ ), PARAMETER :: two_norm_tr = renormalize_radius + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: exact_gcp = two_norm_tr + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: accurate_bqp = exact_gcp + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: space_critical = accurate_bqp + 1
...
!  branch to different sections of the code depending on input status
     ...
     CASE ( 425 ) ! Hessian-vector product
       GO TO 425
     CASE ( 428 ) ! Hessian-vector product
       GO TO 428
     CASE ( 440 ) ! Hessian-vector product
       GO TO 440
     CASE ( 520 ) ! objective evaluation
       GO TO 520
     CASE ( 540 ) ! Hessian-vector product
       GO TO 540
     CASE ( 550 ) ! gradient evaluation
       GO TO 550
     END SELECT
...
!  ensure that the data is consistent
     ...
     data%control%two_norm_tr = .FALSE.
...
!    END IF
     ...
     CALL SPACE_resize_array( nlp%n, data%M_sub%val, inform%status,            &
            inform%alloc_status, array_name = array_name,                      &
            deallocate_error_fatal = control%deallocate_error_fatal,           &
            exact_size = control%space_critical,                               &
            bad_alloc = inform%bad_alloc, out = control%error )
     ...
     CALL SMT_put( data%M_sub%type, 'DIAGONAL', inform%alloc_status )
...
!  if needed, record the diagonal
           ...
           END IF
           IF ( data%diagonal_preconditioner ) data%H_diagonal( : nlp%n ) =    &
               MAX( epsmch ** 0.3333, ABS( data%H_diagonal( : nlp%n ) ) )
         END IF
       END IF
...
!  set the bounds on the variables for the model problem. The bounds are the 
...
!  the exact generalized Cauchy point is required
         ...
         IF ( data%control%exact_gcp ) THEN
           CALL CAUCHY_get_exact_gcp(                                          &
               ...
               epsmch, .FALSE., data%dxsqr, data%radius,                       &
...
!  an approximation to the Cauchy point suffices
         ...
         ELSE
           CALL CAUCHY_get_approx_gcp(                                         &
               ...
               .FALSE., data%radius, data%model, data%P,                       &
...
!  write(6,*) ' dx', MAXVAL( ABS( data%X_trial - data%X_current ) )
...
!  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         ...
         data%itercg = 0
         IF ( COUNT( data%X_status( : nlp%n ) == 0 ) == 0 ) GO TO 500
...
!  is the reciprocal of the square of the distance to the nearest bound
           ...
           DO i = 1, nlp%n
             IF ( data%X_status( i ) == 0 ) THEN
               ii = ii + 1
               val = MIN( data%X_trial( i ) - data%BND( i, 1 ),                &
                          data%BND( i, 2 ) - data%X_trial( i ) )
               data%M_sub%val( ii ) = one / val ** 2
               delta = MIN( delta, val )
             END IF
           END DO
!WRITE( 6, "( ' distance to nearest inactive bound = ', ES11.4 )" )  delta
...
!          CALL TRS_solve( data%n_sub, data%radius_sub, data%f_sub,            &
           CALL TRS_solve( data%n_sub, one, data%f_sub,                        &
...
!            ( inform%iter - 1 ) + inform%TRS_inform%factorizations ) /        &
!            inform%iter
...
!  compute the diagonal scaling matrix M_sub, so that each diagonal for free
!  variables is the reciprocal of the square of the distance to the nearest 
!  bound
           ...
           DO i = 1, nlp%n
             IF ( data%X_status( i ) == 0 ) THEN
               val = MIN( data%X_trial( i ) - data%BND( i, 1 ),                &
                          data%BND( i, 2 ) - data%X_trial( i ) )
               data%M_sub%val( i ) = one / val ** 2
               delta = MIN( delta, val )
             ELSE
               data%M_sub%val( i ) = one
             END IF
           END DO
...
!write(6,*) ' ||s||, Delta = ', inform%GLTR_inform%mnormx, delta
...
!  evaluate the objective function at the trial point
       ...
       IF ( data%reverse_f ) THEN
         data%branch = 520 ; inform%status = 2 ; RETURN
       ELSE
...
!  if necessary, return to the user to obtain the model Hessian product with s
           ...
           ELSE
             IF ( data%reverse_hprod ) THEN
               ...
               data%branch = 540 ; inform%status = 5 ; RETURN
             ELSE
...
!  Non-executable statements
 ...
 2040 FORMAT( A, 'It              f       pgrad  ',                            &
              ' ratio   ||s||  radius  #active gltr its  time' )
 ...
 2080 FORMAT( A, A6, 3A1, ES12.4, ES8.1, ES9.1, 2ES8.1, A7, 1X, A7, F8.2 )
 2090 FORMAT( A, A6, 3X,  ES12.4, ES8.1, 9X, 8X, ES8.1, A7 )
...
!  Deallocate all remaining allocated arrays
     ...
     array_name = 'trb: data%M_sub%VAL'
     CALL SPACE_dealloc_array( data%M_sub%VAL,                                 &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'trb: data%M_sub%type'
     CALL SPACE_dealloc_array( data%M_sub%type,                                &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN
...
!  deallocate workspace
     ...
     CALL TRB_terminate( data%trb_data, data%trb_control, data%trb_inform )
...
!  deallocate any internal problem arrays
     ...
     array_name = 'trb: data%nlp%H%type'
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ), INTENT( IN    ) :: n, nnz_p, n_prods
     INTEGER ( KIND = ip_ ), INTENT( INOUT ) :: nnz_hp
     LOGICAL, INTENT( IN    ) :: dense_p
     INTEGER ( KIND = ip_ ), INTENT( IN    ), DIMENSION( : ) :: INDEX_nz_p
...
!-----------------------------------------------
     ...
     REAL ( KIND = rp_ ), DIMENSION( n ) :: TRB_projection
...
!-*-*-  T R B _ r e d u c e d  _ g r a d i e n t _ n o r m  F U C T I O N  -*-*-
     ...
     REAL ( KIND = rp_ ) :: TRB_reduced_gradient_norm
...
!  collect data for reverse communication
     ...
     CASE( 6 )
       V( : data%nlp%n ) = data%trb_data%V( : data%nlp%n )
     CASE( 7 )
       nnz_v = data%trb_data%nnz_p_u - data%trb_data%nnz_p_l + 1
       INDEX_nz_v( : nnz_v ) =                                                 &
          data%trb_data%INDEX_nz_p( data%trb_data%nnz_p_l :                    &
                                    data%trb_data%nnz_p_u )
...
!  End of module GALAHAD_TRB
   ...
   END MODULE GALAHAD_TRB_precision
