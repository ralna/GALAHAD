...
!  ordering is requested, so only the compiled ones must be listed
   ...
   DO iord = 1, n_pord
     ...
     control%new_a = 2 ; control%new_h = 2 ; control%new_c = 2
     CALL SBLS_form_and_factorize( n, m, H, A, C, data, control, info )
     IF ( info%status < 0 ) THEN
       WRITE( 6, "( I9, I9 )" ) pord_list( iord ), info%status
     ELSE
       SOL( : n ) = (/ 0.0_rp_, 2.0_rp_, 0.0_rp_ /)
       SOL( n + 1 : ) = (/ 2.0_rp_, 1.0_rp_ /)
       CALL SBLS_solve( n, m, A, C, data, control, info, SOL )
       WRITE( 6, "( I9, I9, A9 )" ) pord_list( iord ), info%status,            &
         type_residual( info%norm_residual )
     END IF
     CALL SBLS_terminate( data, control, info )
   END DO
   IF ( n_pord > 0 ) THEN
     ...
     DEALLOCATE( C%val, C%row, C%col, SOL )
   END IF
