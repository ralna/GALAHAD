...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
   ...
   MODULE GALAHAD_USEWCP_precision
...
!  Scalars
      ...
      INTEGER ( KIND = ip_ ) :: status, cutest_status, iter
!     INTEGER ( KIND = ip_ ) :: np1, npm
      INTEGER ( KIND = ip_ ) :: mfixed, mdegen, nfacts, nfixed, ndegen, mequal
      INTEGER ( KIND = ip_ ) :: alloc_stat, A_ne, smt_stat
...
!  Allocate suitable arrays
      ...
      ALLOCATE( prob%C_l( m ), prob%C_u( m ), prob%Y_l( m ), prob%Y_u( m ),    &
                CNAME( m ), EQUATN( m ), LINEAR( m ), STAT = alloc_stat )
      IF ( alloc_stat /= 0 ) THEN
...
!  separable function.
      ...
      CALL CUTEST_csetup_r( cutest_status, input, out, io_buffer, n, m,        &
...
!  Determine the number of nonzeros in the Jacobian
      ...
      CALL CUTEST_cdimsj_r( cutest_status, la ) ; la = MAX( la, 1 )
      IF ( cutest_status /= 0 ) GO TO 910
...
!  Evaluate the linear terms of the constraint functions
      ...
      CALL CUTEST_csgr_r( cutest_status, n, m, prob%X0, prob%Y_l, .FALSE.,     &
...
!  End of module USEWCP

   END MODULE GALAHAD_USEWCP_precision
