...
! sparse co-ordinate storage format
   ...
   ALLOCATE( p%L%val( a_ne ), p%L%row( a_ne ), p%L%col( a_ne ) )
...
!  ALLOCATE( p%L%val( a_ne ), p%L%col( a_ne ), p%L%ptr( m + 1 ) )
!  p%H%val = (/ 1.0_wp, 2.0_wp, -1.0_wp, 5.0_wp, 4.0_wp /) ! Hessian H
...
! re-ordered matrices
   ...
   WRITE( 6, 30 ) 'Jacobian', ( ( 'A', i, p%L%col( j ), p%L%val( j ),          &
     j = p%L%ptr( i ), p%L%ptr( i + 1 ) - 1 ), i = 1, p%m )  ! Jacobian
