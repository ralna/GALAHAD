! THIS VERSION: GALAHAD 2.1 - 22/03/2007 AT 09:00 GMT.
 ...
 WRITE( 6, "( /, ' Order real values ', / )" )
 CALL SORT_heapsort_build( n, A, inform, INDA ) !  Build the heap
 DO i = 1, n
   m = n - i + 1 
   CALL SORT_heapsort_smallest( m, A, inform, INDA ) !  Reorder the variables
 ...
 WRITE( 6, "( /, ' Order integer values ', / )" )
 CALL SORT_heapsort_build( n, IA, inform, INDA ) !  Build the heap
 DO i = 1, n
   m = n - i + 1 
   CALL SORT_heapsort_smallest( m, IA, inform, INDA ) !  Reorder the variables
   WRITE( 6, "( ' The ', I2, '-th(-st) smallest value, a(', I2, ') is ',       &
  &       I3 ) " ) i, INDA( m ), IA( m ) 
 END DO  
 STOP 
