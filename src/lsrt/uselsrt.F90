...
!-*-*-*-*-*-*-*-  G A L A H A D   U S E L S R T   M O D U L E  -*-*-*-*-*-*-*-*-
...
!-*-*-*-*-*-*-*-*-*-   U S E _ C Q P  S U B R O U T I N E   -*-*-*-*-*-*-*-*-
     ...
     SUBROUTINE USE_LSRT( input )

!  --------------------------------------------------------------------
!
!  Form a multilevel incomplete QR factorization of a rectangular
!  matrix A using the GALAHAD package GALAHAD_LSRT
!
!  --------------------------------------------------------------------
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 16 ) :: specname = 'RUNLSRT'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNLSRT.SPC'
      CHARACTER ( LEN = 4 ) :: rowcol

!  The default values for LSRT could have been set as:

! BEGIN RUNLSRT SPECIFICATIONS (DEFAULT)
!  write-problem-data                            NO
!  problem-data-file-name                        LSRT.data
...
!  write-solution                                NO
!  solution-file-name                            LSRTSOL.d
!  solution-file-device                          62
!  write-result-summary                          NO
!  result-summary-file-name                      LSRTRES.d
!  result-summary-file-device                    47
! END RUNLSRT SPECIFICATIONS
...
!     INTEGER ( KIND = ip_ ) :: lfiledevice = 78
      ...
      CHARACTER ( LEN = 30 ) :: dfilename = 'LSRT.data'
      CHARACTER ( LEN = 30 ) :: ifilename = 'INITIAL.SIF'
      CHARACTER ( LEN = 30 ) :: rfilename = 'LSRTRES.d'
      CHARACTER ( LEN = 30 ) :: sfilename = 'LSRTSOL.d'
...
!  Set all default values, and override defaults if requested
      ...
      CALL LSRT_initialize( LSRT_data, LSRT_control, LSRT_inform )

      IF ( is_specfile )                                                       &
        CALL LSRT_read_specfile( LSRT_control, input_specfile )

      printo = out > 0 .AND. LSRT_control%print_level > 0
      printe = out > 0 .AND. LSRT_control%print_level >= 0
...
!       solv = ' LSRT'
!       IF ( printo ) WRITE( out, " ( ' ** LSRT solver used ** ' ) " )
...
!  Iteration to find the minimizer
          ...
          LSRT_inform%status = 1
          U( : A_ls%m ) = B( : A_ls%m )

          DO
            CALL LSRT_solve( A_ls%m, A_ls%n, order, weight, X( : A_ls%n ),     &
                             U( : A_ls%m ), V( : A_ls%n ), LSRT_data,          &
                             LSRT_control, LSRT_inform )
            branch = LSRT_inform%status
...
!  Compute the residuals for checking
              ...
              IF ( printe ) THEN
               ...
               &  ' 2nd pass LSRT iterations, overall time = ', F6.2 )" )      &
                  LSRT_inform%iter, LSRT_inform%iter_pass2, clockt - clocks
                WRITE( out, "( '  ||b||   calculated              =',          &
               &  ES15.8 )" ) SQRT( DOT_PRODUCT( B( : A_ls%m ), B( : A_ls%m)))
                WRITE( out, "( '  ||x||_P recurred and calculated =',          &
               &  2ES15.8 )" ) LSRT_inform%x_norm, x_norm
                WRITE( out, "( ' ||Ax-b|| recurred and calculated =',          &
               &  2ES15.8 )" ) LSRT_inform%r_norm,                             &
...
!  Error returns
            ...
            CASE DEFAULT
              CALL CPU_TIME( timet ) ; CALL CLOCK_time( clockt )
              IF ( printe ) WRITE( out, "( ' LSRT_solve exit status = ', I0 )")&
                LSRT_inform%status
              EXIT
...
!  Iteration to find the minimizer
          ...
          DO
            CALL LSRT_solve( m_used, n_used, order, weight, X, U, V,           &
                             LSRT_data, LSRT_control, LSRT_inform )
            branch = LSRT_inform%status
...
!          pname, n, m, iter, qfval, status, clockt
        IF ( LSRT_inform%status >= 0 ) THEN
          WRITE( rfiledevice, "( A10, 2I8, I3, L2, I10, I8, 2F12.2, I4 )" )    &
            pname, m_used, n_used, preconditioner, consistent, n_fact,         &
            LSRT_inform%iter, clockp - clocks,                                 &
            clockt - clocks, LSRT_inform%status
        ELSE
          WRITE( rfiledevice, "( A10, 2I8, I3, L2, I10, I8, 2F12.2, I4 )" )    &
            pname, m_used, n_used, preconditioner, consistent, - n_fact,       &
            - LSRT_inform%iter,                                                &
            - ( clockp - clocks ), - ( clockt - clocks ), - LSRT_inform%status
        END IF
      END IF
      CALL LSRT_terminate( LSRT_data, LSRT_control, LSRT_inform )
      GO TO 600
...
!  End of subroutine USE_LSRT

     END SUBROUTINE USE_LSRT

!  End of module USELSRT

   END MODULE GALAHAD_USELSRT_precision
