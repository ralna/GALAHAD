...
!-*-*-*-*-*-*-*-  G A L A H A D _  L S R T    C   I N T E R F A C E  -*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_LSRT_double_ciface
    USE iso_c_binding
    USE GALAHAD_common_ciface
    USE GALAHAD_LSRT_double, ONLY:                                             &
        f_lsrt_control_type => LSRT_control_type,                              &
        f_lsrt_inform_type => LSRT_inform_type,                                &
        f_lsrt_full_data_type => LSRT_full_data_type,                          &
        f_lsrt_initialize => LSRT_initialize,                                  &
        f_lsrt_read_specfile => LSRT_read_specfile,                            &
        f_lsrt_import_control => LSRT_import_control,                          &
        f_lsrt_solve_problem => LSRT_solve_problem,                            &
        f_lsrt_information => LSRT_information,                                &
        f_lsrt_terminate => LSRT_terminate
...
!-------------------------------------------------
    ...
    TYPE, BIND( C ) :: lsrt_control_type
      ...
      INTEGER ( KIND = C_INT ) :: stopping_rule
    ...
    END TYPE lsrt_control_type

    TYPE, BIND( C ) :: lsrt_inform_type
...
!  copy C control parameters to fortran
    ...
    TYPE ( lsrt_control_type ), INTENT( IN ) :: ccontrol
    TYPE ( f_lsrt_control_type ), INTENT( OUT ) :: fcontrol
...
!  copy fortran control parameters to C
    ...
    TYPE ( f_lsrt_control_type ), INTENT( IN ) :: fcontrol
    TYPE ( lsrt_control_type ), INTENT( OUT ) :: ccontrol
...
!  copy C inform parameters to fortran
    ...
    TYPE ( lsrt_inform_type ), INTENT( IN ) :: cinform
    TYPE ( f_lsrt_inform_type ), INTENT( OUT ) :: finform
    INTEGER :: i
...
!  copy fortran inform parameters to C
    ...
    TYPE ( f_lsrt_inform_type ), INTENT( IN ) :: finform
    TYPE ( lsrt_inform_type ), INTENT( OUT ) :: cinform
    INTEGER :: i, l
  ...
  END MODULE GALAHAD_LSRT_double_ciface

!  --------------------------------------
!  C interface to fortran lsrt_initialize
!  --------------------------------------

  SUBROUTINE lsrt_initialize( cdata, ccontrol, status ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE
...
!  local variables
  ...
  TYPE ( f_lsrt_full_data_type ), POINTER :: fdata
  TYPE ( f_lsrt_control_type ) :: fcontrol
  TYPE ( f_lsrt_inform_type ) :: finform
  LOGICAL :: f_indexing
...
!  initialize required fortran types
  ...
  CALL f_lsrt_initialize( fdata, fcontrol, finform )
...
!  -----------------------------------------
!  C interface to fortran lsrt_read_specfile
!  -----------------------------------------

  SUBROUTINE lsrt_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( lsrt_control_type ), INTENT( INOUT ) :: ccontrol
...
!  read control parameters from the specfile
  ...
  CALL f_lsrt_read_specfile( fcontrol, device )
...
!  ------------------------------------------
!  C interface to fortran lsrt_import_control
!  ------------------------------------------

  SUBROUTINE lsrt_import_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE
...
!  import the problem data into the required LSRT structure

   CALL f_lsrt_import_control( fcontrol, fdata, status )
...
!  -----------------------------------------
!  C interface to fortran lsrt_solve_problem
!  -----------------------------------------

  SUBROUTINE lsrt_solve_problem( cdata, status, m, n, power, weight,           &
                                 x, u, v ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  REAL ( KIND = wp ), INTENT( IN ), VALUE :: power, weight
...
!  solve the qp
  ...
  CALL f_lsrt_solve_problem( fdata, status, m, n, power, weight, x, u, v )
  RETURN

  END SUBROUTINE lsrt_solve_problem

!  ---------------------------------------
!  C interface to fortran lsrt_information
!  ---------------------------------------

  SUBROUTINE lsrt_information( cdata, cinform, status ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( lsrt_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = C_INT ), INTENT( OUT ) :: status
...
!  obtain LSRT solution information

  CALL f_lsrt_information( fdata, finform, status )
...
!  -------------------------------------
!  C interface to fortran lsrt_terminate
!  -------------------------------------

  SUBROUTINE lsrt_terminate( cdata, ccontrol, cinform ) BIND( C )
  USE GALAHAD_LSRT_double_ciface
  IMPLICIT NONE
...
!  deallocate workspace
  ...
  CALL f_lsrt_terminate( fdata, fcontrol, finform )
