...
!  C interface module to GALAHAD_TRU types and interfaces
  ...
  MODULE GALAHAD_TRU_precision_ciface
    USE GALAHAD_KINDS_precision
    USE GALAHAD_common_ciface
    USE GALAHAD_TRU_precision, ONLY:                                           &
    ...
    USE GALAHAD_SEC_precision_ciface, ONLY:                                    &
        sec_inform_type,                                                       &
        sec_control_type,                                                      &
...
!  non-reentrant (one active solve per process).
    ...
    TYPE ( C_FUNPTR ) :: tru_saved_ceval_f = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: tru_saved_ceval_g = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: tru_saved_ceval_h = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: tru_saved_ceval_prec = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: tru_saved_ceval_hprod = C_NULL_FUNPTR
    INTEGER ( KIND = ipc_ ) :: tru_saved_n
    TYPE ( C_PTR ) :: tru_saved_cuserdata
    INTEGER ( KIND = ipc_ ) :: tru_saved_ne
  CONTAINS
...
!  copy C control parameters to fortran
  ...
  ! Integers
    ...
    CALL copy_sec_control_in( ccontrol%sec_control,fcontrol%sec_control )
...
!  copy fortran control parameters to C
  ...
  ! C or Fortran sparse matrix indexing
    ...
    CALL copy_sec_control_out( fcontrol%sec_control,ccontrol%sec_control )
...
!  copy C information parameters to fortran
    ...
    CALL copy_sec_inform_in( cinform%sec_inform, finform%sec_inform )
...
!  copy fortran information parameters to C
    ...
    CALL copy_sec_inform_out( finform%sec_inform, cinform%sec_inform )
...
!  call C interoperable eval_f
    ...
    PROCEDURE( eval_f ), POINTER :: tru_saved_eval_f
    CALL C_F_PROCPOINTER( tru_saved_ceval_f, tru_saved_eval_f )
    status = tru_saved_eval_f( tru_saved_n, x, f, tru_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_g
    ...
    PROCEDURE( eval_g ), POINTER :: tru_saved_eval_g
    CALL C_F_PROCPOINTER( tru_saved_ceval_g, tru_saved_eval_g )
    status = tru_saved_eval_g( tru_saved_n, x, g, tru_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_h
    ...
    PROCEDURE( eval_h ), POINTER :: tru_saved_eval_h
    CALL C_F_PROCPOINTER( tru_saved_ceval_h, tru_saved_eval_h )
    status = tru_saved_eval_h( tru_saved_n, tru_saved_ne, x, hval, tru_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_prec
    ...
    PROCEDURE( eval_prec ), POINTER :: tru_saved_eval_prec
    CALL C_F_PROCPOINTER( tru_saved_ceval_prec, tru_saved_eval_prec )
    status = tru_saved_eval_prec( tru_saved_n, x, u, v, tru_saved_cuserdata )
    RETURN
...
!  call C interoperable eval_hprod
    ...
    PROCEDURE( eval_hprod ), POINTER :: tru_saved_eval_hprod
    CALL C_F_PROCPOINTER( tru_saved_ceval_hprod, tru_saved_eval_hprod )
    IF ( PRESENT( fgot_h ) ) THEN
      cgot_h = fgot_h
    ELSE
      cgot_h = .false.
    END IF
    status = tru_saved_eval_hprod( tru_saved_n, x, u, v, cgot_h, tru_saved_cuserdata )
    RETURN

    END SUBROUTINE wrap_eval_hprod

  END MODULE GALAHAD_TRU_precision_ciface
...
!  -------------------------------------
  ...
  USE GALAHAD_TRU_precision_ciface
  IMPLICIT NONE
...
!  associate procedure pointers
  ...
  tru_saved_ceval_f = ceval_f
  tru_saved_ceval_g = ceval_g
  tru_saved_ceval_h = ceval_h
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    tru_saved_ceval_prec = ceval_prec
  ELSE
    tru_saved_ceval_prec = C_NULL_FUNPTR
  END IF

  tru_saved_n = n
  tru_saved_cuserdata = cuserdata
  tru_saved_ne = ne

!  solve the problem when the Hessian is explicitly available

  IF ( C_ASSOCIATED( tru_saved_ceval_prec ) ) THEN
    CALL f_tru_solve_with_mat( fdata, fuserdata, status, x, g,                 &
...
!  associate procedure pointers
  ...
  tru_saved_ceval_f = ceval_f
  tru_saved_ceval_g = ceval_g
  tru_saved_ceval_hprod = ceval_hprod
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    tru_saved_ceval_prec = ceval_prec
  ELSE
    tru_saved_ceval_prec = C_NULL_FUNPTR
...
!  solve the problem when the Hessian is only available via products
  ...
  IF ( C_ASSOCIATED( tru_saved_ceval_prec ) ) THEN
    CALL f_tru_solve_without_mat( fdata, fuserdata, status, x, g,              &
