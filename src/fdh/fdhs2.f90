...
!  INTEGER, PARAMETER :: n = 5, nz = 9
   ...
   INTEGER :: i, status
   INTEGER, ALLOCATABLE, DIMENSION( : ) :: DIAG, ROW
...
!  REAL ( KIND = wp ) ::  H( nz )
   ...
   END INTERFACE
! start problem data
! Record the sparsity pattern of the Hessian (lower triangle)
   READ( 5, * ) n, nz
   ALLOCATE( DIAG( n ), X1( n ), X2( n ), STEPSIZE( n ), G( n ) )
   ALLOCATE( ROW( nz ), H( nz ) )
   DO i = 1, n
     READ( 5, * ) DIAG( i )
   END DO
   DO i = 1, nz
