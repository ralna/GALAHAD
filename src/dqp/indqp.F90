...
!    | to DQP, a dual gradient-projection method        |
...
!  Specfile characteristics
      ...
      INTEGER ( KIND = ip_ ), PARAMETER :: lspec = 27
      CHARACTER ( LEN = 16 ) :: specname = 'RUNDQP'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNDQP.SPC'
...
!  check that the problem is a convex QP
      ...
      CASE ( 'Q' )
       ...
       &  ' might not be strictly convex' )" ) TRIM( pname )
      CASE DEFAULT
        WRITE( out, "( /, ' ** Problem ', A, ', objective function',           &
       &  ' is not strictly convex. Stopping' )" ) TRIM( pname )
        STOP
      END SELECT
...
!  refactorize H
          ...
          CALL SLS_initialize( DQP_control%SBLS_control%definite_linear_solver,&
                               sls_data, sls_control, sls_inform )
          CALL SLS_analyse( prob%H, sls_data, sls_control, sls_inform )
          IF ( sls_inform%status < 0 ) THEN
            WRITE( 6, '( A, I0 )' )                                            &
...
!  Set all default values, and override defaults if requested
      ...
      control%print_level = DQP_control%print_level
      control%out         = DQP_control%out
      control%out_error   = DQP_control%error
...
!  If required, write a SIF file containing the presolved problem
        ...
        IF ( write_presolved_sif ) THEN
          CALL QPT_write_to_sif( prob, pname, pfilename, pfiledevice,          &
                                 .FALSE., .FALSE., DQP_control%infinity )
        END IF
      END IF
