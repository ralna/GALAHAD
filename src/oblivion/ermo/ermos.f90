   PROGRAM GALAHAD_ERMO_EXAMPLE  !  GALAHAD 4.1 - 2022-12-29 AT 08:30 GMT
   USE GALAHAD_ERMO_double                      ! double precision version
   IMPLICIT NONE
...
! problem data complete
   ...
   CALL ERMO_initialize( data, control, inform ) ! Initialize control parameters
   control%TRU_control%subproblem_direct = .TRUE. ! Use a direct method
!  control%print_level = 1
   inform%status = 1                            ! set for initial entry
   CALL ERMO_solve( nlp, control, inform, data, userdata, eval_F = FUN,        &
                   eval_G = GRAD, eval_H = HESS )  ! Solve problem
   IF ( inform%status == 0 ) THEN               ! Successful return
     WRITE( 6, "( ' ERMO: ', I0, ' iterations -',                              &
    &     ' optimal objective value =',                                        &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     inform%iter, inform%obj, nlp%X
   ELSE                                         ! Error returns
     WRITE( 6, "( ' ERMO_solve exit status = ', I6 ) " ) inform%status
   END IF
   CALL ERMO_terminate( data, control, inform )  ! delete internal workspace
   DEALLOCATE( nlp%X, nlp%G, nlp%H%val, nlp%H%row, nlp%H%col, userdata%real )
   END PROGRAM GALAHAD_ERMO_EXAMPLE
