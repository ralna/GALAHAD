! THIS VERSION: GALAHAD 4.1 - 2022-01-06 AT 08:30 GMT.
...
!-*-*-*-*-*-*-*-*-*-  U S E _ E R M O   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_ERMO( input )
...
!-------------------------------
     ...
     TYPE ( ERMO_inform_type ) :: inform
     TYPE ( ERMO_data_type ) :: data
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'ERMO.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'ERMORES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'ERMOSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'ERMOSAVE.d'
...
!  Set up data for next problem
     ...
     CALL ERMO_initialize( data, control, inform )
     IF ( is_specfile ) CALL ERMO_read_specfile( control, input_specfile )
...
!  Solve the problem
     ...
     inform%status = 1
     CALL ERMO_solve( nlp, control, inform, data, userdata,                    &
      ...
      IF ( write_result_summary ) THEN
        ...
        IF ( inform%status == 0 ) THEN
          WRITE( rfiledevice, 2040 ) nlp%pname, nlp%n, inform%obj,             &
            ...
            inform%h_prod, inform%time%total, inform%status
        ELSE
          WRITE( rfiledevice, 2040 ) nlp%pname, nlp%n, inform%obj,             &
            inform%norm_g, - inform%iter, - inform%f_eval, - inform%g_eval,    &
            - inform%h_prod, - inform%time%total, inform%status
...
!  If required, write the solution
     ...
     &  '   its     #f     #g     #h_pr     time stat' )" )
      IF ( inform%status == 0 ) THEN
        WRITE( errout, 2040 ) nlp%pname, nlp%n, inform%obj, inform%norm_g,     &
          inform%iter, inform%f_eval, inform%g_eval, inform%h_prod,            &
...
!  Non-executable statements
 ...
 2040 FORMAT( A10, I6, ES16.8, ES9.1, bn, 3I7, I10, F9.2, I5 )

!  End of subroutine USE_ERMO
