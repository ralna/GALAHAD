...
#include "galahad_modules.h"
   PROGRAM GALAHAD_TR2_interface_test
...
!    control%print_level = 1
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       st = 'C'
       CALL TR2_import( control, data, status, n,                              &
                        'coordinate', ne, H_row, H_col, H_ptr )
       CALL TR2_solve_with_mat( data, userdata, status, X, G,                  &
                                FUN, GRAD, HESS, PREC )
     ...
     CASE ( 5 ) ! access by products
       ...
       CALL TR2_solve_without_mat( data, userdata, status, X, G,               &
                                   FUN, GRAD, HESSPROD, PREC )
     END SELECT
     CALL TR2_information( data, inform, status )
     IF ( inform%status == 0 ) THEN
       WRITE( 6, "( A1, ':', I6, ' iterations. Optimal objective value = ',    &
     &    F5.2, ' status = ', I0 )" ) st, inform%iter, inform%obj, inform%status
     ELSE
       WRITE( 6, "( A1, ': TR2_solve exit status = ', I0 ) " ) st, inform%status
     END IF
!    WRITE( 6, "( ' X ', 3ES12.5 )" ) X
!    WRITE( 6, "( ' G ', 3ES12.5 )" ) G
     CALL TR2_terminate( data, control, inform )  ! delete internal workspace
   END DO
...
!    control%print_level = 1
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       ...
       DO ! reverse-communication loop
         CALL TR2_solve_reverse_with_mat( data, status, eval_status,           &
                                          X, f, G, H_val, U, V )
         SELECT CASE ( status )
     ...
     CASE ( 5 ) ! access by products
       ...
       CALL TR2_import( control, data, status, n,                              &
                        'absent', ne, H_row, H_col, H_ptr )
       DO ! reverse-communication loop
         CALL TR2_solve_reverse_without_mat( data, status, eval_status,        &
                                             X, f, G, U, V )
         SELECT CASE ( status )
         CASE ( 0 ) ! successful termination
           EXIT
...
CONTAINS
   ...
   END PROGRAM GALAHAD_TR2_interface_test
