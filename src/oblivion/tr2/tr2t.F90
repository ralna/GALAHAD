...
!    IF ( s == - GALAHAD_error_cpu_limit ) CYCLE
     ...
     CALL TR2_initialize( data, control,inform ) ! Initialize control parameters
...
!      write(6,*) 'out ', inform%status
     ...
     ELSE
       WRITE( 6, "( I2, ': TR2_solve exit status = ', I6 ) " ) s, inform%status
     END IF
...
!control%gltr_control%print_level = 1
     ...
     &     F6.1, ' status = ', I6 )" ) i, inform%iter, inform%obj, inform%status
     ELSE
       WRITE( 6, "( I2, ': TR2_solve exit status = ', I6 ) " ) i, inform%status
     END IF
...
!    control%print_level = 1
     ...
     ELSE IF ( i == 3 ) THEN
       control%hessian_available = .FALSE.       ! Hessian products will be used
       control%norm = - 3               ! User's preconditioner
       CALL TR2_solve( nlp, control, inform, data, userdata, eval_F = FUN,     &
   ...
   END PROGRAM GALAHAD_TR2_test_deck
