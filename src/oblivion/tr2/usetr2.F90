...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ T R 2  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  July 1st 2023

   MODULE GALAHAD_USETR2_precision

!  This is the driver program for running TR2 for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ T R 2   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_TR2( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNTR2'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNTR2.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'TR2.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'TR2RES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'TR2SOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'TR2SAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'TR2   '

!  ------------------ Open the specfile for tr2 ----------------
...
!  Set up control parameters prior to the next solution
     ...
     CALL TR2_initialize( data, control, inform )
     IF ( is_specfile ) CALL TR2_read_specfile( control, input_specfile )
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from usetr2: re-entry requested with',   &
       ...
       IF ( i /= nlp%n ) THEN
         WRITE( out, "( /, ' *** Exit from usetr2: number of variables'      , &
        &        ' changed from ', I0,' to ', I0,' on re-entry ' )" ) nlp%n, i
         STOP
       END IF
       READ( wfiledevice, "( I8 )" )i
       IF ( i /= nlp%m ) THEN
         WRITE( out, "( /, ' *** Exit from usetr2: number of residuals',       &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from usetr2: variable named ',         &
...
!    CALL CPU_TIME( timeo ) ; CALL CLOCK_time( clocko )
     CALL TR2_solve( nlp, control, inform, data, userdata,                     &
...
!  End of subroutine USE_TR2

     END SUBROUTINE USE_TR2

!  End of module USETR2

   END MODULE GALAHAD_USETR2_precision
