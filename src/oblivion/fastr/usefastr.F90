! THIS VERSION: GALAHAD 4.1 - 2022-12-29 AT 11:40 GMT.

#include "galahad_modules.h"

!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ F A S T R  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  Started: May 25th 2005

   MODULE GALAHAD_USEFASTR_precision

!  This is the driver program for running FASTR for a variety of computing
!  systems. It opens and closes all the files, allocate arrays, reads and
!  checks data, and calls the appropriate minimizers

     USE GALAHAD_PRECISION
     USE GALAHAD_FASTR_precision
...
!-*-*-*-*-*-*-*-*-*-  U S E _ F A S T R   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_FASTR( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNFASTR'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNFASTR.SPC'
...
!    LOGICAL :: write_result_summary  = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'FASTR.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'FASTRRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'FASTRSOL.d'
     CHARACTER ( LEN = 30 ) :: vfilename = 'FASTRSOLVEC.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'FASTRSAVE.d'
...
!  Set copyright
     ...
     IF ( out > 0 ) CALL COPYRIGHT( out, '2005' )

!  Set up control parameters prior to the next solution

     CALL FASTR_initialize( data, control, inform )
     IF ( is_specfile ) CALL FASTR_read_specfile( control, input_specfile )
...
!    DEALLOCATE( nlp%LINEAR, STAT = inform%alloc_status )
!    IF ( inform%alloc_status /= 0 ) THEN
!      inform%status = 2 ; inform%bad_alloc = 'LINEAR' ; GO TO 910 ; END IF

     STOP

!910 CONTINUE
     IF ( inform%status == - 1 ) THEN
       WRITE( control%error, "( ' ** Message from -FASTR-',                    &
      &          '    Allocation error (status = ', I0, ') for ', A24, / )" )  &
       inform%status, inform%bad_alloc
     ELSE
       WRITE( control%error, "( ' ** Message from -FASTR-',                    &
      &          '    Deallocation error (status = ', I0, ') for ', A24, / )" )&
       inform%status, inform%bad_alloc
     END IF
...
!  End of subroutine USE_FASTR

     END SUBROUTINE USE_FASTR

!  End of module USEFASTR

   END MODULE GALAHAD_USEFASTR_precision
