...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ A G D  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  May 15th 2008

   MODULE GALAHAD_USEAGD_precision

!  This is the driver program for running AGD for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ A G D   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_AGD( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNAGD'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNAGD.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'AGD.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'AGDRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'AGDSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'AGDSAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'AGD   '

!  ------------------ Open the specfile for agd ----------------
...
!  Set up control parameters prior to the next solution
     ...
     IF ( is_specfile ) CALL AGD_read_specfile( control, input_specfile )
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from useagd: re-entry requested with',   &
       ...
       IF ( i /= nlp%n ) THEN
         WRITE( out, "( /, ' *** Exit from useagd: number of variables'      , &
        &        ' changed from ', I0,' to ', I0,' on re-entry ' )" ) nlp%n, i
         STOP
       END IF
       READ( wfiledevice, "( I8 )" )i
       IF ( i /= nlp%m ) THEN
         WRITE( out, "( /, ' *** Exit from useagd: number of residuals',       &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from useagd: variable named ',         &
...
!  End of subroutine USE_AGD

     END SUBROUTINE USE_AGD

!  End of module USEAGD

   END MODULE GALAHAD_USEAGD_precision
