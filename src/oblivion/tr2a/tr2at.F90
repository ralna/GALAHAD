...
#include "galahad_modules.h"
   PROGRAM GALAHAD_TR2A_test_deck
...
!      write(6,*) 'out ', inform%status
     ...
     ELSE
       WRITE( 6, "( I2, ': TR2A_solve exit status = ', I6 ) " ) s, inform%status
     END IF
...
!control%gltr_control%print_level = 1
     ...
     &     F6.1, ' status = ', I6 )" ) i, inform%iter, inform%obj, inform%status
     ELSE
       WRITE( 6, "( I2, ': TR2A_solve exit status = ', I6 ) " ) i, inform%status
     END IF
...
!    control%print_level = 1
   ...
   END PROGRAM GALAHAD_TR2A_test_deck
