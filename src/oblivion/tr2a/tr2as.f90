   PROGRAM GALAHAD_TR2A_EXAMPLE  !  GALAHAD 4.2 - 2023-07-02 AT 12:00 GMT
   USE GALAHAD_TR2A_double                       ! double precision version
   IMPLICIT NONE
...
! problem data complete
   ...
   CALL TR2A_initialize( data, control, inform ) ! Initialize control parameters
   control%subproblem_direct = .TRUE.           ! Use a direct method
!  control%print_level = 1
   inform%status = 1                            ! set for initial entry
   CALL TR2A_solve( nlp, control, inform, data, userdata, eval_F = FUN,        &
                   eval_G = GRAD, eval_H = HESS )  ! Solve problem
   IF ( inform%status == 0 ) THEN               ! Successful return
     WRITE( 6, "( ' TR2A: ', I0, ' iterations -',                              &
    &     ' optimal objective value =',                                        &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     inform%iter, inform%obj, nlp%X
   ELSE                                         ! Error returns
     WRITE( 6, "( ' TR2A_solve exit status = ', I6 ) " ) inform%status
   END IF
   CALL TR2A_terminate( data, control, inform )  ! delete internal workspace
   DEALLOCATE( nlp%X, nlp%G, nlp%H%val, nlp%H%row, nlp%H%col, userdata%real )
   END PROGRAM GALAHAD_TR2A_EXAMPLE
