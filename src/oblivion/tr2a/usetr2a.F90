...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ T R 2 A  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  July 1st 2023

   MODULE GALAHAD_USETR2A_precision

!  This is the driver program for running TR2A for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ T R 2 A   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_TR2A( input )
...
!-------------------------------
     ...
     TYPE ( TR2A_inform_type ) :: inform
     TYPE ( TR2A_data_type ) :: data
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNTR2A'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNTR2A.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'TR2A.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'TR2ARES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'TR2ASOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'TR2ASAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'TR2A   '

!  ------------------ Open the specfile for tr2a ----------------
...
!  Set up control parameters prior to the next solution
     ...
     CALL TR2A_initialize( data, control, inform )
     IF ( is_specfile ) CALL TR2A_read_specfile( control, input_specfile )
...
!  End of subroutine USE_TR2A

     END SUBROUTINE USE_TR2A

!  End of module USETR2A

   END MODULE GALAHAD_USETR2A_precision
