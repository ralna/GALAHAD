...
!  August 12th 2002
      ...
      PROGRAM RUNLPQPA_SIF_precision
...
!  Scalars
      ...
      INTEGER ( KIND = ip_ ) :: iores, ndegen, mequal, A_ne, H_ne
...
!     LOGICAL :: write_problem_data   = .FALSE.
      ...
      CHARACTER ( LEN = 30 ) :: dfilename = 'LPQPA.data'
      CHARACTER ( LEN = 30 ) :: ifilename = 'ORIG.SIF'
      CHARACTER ( LEN = 30 ) :: pfilename = 'PRE.SIF'
      CHARACTER ( LEN = 30 ) :: rfilename = 'LPQPARES.d'
      CHARACTER ( LEN = 30 ) :: sfilename = 'LPQPASOL.d'
...
!  Arrays
      ...
      TYPE ( LPQPA_control_type ) :: control
      TYPE ( LPQPA_inform_type ) :: inform
      TYPE ( LPQPA_data_type ) :: data
...
!   Define the keywords
      ...
      spec( 8 )%keyword = 'initial-rho-g'
      spec( 9 )%keyword = 'initial-rho-b'
      spec( 10 )%keyword = 'scale-problem'
      spec( 11 )%keyword = 'pre-solve-problem'
      spec( 12 )%keyword = 'write-presolved-sif'
      spec( 13 )%keyword = 'presolved-sif-file-name'
      spec( 14 )%keyword = 'presolved-sif-file-device'
      spec( 15 )%keyword = 'solve-problem'
      spec( 16 )%keyword = 'print-full-solution'
      spec( 17 )%keyword = 'write-solution'
      spec( 18 )%keyword = 'solution-file-name'
      spec( 19 )%keyword = 'solution-file-device'
      spec( 20 )%keyword = 'write-result-summary'
      spec( 21 )%keyword = 'result-summary-file-name'
      spec( 22 )%keyword = 'result-summary-file-device'
      spec( 23 )%keyword = 'one-norm-penalty'
...
!   Interpret the result
      ...
      CALL SPECFILE_assign_real( spec( 8 ), prob%rho_g, errout )
      CALL SPECFILE_assign_real( spec( 9 ), prob%rho_b, errout )
      CALL SPECFILE_assign_integer( spec( 10 ), scale, errout )
      CALL SPECFILE_assign_logical( spec( 11 ), do_presolve, errout )
      CALL SPECFILE_assign_logical( spec( 12 ), write_presolved_sif, errout )
      CALL SPECFILE_assign_string ( spec( 13 ), pfilename, errout )
      CALL SPECFILE_assign_integer( spec( 14 ), pfiledevice, errout )
      CALL SPECFILE_assign_logical( spec( 15 ), do_solve, errout )
      CALL SPECFILE_assign_logical( spec( 16 ), fulsol, errout )
      CALL SPECFILE_assign_logical( spec( 17 ), write_solution, errout )
      CALL SPECFILE_assign_string ( spec( 18 ), sfilename, errout )
      CALL SPECFILE_assign_integer( spec( 19 ), sfiledevice, errout )
      CALL SPECFILE_assign_logical( spec( 20 ), write_result_summary, errout )
      CALL SPECFILE_assign_string ( spec( 21 ), rfilename, errout )
      CALL SPECFILE_assign_integer( spec( 22 ), rfiledevice, errout )
      CALL SPECFILE_assign_logical( spec( 23 ), one_norm, errout )
...
!  Update control parameters if required.
      ...
      CALL LPQPA_initialize( data, control, inform )
      CALL LPQPA_read_specfile( control, input_specfile )

      printo = out > 0 .AND. control%QPA_control%print_level > 0
      printe = out > 0 .AND. control%QPA_control%print_level >= 0
...
!  =================
      ...
      control%QPA_control%restore_problem = 2
...
!       WRITE( 6, "( ' z ', /, (5ES12.4) )" ) prob%Z
        ...
        solv = 'LPQPA'
        IF ( printo ) WRITE( out, " ( /, ' ** QPA solver used ** ' ) " )
        CALL LPQPA_solve( prob, rho, one_norm, C_stat, B_stat, data,           &
                          control, inform )

        IF ( printo ) WRITE( out, " ( /, ' ** LPQPA solver used ** ' ) " )
        qfval = inform%QPA_inform%obj
        nmods = inform%QPA_inform%nmods ; newton = 0

        CALL CPU_TIME( timet )

!  Deallocate arrays from the minimization

        status = inform%QPA_inform%status ; iter = inform%QPA_inform%iter
        nfacts = inform%QPA_inform%nfacts
        factorization_integer = inform%QPA_inform%factorization_integer
        factorization_real = inform%QPA_inform%factorization_real
      ELSE
        timeo  = 0.0
        timet  = 0.0
        iter   = 0
...
!  Print details of the solution obtained
      ...
      IF ( status == 0 .OR. status == - 8 .OR. status == - 9 .OR.              &
        ...
        IF ( fulsol ) l = prob%n
        IF ( do_presolve ) THEN
          IF ( control%QPA_control%print_level >= DEBUG ) l = prob%n
        END IF
...
!  Print details of the constraints.
        ...
        IF ( prob%m > 0 ) THEN
          ...
          IF ( do_presolve ) THEN
            IF ( control%QPA_control%print_level >= DEBUG ) l = prob%m
          END IF
          DO j = 1, 2
            IF ( j == 1 ) THEN
...
!     DEALLOCATE( RES )
      ...
      CALL LPQPA_terminate( data, control, inform )
...
!  End of program RUNLPQPA_SIF_precision

      END PROGRAM RUNLPQPA_SIF_precision
