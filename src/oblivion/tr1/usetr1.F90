...
!-*-*-*-*-*-*-*-*-*-  U S E _ T R 1   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_TR1( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNTR1'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNTR1.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'TR1.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'TR1RES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'TR1SOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'TR1SAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'TR1   '

!  ------------------ Open the specfile for tr1 ----------------
...
!  Set up control parameters prior to the next solution
     ...
     IF ( is_specfile ) CALL TR1_read_specfile( control, input_specfile )
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from usetr1: re-entry requested with',   &
       ...
       IF ( i /= nlp%n ) THEN
         WRITE( out, "( /, ' *** Exit from usetr1: number of variables'      , &
        &        ' changed from ', I0,' to ', I0,' on re-entry ' )" ) nlp%n, i
         STOP
       END IF
       READ( wfiledevice, "( I8 )" )i
       IF ( i /= nlp%m ) THEN
         WRITE( out, "( /, ' *** Exit from usetr1: number of residuals',       &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from usetr1: variable named ',         &
...
!  End of subroutine USE_TR1
     ...
     END SUBROUTINE USE_TR1

!  End of module USETR1

   END MODULE GALAHAD_USETR1_precision
