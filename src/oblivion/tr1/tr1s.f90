   PROGRAM GALAHAD_SDA_EXAMPLE  !  GALAHAD 4.1 - 2023-01-29 AT 14:45 GMT
   USE GALAHAD_TR1_double                       ! double precision version
   IMPLICIT NONE
...
!  control%print_level = 1
   ...
   CALL TR1_solve( nlp, control, inform, data, userdata, eval_F = FUN,         &
                   eval_G = GRAD, eval_H = HESS )  ! Solve problem
   IF ( inform%status == 0 ) THEN               ! Successful return
     WRITE( 6, "( ' TR1: ', I0, ' iterations -',                               &
    &     ' optimal objective value =',                                        &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     inform%iter, inform%obj, nlp%X
   ELSE                                         ! Error returns
     WRITE( 6, "( ' TR1_solve exit status = ', I6 ) " ) inform%status
   END IF
   ...
   END PROGRAM GALAHAD_TR1_EXAMPLE
