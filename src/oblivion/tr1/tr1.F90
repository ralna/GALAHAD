...
!-*-*-*-*-*-*-*-*-  G A L A H A D _ T R 1   M O D U L E  *-*-*-*-*-*-*-*-*-*-
...
!    USE IEEE_ARITHMETIC
     ...
     PRIVATE
     PUBLIC :: TR1_initialize, TR1_read_specfile, TR1_solve,                   &
               TR1_terminate, NLPT_problem_type, GALAHAD_userdata_type,        &
               TR1_import, TR1_solve_direct, TR1_solve_reverse,                &
               TR1_full_initialize, TR1_full_terminate, TR1_reset_control,     &
               TR1_information
...
!----------------------
     ...
     INTERFACE TR1_initialize
       MODULE PROCEDURE TR1_initialize, TR1_full_initialize
     END INTERFACE TR1_initialize

     INTERFACE TR1_terminate
       MODULE PROCEDURE TR1_terminate, TR1_full_terminate
     END INTERFACE TR1_terminate
...
!  control parameters for SHA
     ...
     END TYPE TR1_control_type

!  - - - - - - - - - - - - - - - - - - - - - -
!   time derived type with component defaults
!  - - - - - - - - - - - - - - - - - - - - - -

     TYPE, PUBLIC :: TR1_time_type
...
!  - - - - - - - - - - - - - - - - - - - - - - -
     ...
     TYPE, PUBLIC :: TR1_inform_type

!  return status. See TR1_solve for details
...
!  timings (see above)
       ...
       TYPE ( TR1_time_type ) :: time

!  inform parameters for SHA

       TYPE ( SHA_inform_type ) :: SHA_inform
     END TYPE TR1_inform_type

!  - - - - - - - - - -
!   data derived types
!  - - - - - - - - - -

     TYPE, PUBLIC :: TR1_data_type
...
!  data for SHA
     ...
     END TYPE TR1_data_type

     TYPE, PUBLIC :: TR1_full_data_type
       LOGICAL :: f_indexing = .TRUE.
       TYPE ( TR1_data_type ) :: tr1_data
       TYPE ( TR1_control_type ) :: tr1_control
       TYPE ( TR1_inform_type ) :: tr1_inform
       TYPE ( NLPT_problem_type ) :: nlp
       TYPE ( GALAHAD_userdata_type ) :: userdata
     END TYPE TR1_full_data_type

   CONTAINS

!-*-*-  G A L A H A D -  T R 1 _ I N I T I A L I Z E  S U B R O U T I N E  -*-

     SUBROUTINE TR1_initialize( data, control, inform )
...
!-----------------------------------------------
     ...
     TYPE ( TR1_data_type ), INTENT( INOUT ) :: data
     TYPE ( TR1_control_type ), INTENT( OUT ) :: control
     TYPE ( TR1_inform_type ), INTENT( OUT ) :: inform
...
!  End of subroutine TR1_initialize

     END SUBROUTINE TR1_initialize

!- G A L A H A D -  T R 1 _ F U L L _ I N I T I A L I Z E  S U B R O U T I N E -

     SUBROUTINE TR1_full_initialize( data, control, inform )
...
!-----------------------------------------------
     ...
     TYPE ( TR1_full_data_type ), INTENT( INOUT ) :: data
     TYPE ( TR1_control_type ), INTENT( OUT ) :: control
     TYPE ( TR1_inform_type ), INTENT( OUT ) :: inform

     CALL TR1_initialize( data%tr1_data, data%tr1_control, data%tr1_inform )
     control = data%tr1_control
     inform = data%tr1_inform

     RETURN

!  End of subroutine TR1_full_initialize

     END SUBROUTINE TR1_full_initialize

!-*-*-*-*-   T R 1 _ R E A D _ S P E C F I L E  S U B R O U T I N E  -*-*-*-*-

     SUBROUTINE TR1_read_specfile( control, device, alt_specname )

!  Reads the content of a specification file, and performs the assignment of
!  values associated with given keywords to the corresponding control parameters

!  The default values as given by TR1_initialize could (roughly)
!  have been set as:
...
!-----------------------------------------------
     ...
     TYPE ( TR1_control_type ), INTENT( INOUT ) :: control
...
!  read the controls for the sub-problem solvers and preconditioner
     ...
     END SUBROUTINE TR1_read_specfile

!-*-*-*-  G A L A H A D -  T R 1 _ s o l v e  S U B R O U T I N E  -*-*-*-

     SUBROUTINE TR1_solve( nlp, control, inform, data, userdata,               &
...
!  *-*-*-*-*-*-*-*-*-*-*-*-  A R G U M E N T S  -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
!
!  For full details see the specification sheet for GALAHAD_TR1.
!
!  ** NB. default real/complex means double precision real/complex in
!  ** GALAHAD_TR1_precision
!
...
!   not allocated.
!
! control is a scalar variable of type TR1_control_type. See TR1_initialize
!  for details
!
! inform is a scalar variable of type TR1_inform_type. On initial entry,
...
!
!  time is a scalar variable of type TR1_time_type whose components are used to
...
!
!  data is a scalar variable of type TR1_data_type used for internal data.
!
...
!   be set to a nonzero value. If eval_F is not present, TR1_solve will
...
!   TR1_solve will return to the user with inform%status = 3 each time an
...
!   value. If eval_H is not present, TR1_solve will return to the user with
...
!-----------------------------------------------
     ...
     TYPE ( TR1_control_type ), INTENT( IN ) :: control
     TYPE ( TR1_inform_type ), INTENT( INOUT ) :: inform
     TYPE ( TR1_data_type ), INTENT( INOUT ) :: data
...
!  -------------
 ...
 990 CONTINUE
     ...
     IF ( control%error > 0 ) THEN
       CALL SYMBOLS_status( inform%status, control%error, prefix, 'TR1_solve' )
       WRITE( control%error, "( ' ' )" )
     END IF
     RETURN
...
!  Non-executable statements
 ...
 !  End of subroutine TR1_solve
     ...
     END SUBROUTINE TR1_solve

!-*-*-  G A L A H A D -  T R 1 _ t e r m i n a t e  S U B R O U T I N E -*-*-

     SUBROUTINE TR1_terminate( data, control, inform )
...
!  Deallocate all remaining allocated arrays
     ...
     array_name = 'tr1: data%RHO'
     CALL SPACE_dealloc_array( data%RHO,                                       &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%ALPHA'
     ...
     array_name = 'tr1: data%DX'
     CALL SPACE_dealloc_array( data%DX,                                        &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%DG'
...
!  End of subroutine TR1_terminate

     END SUBROUTINE TR1_terminate

!-  G A L A H A D -  T R 1 _ f u l l _ t e r m i n a t e  S U B R O U T I N E -

     SUBROUTINE TR1_full_terminate( data, control, inform )
...
!  deallocate workspace
     ...
     CALL TR1_terminate( data%tr1_data, data%tr1_control, data%tr1_inform )
     inform = data%tr1_inform

!  deallocate any internal problem arrays

     array_name = 'tr1: data%nlp%X'
     CALL SPACE_dealloc_array( data%nlp%X,                                     &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%G'
     CALL SPACE_dealloc_array( data%nlp%G,                                     &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%Z'
     CALL SPACE_dealloc_array( data%nlp%Z,                                     &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%H%row'
     CALL SPACE_dealloc_array( data%nlp%H%row,                                 &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%H%col'
     CALL SPACE_dealloc_array( data%nlp%H%col,                                 &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%H%ptr'
     CALL SPACE_dealloc_array( data%nlp%H%ptr,                                 &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%H%val'
     CALL SPACE_dealloc_array( data%nlp%H%val,                                 &
        inform%status, inform%alloc_status, array_name = array_name,           &
        bad_alloc = inform%bad_alloc, out = control%error )
     IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

     array_name = 'tr1: data%nlp%H%type'
...
!  End of subroutine TR1_full_terminate

     END SUBROUTINE TR1_full_terminate
...
!   comments to TR1_solve
!
!  data is a scalar variable of type TR1_full_data_type used for internal data
!
...
!  copy control to data
     ...
     data%tr1_control = control

     error = data%tr1_control%error
     space_critical = data%tr1_control%space_critical
     deallocate_error_fatal = data%tr1_control%deallocate_error_fatal

!  allocate space if required

     array_name = 'tr1: data%nlp%X'
     CALL SPACE_resize_array( n, data%nlp%X,                                   &
            data%tr1_inform%status, data%tr1_inform%alloc_status,              &
            array_name = array_name,                                           &
            deallocate_error_fatal = deallocate_error_fatal,                   &
            exact_size = space_critical,                                       &
            bad_alloc = data%tr1_inform%bad_alloc, out = error )
     IF ( data%tr1_inform%status /= 0 ) GO TO 900
...
!  error returns
 ...
 900 CONTINUE
     status =  data%tr1_inform%status
     RETURN

!  End of subroutine TR1_import

     END SUBROUTINE TR1_import

!-  G A L A H A D -  T R 1 _ r e s e t _ c o n t r o l   S U B R O U T I N E  -

     SUBROUTINE TR1_reset_control( control, data, status )

!  reset control parameters after import if required.
!  See TR1_solve for a description of the required arguments
...
!  end of subroutine TR1_reset_control

     END SUBROUTINE TR1_reset_control

! - G A L A H A D -  T R 1 _ s o l v e _ d i r e c t   S U B R O U T I N E -

     SUBROUTINE TR1_solve_direct( data, userdata, status, X, G,           &
                                  eval_F, eval_G )
...
!  are available via subroutine calls. See TR1_solve for a description
...
!-----------------------------------------------
     ...
     data%tr1_inform%status = status
     IF ( data%tr1_inform%status == 1 )                                        &
       data%nlp%X( : data%nlp%n ) = X( : data%nlp%n )

!  call the solver

     CALL TR1_solve( data%nlp, data%tr1_control, data%tr1_inform,              &
                     data%tr1_data, userdata, eval_F = eval_F,                 &
                     eval_G = eval_G )

     X( : data%nlp%n ) = data%nlp%X( : data%nlp%n )
     IF ( data%tr1_inform%status == GALAHAD_ok )                               &
       G( : data%nlp%n ) = data%nlp%G( : data%nlp%n )
     status = data%tr1_inform%status

     RETURN

!  end of subroutine TR1_solve_direct

     END SUBROUTINE TR1_solve_direct
...
!  available via reverse communication. See TR1_solve for a description
!  of the required arguments. The variable status is a proxy for inform%status
...
!  end of subroutine TR1_solve_reverse

     END SUBROUTINE TR1_solve_reverse

!-  G A L A H A D -  T R 1 _ i n f o r m a t i o n   S U B R O U T I N E  -

     SUBROUTINE TR1_information( data, inform, status )
...
!  end of subroutine TR1_information

     END SUBROUTINE TR1_information

!  End of module GALAHAD_TR1

   END MODULE GALAHAD_TR1_precision
