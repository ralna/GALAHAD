! THIS VERSION: GALAHAD 4.1 - 2022-12-19 AT 15:30 GMT.
...
!----------------------
     ...
     REAL ( KIND = rp_ ), PARAMETER :: minus_infinity = - infinity / two 
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ), PARAMETER :: beta_filter = initial_filter_size + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: gamma_filter = beta_filter + 1
     INTEGER ( KIND = ip_ ), PARAMETER :: space_critical = gamma_filter + 1
...
!-----------------------------------------------
     ...
     data%n_filter = 0 ; inform%n_filter = data%n_filter
...
!-*-  G A L A H A D -  F I L T E R _ a c c e p t a b l e  F U N C T I O N  -*-

     FUNCTION FILTER_acceptable( o_new, v_new, data, control, o, v )
...
!      v_new < control%beta_filter v_i 
!   or
!      o_new < o_i - control%gamma_filter control%beta_filter v_i
!   where 0 < control%gamma_filter < control%beta_filter < 1

!  *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-

     LOGICAL :: FILTER_acceptable
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ) :: i
     REAL ( KIND = rp_ ) :: v_hash

!    WRITE(6,"( ' o_new,o ', 2ES22.14, /, ' v_new,v ', 2ES22.14 )" )           &
!      o_new,  o - control%gamma_filter * v,                                   &
!      v_new, control%beta_filter * v

!     IF ( v_new > v_zero ) THEN
        v_hash = v_new

!  check acceptability for current iterate

        IF ( PRESENT( o ) .AND. PRESENT( v ) ) THEN
!         WRITE(6,"( ' o_new,f ', 2ES22.14, /, ' v_new,c ', 2ES22.14 )" )      &
!           o_new, o - control%gamma_filter * v,                               &
!           v_hash, control%beta_filter * v
          IF ( o_new > o - control%gamma_filter * v .AND.                      &
               v_hash >= control%beta_filter * v ) THEN
...
!          WRITE(6,"( ' o_new,fi', 2ES22.14, /, ' v_new,ci', 2ES22.14 )" )     &
!            o_new, filter( i )%o - control%gamma_filter * data%filter( i )%v, &
!            v_hash, control%beta_filter * data%filter( i )%v
          IF ( o_new > data%filter( i )%o                                      &
                 - control%gamma_filter * data%filter( i )%v                   &
               .AND. v_hash >= control%beta_filter * data%filter( i )%v ) THEN
!           write(6,*) ' false'
            FILTER_acceptable = .FALSE.
            RETURN
          END IF
...
!  End of function FILTER_acceptable
...
!  add the new filter entry
     ...
     data%n_filter = data%n_filter + 1 ; inform%n_filter = data%n_filter
