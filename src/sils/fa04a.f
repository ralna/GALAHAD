...
C       Reference to FA04F removed.
C       DATA statements changed to PARAMETER.
C       FA04A is a renamed version of FA04AS (no other changes)
C  EAT 21/6/93 EXTERNAL statement put in for block data on VAXs.
C.
C
      REAL FUNCTION FA04A(I)
      INTEGER A,B15,B16,P
      ...
      IF (I.GE.0) FA04A = X*4.6566128752457969241D-10
      IF (I.LT.0) FA04A = X*9.3132257504915938482D-10 - 1.0D0
      RETURN
      END
      SUBROUTINE FA04B(MAX,NRAND)
      INTEGER A,B15,B16,P
      PARAMETER (A=16807,B15=32768,B16=65536,P=2147483647)
      INTEGER B30,Q
