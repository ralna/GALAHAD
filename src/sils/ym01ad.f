...
*######DATE 4 Oct 1992 modified.
C       Calls to FA04AS/BS -> FA04AD/BD.
C
      SUBROUTINE YM01AD(M,N,NZIN,NZOUT,IBW,ITYPE,NONSIN,IRN,A,JCOLST,IW)
     ...
     +                              (IBW-IRECT)* (IBW-IRECT+1)/2
      ...
      IF (NONSIN .AND. .NOT.BAND .AND. M.LT.N) CALL YM01BD(M,N,JCOLST,
     +    IW)
      DO 10 I = 1,N
        IW(I) = 0
   10 CONTINUE
   ...
   60   CONTINUE
      END IF
      IF (NONSIN .AND. UNSYM .AND. .NOT.BAND) CALL YM01BD(MINMN,M,
     +    JCOLST,IW(N+1))
      DO 70 K = 1,M
        IW(N+K) = 0
   70 CONTINUE
  ...
  170 RETURN
      END
      SUBROUTINE YM01BD(N,M,PERM,INVPRM)
      INTEGER M,N
