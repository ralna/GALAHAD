! THIS VERSION: GALAHAD 4.1 - 2022-12-20 AT 13:55 GMT.
...
!      control%print_level = 1
       p%rho_g = 0.1_rp_ ; p%rho_b = 0.1_rp_
       control%precon = 1
...
!  special test for status = - 7
   ...
   p%f = 0.0_rp_ ; p%rho_g = 10.0_rp_ ; p%rho_b = 10.0_rp_
   p%G = (/ 0.0_rp_ /)
   p%X_l = (/ 0.0_rp_ /)
   p%X_u = (/ infty /)
   p%H%val = (/ - 1.0_rp_ /)
...
!  control%print_level = 1
   p%X = 1.0_rp_ ; p%Y = 0.0_rp_ ; p%Z = 0.0_rp_
   CALL QPA_solve( p, C_stat, B_stat, data, control, info )
   IF ( info%status == 0 ) THEN
       WRITE( 6, "( I2, ':', I6, ' iterations. Optimal objective value = ',    &
     &      F6.1, ' status = ', I6 )" ) status, info%iter, info%obj, info%status
...
!  ============================
   ...
   CALL SMT_put( p%A%type, 'COORDINATE', smt_stat )
   p%n = n ; p%m = m ; p%H%ne = h_ne ; p%A%ne = a_ne 
   p%f = 1.0_rp_ ; p%rho_g = 10.0_rp_ ; p%rho_b = 10.0_rp_
