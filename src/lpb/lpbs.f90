...
! problem data complete
   CALL LPB_initialize( data, control, inform ) ! Initialize control parameters
   control%infinity = infinity                ! Set infinity
! Solve the problem
   CALL LPB_solve( p, data, control, inform, C_stat = C_stat, X_stat = X_stat )
   IF ( inform%status == 0 ) THEN               !  Successful return
     WRITE( 6, "( ' LPB: ', I0, ' iterations. Optimal objective value =',      &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     inform%iter, inform%obj, p%X
   ELSE                                       !  Error returns
     WRITE( 6, "( ' LPB_solve exit status = ', I6 ) " ) inform%status
   END IF
