...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_LPB_precision_ciface
    USE GALAHAD_KINDS
    USE GALAHAD_common_ciface
    USE GALAHAD_LPB_precision, ONLY:                                           &
        f_lpb_control_type   => LPB_control_type,                              &
        f_lpb_time_type      => LPB_time_type,                                 &
        f_lpb_inform_type    => LPB_inform_type,                               &
        f_lpb_full_data_type => LPB_full_data_type,                            &
        f_lpb_initialize     => LPB_initialize,                                &
        f_lpb_read_specfile  => LPB_read_specfile,                             &
        f_lpb_import         => LPB_import,                                    &
        f_lpb_reset_control  => LPB_reset_control,                             &
        f_lpb_solve_lp       => LPB_solve_lp,                                  &
...
!-------------------------------------------------
    ...
    END TYPE lpb_control_type

    TYPE, BIND( C ) :: lpb_time_type
      REAL ( KIND = rp_ ) :: total
    ...
    END TYPE lpb_time_type

    TYPE, BIND( C ) :: lpb_inform_type
...
!  copy fortran inform parameters to C
  ...
  END MODULE GALAHAD_LPB_precision_ciface
...
!  -------------------------------------
  ...
  USE GALAHAD_LPB_precision_ciface
  IMPLICIT NONE
...
!  initialize required fortran types
  ...
  CALL f_lpb_initialize( fdata, fcontrol, finform )
...
!  ----------------------------------------
!  C interface to fortran lpb_read_specfile
!  ----------------------------------------

  SUBROUTINE lpb_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_LPB_precision_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( lpb_control_type ), INTENT( INOUT ) :: ccontrol
...
!  ---------------------------------
!  C interface to fortran lpb_inport
!  ---------------------------------

  SUBROUTINE lpb_import( ccontrol, cdata, status, n, m,                        &
...
!  import the problem data into the required LPB structure

  CALL f_lpb_import( fcontrol, fdata, status, n, m,                            &
...
!  ----------------------------------------
!  C interface to fortran lpb_reset_control
!  ----------------------------------------

  SUBROUTINE lpb_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_LPB_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_lpb_reset_control( fcontrol, fdata, status )
  RETURN

  END SUBROUTINE lpb_reset_control

!  ------------------------------------
!  C interface to fortran lpb_solve_lpb
!  ------------------------------------

  SUBROUTINE lpb_solve_lp( cdata, status, n, m, g, f, ane, aval,               &
...
!  solve the qp
  ...
  CALL f_lpb_solve_lp( fdata, status, g, f, aval, cl, cu, xl, xu, x, c, y, z,  &
                       xstat, cstat )
  RETURN
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( lpb_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain LPB solution information

  CALL f_lpb_information( fdata, finform, status )
...
!  deallocate workspace
  ...
  CALL f_lpb_terminate( fdata, fcontrol, finform )
