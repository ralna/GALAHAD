...
static struct lpb_control_type control;  // control struct
static struct lpb_inform_type inform;    // inform struct
...
PyObject* lpb_make_options_dict(const struct lpb_control_type *control){
...
static PyObject* lpb_make_time_dict(const struct lpb_time_type *time){
...
static PyObject* lpb_make_inform_dict(const struct lpb_inform_type *inform){
    ...
    PyDict_SetItemString(py_inform, "time",
                         lpb_make_time_dict(&inform->time));
...
static PyObject* py_lpb_initialize(PyObject *self){

    // Call lpb_initialize
    lpb_initialize(&data, &control, &status);

    // Record that LPB has been initialised
    init_called = true;
...
static PyObject* py_lpb_load(PyObject *self, PyObject *args, PyObject *keywds){
    ...
    // Update LPB control options
    if(!lpb_update_control(&control, py_options))
        return NULL;
...
static PyObject* py_lpb_information(PyObject *self){

    // Check that package has been initialised
    if(!check_init(init_called))
        return NULL;

    // Call lpb_information
...
static PyObject* py_lpb_terminate(PyObject *self){
...
/* lpb python module documentation */

PyDoc_STRVAR(lpb_module_doc,

"The lpb package uses a primal-dual interior-point method to solve the \n"
...
"\n"
"See $GALAHAD/html/Python/lpb.html for argument lists, call order\n"
"and other details.\n"
"\n"
);

/* lpb python module definition */
static struct PyModuleDef module = {
   PyModuleDef_HEAD_INIT,
   "lpb",               /* name of module */
   lpb_module_doc,      /* module documentation, may be NULL */
   -1,                  /* size of per-interpreter state of the module,or -1
                           if the module keeps state in global variables */
   lpb_module_methods   /* module methods */
};

/* Python module initialization */
PyMODINIT_FUNC PyInit_lpb(void) { // must be same as module name above
