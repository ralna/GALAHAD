...
!  non-reentrant (one active solve per process).
    ...
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_f = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_g = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_h = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_hprod = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_prec = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: dgo_saved_ceval_shprod = C_NULL_FUNPTR
    INTEGER ( KIND = ipc_ ) :: dgo_saved_n
    TYPE ( C_PTR ) :: dgo_saved_cuserdata
    INTEGER ( KIND = ipc_ ) :: dgo_saved_ne
    LOGICAL ( KIND = C_BOOL ) :: dgo_saved_f_indexing
  CONTAINS
...
!  call C interoperable eval_f
    ...
    PROCEDURE( eval_f ), POINTER :: dgo_saved_eval_f
    CALL C_F_PROCPOINTER( dgo_saved_ceval_f, dgo_saved_eval_f )
    status = dgo_saved_eval_f( dgo_saved_n, x, f, dgo_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_g
    PROCEDURE( eval_g ), POINTER :: dgo_saved_eval_g
    CALL C_F_PROCPOINTER( dgo_saved_ceval_g, dgo_saved_eval_g )
    status = dgo_saved_eval_g( dgo_saved_n, x, g, dgo_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_h
    PROCEDURE( eval_h ), POINTER :: dgo_saved_eval_h
    CALL C_F_PROCPOINTER( dgo_saved_ceval_h, dgo_saved_eval_h )
    status = dgo_saved_eval_h( dgo_saved_n, dgo_saved_ne, x, hval, dgo_saved_cuserdata )
    RETURN
...
! Call C interoperable eval_hprod
    PROCEDURE( eval_hprod ), POINTER :: dgo_saved_eval_hprod
    CALL C_F_PROCPOINTER( dgo_saved_ceval_hprod, dgo_saved_eval_hprod )
    IF ( PRESENT( fgot_h ) ) THEN
      cgot_h = fgot_h
    ELSE
      cgot_h = .FALSE.
    END IF
    status = dgo_saved_eval_hprod( dgo_saved_n, x, u, v, cgot_h, dgo_saved_cuserdata )
    RETURN
...
!  call C interoperable eval_prec
    ...
    PROCEDURE( eval_prec ), POINTER :: dgo_saved_eval_prec
    CALL C_F_PROCPOINTER( dgo_saved_ceval_prec, dgo_saved_eval_prec )
    status = dgo_saved_eval_prec( dgo_saved_n, x, u, v, dgo_saved_cuserdata )
    RETURN
...
!  call C interoperable eval_shprod
    ...
    PROCEDURE( eval_shprod ), POINTER :: dgo_saved_eval_shprod
    CALL C_F_PROCPOINTER( dgo_saved_ceval_shprod, dgo_saved_eval_shprod )
    IF ( PRESENT( fgot_h ) ) THEN
      cgot_h = fgot_h
    ELSE
      cgot_h = .false.
    ...
    IF ( dgo_saved_f_indexing ) then
      status = dgo_saved_eval_shprod( dgo_saved_n, x, nnz_v, index_nz_v, v, nnz_u, index_nz_u,    &
                             u, cgot_h, dgo_saved_cuserdata )
    ELSE ! handle C sparse matrix indexing
      status = dgo_saved_eval_shprod(dgo_saved_n, x, nnz_v, index_nz_v - 1, v, nnz_u, index_nz_u, &
                             u, cgot_h, dgo_saved_cuserdata )
...
!  associate procedure pointers
  ...
  dgo_saved_ceval_f = ceval_f
  dgo_saved_ceval_g = ceval_g
  dgo_saved_ceval_h = ceval_h
  IF ( C_ASSOCIATED( ceval_hprod ) ) THEN
    dgo_saved_ceval_hprod = ceval_hprod
  ELSE
    dgo_saved_ceval_hprod = C_NULL_FUNPTR
  END IF
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    dgo_saved_ceval_prec = ceval_prec
  ELSE
    dgo_saved_ceval_prec = C_NULL_FUNPTR
  END IF

  dgo_saved_n = n
  dgo_saved_cuserdata = cuserdata
  dgo_saved_ne = ne

!  solve the problem when the Hessian is explicitly available

  IF ( C_ASSOCIATED( dgo_saved_ceval_prec ) .AND. C_ASSOCIATED( dgo_saved_ceval_hprod ) ) THEN
  ...
  ELSE IF ( C_ASSOCIATED( dgo_saved_ceval_prec ) ) THEN
    CALL f_dgo_solve_with_mat( fdata, fuserdata, status, x, g,                 &
                               wrap_eval_f, wrap_eval_g, wrap_eval_h,          &
                               eval_PREC = wrap_eval_prec )
  ELSE IF ( C_ASSOCIATED( dgo_saved_ceval_hprod ) ) THEN
    CALL f_dgo_solve_with_mat( fdata, fuserdata, status, x, g,                 &
                               wrap_eval_f, wrap_eval_g, wrap_eval_h,          &
                               eval_HPROD = wrap_eval_hprod )
  ELSE
...
!  associate procedure pointers
  ...
  dgo_saved_ceval_g = ceval_g
  dgo_saved_ceval_hprod = ceval_hprod
  dgo_saved_ceval_shprod = ceval_shprod
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    dgo_saved_ceval_prec = ceval_prec
  ELSE
    dgo_saved_ceval_prec = C_NULL_FUNPTR
  ...
  dgo_saved_n = n
  dgo_saved_cuserdata = cuserdata
  dgo_saved_f_indexing = f_indexing

!  solve the problem when the Hessian is only available via products

  IF ( C_ASSOCIATED( dgo_saved_ceval_prec ) ) THEN
