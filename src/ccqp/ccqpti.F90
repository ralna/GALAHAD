...
!  =====================================
   ...
   DO data_storage_type = 1, 7
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       ...
       CALL CCQP_solve_qp( data, status, H_val, G, f, A_val, C_l, C_u,         &
     ...
     CASE ( 3 ) ! dense
       ...
       CALL CCQP_solve_qp( data, status, H_dense, G, f, A_dense, C_l, C_u,     &
                          X_l, X_u, X, C, Y, Z, X_stat, C_stat )
     CASE ( 4 ) ! diagonal
       st = ' L'
       CALL CCQP_import( control, data, status, n, m,                          &
                        ...
                        'sparse_by_rows', A_ne, A_row, A_col, A_ptr )
       CALL CCQP_solve_qp( data, status, H_diag, G, f, A_val, C_l, C_u,        &
                          X_l, X_u, X, C, Y, Z, X_stat, C_stat )
     ...
     CASE ( 6 ) ! identity
       ...
       CALL CCQP_import( control, data, status, n, m,                          &
                        ...
                        'sparse_by_rows', A_ne, A_row, A_col, A_ptr )
       CALL CCQP_solve_qp( data, status, H_zero, G, f, A_val, C_l, C_u,        &
                          X_l, X_u, X, C, Y, Z, X_stat, C_stat )
     CASE ( 7 ) ! zero
       st = ' Z'
       CALL CCQP_import( control, data, status, n, m,                          &
...
!    control%print_level = 1
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       ...
       CALL CCQP_import( control, data, status, n, m,                          &
                        ...
                        'coordinate', A_ne, A_row, A_col, A_ptr )
       CALL CCQP_solve_sldqp( data, status, W, X_0, G, f, A_val, C_l, C_u,     &
                             X_l, X_u, X, C, Y, Z, X_stat, C_stat )
