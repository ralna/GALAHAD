! THIS VERSION: GALAHAD 4.1 - 2022-12-21 AT 12:00 GMT.
...
!  History -
!   originally released as usecqp with GALAHAD Version 2.4, January 1st 2010
!   modified to form useccqp with GALAHAD 4.1, May 17th 2022

!  For full documentation, see
!   http://galahad.rl.ac.uk/galahad-www/specs.html

    MODULE GALAHAD_USECCQP_precision

!    ---------------------------------------------------------------
!    | CUTEst/AMPL interface to CCQP, an interior-point crossover  |
...
!     INTEGER ( KIND = ip_ ) :: np1, npm
      INTEGER ( KIND = ip_ ) :: i, j, l, neh, nea, mequal, mredun
...
!  checkpoint-fine-name                      CCQP.checkpoints
...
!  Default values for specfile-defined parameters
      ...
      CHARACTER ( LEN = 30 ) :: cfilename = 'CCQP.checkpoints'
...
!  If required, write the solution to a file
    ...
    &                ' entries in matrix, factors' )" )                        &
     ...
     &  3( 1X, F8.3 ) )" )                                                     &
        CCQP_inform%time%analyse, CCQP_inform%time%factorize,                  &
...
!  Compare the variants used so far
      ...
      IF ( write_checkpoints ) THEN
        ...
        WRITE( cfiledevice, "( A10, 16( 1X, I7 ), 16( 1X, F0.2 ) )" )          &
          pname, CCQP_inform%checkpointsIter( 1 : 16 ),                        &
          CCQP_inform%checkpointsTime( 1 : 16 )
      END IF
...
!  End of module USECCQP

   END MODULE GALAHAD_USECCQP_precision
