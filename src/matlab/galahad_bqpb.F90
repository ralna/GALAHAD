...
!  local variables
      ...
      mwSize :: s_len
      mwPointer :: g_pr, f_pr, xl_pr, xu_pr
      mwPointer :: h_in, g_in, f_in, xl_in, xu_in, c_in
...
!  Test input/output arguments
      ...
      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_bqpb requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs < 2 )                                                      &
            CALL mexErrMsgTxt( ' Too few input arguments to galahad_bqpb' )
          h_arg = 2 ; g_arg = 3 ; f_arg = 4
          xl_arg = 5 ; xu_arg = 6 ; c_arg = 7
          x_arg = 1 ; i_arg = 2 ; aux_arg = 3
          IF ( nrhs > c_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_bqpb' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
          CALL mexErrMsgTxt( ' Too few input arguments to galahad_bqpb' )
        h_arg = 1 ; g_arg = 2 ; f_arg = 3
      ...
      IF ( nlhs > 3 )                                                          &
        CALL mexErrMsgTxt( ' galahad_bqpb provides at most 3 output arguments' )

!  Initialize the internal structures for bqpb
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL BQPB_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  Check that BQPB_initialize has been called 
...
!  If the third argument is present, extract the input control data
        ...
        IF ( nrhs == c_arg ) THEN
          ...
          CALL BQPB_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Create inform output structure
        ...
        CALL BQPB_matlab_inform_create( plhs( i_arg ), BQPB_pointer )
...
!  Allocate space for input vectors
        ...
        ALLOCATE( p%G( p%n ), p%X_l( p%n ), p%X_u( p%n ), STAT = info )
...
!  Record output information
        ...
        CALL BQPB_matlab_inform_get( inform, BQPB_pointer )
...
!  Check for errors
        ...
        IF ( inform%status < 0 )                                               &
          CALL mexErrMsgTxt( ' Call to BQPB_solve failed ' )
      END IF
