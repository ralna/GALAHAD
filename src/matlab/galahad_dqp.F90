...
!   [ x, inform, aux ]
!     = galahad_dqp( 'existing', H, g, f, A, c_l, c_u, x_l, x_u, control )
!
...
!  arguments for DQP
      ...
      TYPE ( DQP_control_type ), SAVE :: control
      TYPE ( DQP_inform_type ), SAVE :: inform
      TYPE ( DQP_data_type ), SAVE :: data
...
!  Test input/output arguments
      ...
      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_dqp requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs > c_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_dqp' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
      ...
      IF ( nlhs > 3 )                                                          &
        CALL mexErrMsgTxt( ' galahad_dqp provides at most 3 output arguments' )

!  Initialize the internal structures for dqp
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL DQP_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  If the third argument is present, extract the input control data
        ...
        IF ( nrhs == c_arg ) THEN
          ...
          CALL DQP_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Open i/o units
        ...
        IF ( control%error > 0 ) THEN
          ...
          filename = "output_dqp." // TRIM( char_output_unit )
...
!  Create inform output structure
        ...
        CALL DQP_matlab_inform_create( plhs( i_arg ), DQP_pointer )
...
!  Solve the QP
        ...
        CALL DQP_solve( p, data, control, inform, C_stat, B_stat )
...
!  Record output information
        ...
        CALL DQP_matlab_inform_get( inform, DQP_pointer )
...
!  Check for errors
        ...
        IF ( inform%status < 0 )                                              &
          CALL mexWarnMsgTxt( ' Call to DQP_solve failed ' )
      END IF
