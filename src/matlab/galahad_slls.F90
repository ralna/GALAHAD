...
!  THIS VERSION: GALAHAD 5.5 - 2026-03-24 AT 11:40 GMT.
...
!  local variables
      ...
      INTEGER :: i, n_v, info
      INTEGER * 4 :: i4
      mwSize :: a_arg, b_arg, sigma_arg, x0_arg
      mwSize :: w_arg, xs_arg, co_arg, c_arg, optional_arg
      mwSize :: x_arg, i_arg, aux_arg
      mwSize :: s_len
      mwPointer :: a_in, b_in, sigma_in, x0_in, w_in, xs_in, co_in, c_in, i_in
      mwPointer :: b_pr, sigma_pr, x0_pr, w_pr, xs_pr, co_pr
      mwPointer :: x_pr, y_pr, z_pr, r_pr, g_pr, x_status_pr
...
!  Test input/output arguments
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          a_arg = 2 ; b_arg = 3 ; sigma_arg = 4 ; x0_arg = 5 ; optional_arg = 6
      ...
      ELSE
        ...
        IF ( nrhs < 4 )                                                        &
          CALL mexErrMsgTxt( ' Too few input arguments to galahad_slls' )
        a_arg = 1 ; b_arg = 2 ; sigma_arg = 3 ; x0_arg = 4 ; optional_arg = 5
        x_arg = 1 ; i_arg = 2 ; aux_arg = 3
        IF ( nrhs > 8 )                                                        &
          CALL mexErrMsgTxt( ' Too many input arguments to galahad_slls' )
      END IF
...
!  Input x_0 as initial x
        ...
        x0_pr = mxGetPr( x0_in )
        CALL MATLAB_copy_from_ptr( x0_pr, p%X, p%n )
          ...
          ELSE IF ( mxIsClass( i_in, 'double') ) THEN
            ...
            IF ( p%n /= p%o ) THEN
              IF ( n_v == p%o ) THEN  ! argument is w
                array = 'w'
              ELSE IF ( n_v == p%n ) THEN
                IF ( co_arg < 0 ) THEN  ! argument is cohort
                  array = 'c'
                ELSE  ! argument is x_s
                  array = 'x'
                END IF
