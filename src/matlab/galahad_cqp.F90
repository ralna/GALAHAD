...
!  arguments for CQP
      ...
      TYPE ( CQP_control_type ), SAVE :: control
      TYPE ( CQP_inform_type ), SAVE :: inform
      TYPE ( CQP_data_type ), SAVE :: data
...
!  Test input/output arguments
      ...
      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_cqp requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs < 2 )                                                      &
            CALL mexErrMsgTxt( ' Too few input arguments to galahad_cqp' )
          h_arg = 2 ; g_arg = 3 ; f_arg = 4 ; a_arg = 5
          cl_arg = 6 ; cu_arg = 7 ; xl_arg = 8 ; xu_arg = 9 ; c_arg = 10
          x_arg = 1 ; i_arg = 2 ; aux_arg = 3
          IF ( nrhs > c_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_cqp' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
      ...
      IF ( nlhs > 3 )                                                          &
        CALL mexErrMsgTxt( ' galahad_cqp provides at most 3 output arguments' )

!  Initialize the internal structures for cqp
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL CQP_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  Check that CQP_initialize has been called
...
!  If the third argument is present, extract the input control data
        ...
        IF ( nrhs == c_arg ) THEN
          ...
          CALL CQP_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Open i/o units
        ...
        IF ( control%error > 0 ) THEN
          ...
          filename = "output_cqp." // TRIM( char_output_unit )
...
!  Create inform output structure
        ...
        CALL CQP_matlab_inform_create( plhs( i_arg ), CQP_pointer )
...
!       i = mexPrintf( TRIM( str ) // ACHAR( 10 ) )
        CALL CQP_solve( p, data, control, inform, C_stat, B_stat )
...
!  Record output information
        ...
        CALL CQP_matlab_inform_get( inform, CQP_pointer )
...
!  Check for errors
        ...
        IF ( inform%status < 0 )                                               &
          CALL mexWarnMsgTxt( ' Call to CQP_solve failed ' )
      END IF
