...
!  THIS VERSION: GALAHAD 2.4 - 09/03/2010 AT 08:15 GMT.
...
!   [ x, inform, aux ]
!     = galahad_qpb( 'existing', H, g, f, A, c_l, c_u, x_l, x_u, control )
!
...
!      manual for the fortran 90 package GALAHAD_QPB.
...
!      the fortran 90 package GALAHAD_QPB.
!      See: http://galahad.rl.ac.uk/galahad-www/doc/qpb.pdf
...
!  arguments for QPB
      ...
      TYPE ( QPB_inform_type ), SAVE :: inform
...
!  Test input/output arguments
      ...
      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_qpb requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs < 2 )                                                      &
            CALL mexErrMsgTxt( ' Too few input arguments to galahad_qpb' )
          h_arg = 2 ; g_arg = 3 ; f_arg = 4 ; a_arg = 5
          cl_arg = 6 ; cu_arg = 7 ; xl_arg = 8 ; xu_arg = 9 ; c_arg = 10
          x_arg = 1 ; i_arg = 2 ; aux_arg = 3
          IF ( nrhs > c_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_qpb' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
      ...
      IF ( nlhs > 3 )                                                          &
        CALL mexErrMsgTxt( ' galahad_qpb provides at most 3 output arguments' )
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL QPB_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  If the third argument is present, extract the input control data
        ...
        IF ( nrhs == c_arg ) THEN
          ...
          CALL QPB_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Create inform output structure
        ...
        CALL QPB_matlab_inform_create( plhs( i_arg ), QPB_pointer )
...
!  Record output information
        ...
        CALL QPB_matlab_inform_get( inform, QPB_pointer )
