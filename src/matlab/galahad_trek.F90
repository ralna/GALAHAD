...
!            the manual for the modern fortran package GALAHAD_TREK.
...
!      of inform.time, inform.TREK_inform and inform.SLS_inform 
!      are themselves structures, holding the
!      components of the derived types TREK_time_type, 
!      TREK_inform_type and SLS_inform_type, respectively.
!      See: http://galahad.rl.ac.uk/galahad-www/doc/trek.pdf
!
! *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
...
!  local variables
      ...
      INTEGER :: i, info
      INTEGER * 4 :: i4, n
      mwSize :: h_arg, c_arg, radius_arg, con_arg, s_arg
      mwSize :: x_arg, i_arg, s_len

      mwPointer :: h_in, c_in, radius_in, g_in, s_in
      mwPointer :: x_pr, g_pr, radius_pr
...
!  arguments for TREK
      ...
      TYPE ( SMT_type ) :: H, S
      TYPE ( TREK_data_type ), SAVE :: data
      TYPE ( TREK_control_type ), SAVE :: control
      TYPE ( TREK_inform_type ) :: inform

!  Test input/output arguments

      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_trek requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs < 2 )                                                      &
            CALL mexErrMsgTxt( ' Too few input arguments to galahad_trek' )
          h_arg = 2 ; c_arg = 3 ; radius_arg = 4 ; con_arg = 5 ; s_arg = 6
          x_arg = 1 ; i_arg = 2
          IF ( nrhs > s_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_trek' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
          CALL mexErrMsgTxt( ' Too few input arguments to galahad_trek' )
        h_arg = 1 ; c_arg = 2 ; radius_arg = 3 ; con_arg = 4 ; s_arg = 5
        x_arg = 1 ; i_arg = 2
        IF ( nrhs > s_arg )                                                    &
          CALL mexErrMsgTxt( ' Too many input arguments to galahad_trek' )
      END IF
      ...
      IF ( nlhs > 2 )                                                          &
        CALL mexErrMsgTxt( ' galahad_trek provides at most 2 output arguments' )

!  Initialize the internal structures for trek
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL TREK_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  If the control argument is present, extract the input control data
        ...
        IF ( nrhs >= con_arg ) THEN
          ...
          CALL TREK_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Open i/o units
        ...
        IF ( control%error > 0 ) THEN
          ...
          filename = "output_trek." // TRIM( char_output_unit )
...
!  Create inform output structure
        ...
        CALL TREK_matlab_inform_create( plhs( i_arg ), TREK_pointer )
...
!  Record output information
         ...
         CALL TREK_matlab_inform_get( inform, TREK_pointer )

!  Check for errors

        IF ( inform%status < 0 )                                               &
          CALL mexErrMsgTxt( ' Call to TREK_solve failed ' )
      END IF
