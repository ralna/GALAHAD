...
!   [ x, inform, aux ]
!     = galahad_bqp( 'existing', H, g, f, x_l, x_u, control )
!
...
!  Test input/output arguments
      ...
      IF ( nrhs < 1 )                                                          &
        CALL mexErrMsgTxt( ' galahad_bqp requires at least 1 input argument' )
      ...
      IF ( mxIsChar( prhs( 1 ) ) ) THEN
        ...
        IF ( .NOT. ( TRIM( mode ) == 'initial' .OR.                            &
          ...
          IF ( nrhs < 2 )                                                      &
            CALL mexErrMsgTxt( ' Too few input arguments to galahad_bqp' )
          h_arg = 2 ; g_arg = 3 ; f_arg = 4
          xl_arg = 5 ; xu_arg = 6 ; c_arg = 7
          x_arg = 1 ; i_arg = 2 ; aux_arg = 3
          IF ( nrhs > c_arg )                                                  &
            CALL mexErrMsgTxt( ' Too many input arguments to galahad_bqp' )
        END IF
      ELSE
        mode = 'all'
        IF ( nrhs < 2 )                                                        &
      ...
      IF ( nlhs > 3 )                                                          &
        CALL mexErrMsgTxt( ' galahad_bqp provides at most 3 output arguments' )

!  Initialize the internal structures for bqp
...
!  If required, return the default control parameters
          ...
          IF ( nlhs > 0 )                                                      &
            CALL BQP_matlab_control_get( plhs( c_arg ), control )
          RETURN
        END IF
      END IF
...
!  If the third argument is present, extract the input control data
        ...
        IF ( nrhs == c_arg ) THEN
          ...
          CALL BQP_matlab_control_set( c_in, control, s_len )
        END IF
...
!  Create inform output structure
        ...
        CALL BQP_matlab_inform_create( plhs( i_arg ), BQP_pointer )
...
!  Record output information
        ...
        CALL BQP_matlab_inform_get( inform, BQP_pointer )
...
!  Check for errors
        ...
        IF ( inform%status < 0 )                                              &
          CALL mexErrMsgTxt( ' Call to BQP_solve failed ' )
      END IF
