#include "galahad_modules.h"
   PROGRAM GALAHAD_UGO_TEST_PROGRAM2  !  GALAHAD 4.1 - 2022-12-09 AT 07:45 GMT
...
!  REAL ( KIND = rp_ ), PARAMETER :: p = 4.0_rp_
   REAL ( KIND = rp_ ), PARAMETER :: p = 0.0_rp_
...
!  problem 23 (from the Chebfun manual)
   ...
   CASE ( 23 )
     x_l = 0.0_rp_ ; x_u = 15.0_rp_
     f_min = -1.990085468159411_rp_ ; x_min = 4.852581429906174_rp_
   END SELECT
...
!  control%stop_length = 5.0_rp_ * 10.0_rp_ ** ( - 5 )
   ...
   END PROGRAM GALAHAD_UGO_TEST_PROGRAM2
   ...
   INTEGER ( KIND = ip_ ) :: k
   REAL ( KIND = rp_ ) :: a, b, c, ca, c2, c3, c4, c5, c6, e, rk, rkp1
   REAL ( KIND = rp_ ) :: da, db, bm1, s, sa, s2
   REAL ( KIND = rp_ ), PARAMETER :: pi = 4.0_rp_ * ATAN( 1.0_rp_ )
...
!  problem 01
   ...
   CASE ( 01 )
     ...
     f = c6 * x ** 6 + c5 * x ** 5 + c4 * x ** 4 + c3 * x ** 3                 &
         + c2 * x ** 2 - x + 0.1_rp_
     g = 6.0_rp_ * c6 * x ** 5 + 5.0_rp_ * c5 * x ** 4  + 4.0_rp_ * c4 * x **3 &
         + 3.0_rp_ * c3 * x ** 2 + 2.0_rp_ * c2 * x - 1.0_rp_
...
!  problem 13
   ...
   CASE ( 13 )
     ...
     h = - a * ( a - 1.0_rp_ ) * x ** ( a - 2.0_rp_ ) + 2.0_rp_ * b * c ** bm1 &
         - 4.0_rp_ * b * bm1 * x * x * c ** ( b - 2.0_rp_ )
...
!  problem 23
   ...
   CASE ( 23 )
     ...
     s2 = SIN( x ** 2 )
     f = s + s2
     g = COS( x ) + 2.0_rp_ * x * s2
     h = - s + 2.0_rp_ * s2 - 4.0_rp_ * x * x * s2
