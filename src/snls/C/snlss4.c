/* snlss4.c */
/* Examples for the SNLS C interface using C sparse matrix indexing */
...
int main(void) {
    ...
    snls_import( &control, &data, &status, n, m,
                "absent", j_ne, NULL, NULL, NULL,
                "absent", h_ne, NULL, NULL, NULL,
                "absent", p_ne, NULL, NULL, NULL, W );
    while(true){ // reverse-communication loop
      snls_solve_reverse_without_mat( &data, &status, &eval_status,
                                     n, m, x, c, g, &transpose,
                                     u, v, y, p_ne, NULL );
    ...
    if(inform.status == 0){
        ...
        printf(" SNLS_solve exit status = %1" d_ipc_ "\n", inform.status);
    }
