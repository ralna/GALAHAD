...
#include "galahad_modules.h"
   PROGRAM GALAHAD_SNLS_test_deck
...
!      write(6,*) ' problem ', s, 'in staus', inform%status
       SELECT CASE( i )
       CASE( 3 )
         CALL SNLS_solve( nlp, control, inform, data, userdata, W = W )
       CASE DEFAULT
...
!           nlp%C( 1 ) = nlp%C( 1 ) / nlp%C( 1 )
     ...
     ELSE
       WRITE( 6, "( I3, ': SNLS_solve exit status = ', I6 )" ) s, inform%status
     END IF
...
!    control%print_level = 1
     ...
     ELSE
       WRITE( 6, "( I2, ': SNLS_solve exit status = ', I6 )" ) i, inform%status
     END IF
...
!        DO usew = 0, 0
!if(usew /=0) cycle
         CALL SNLS_initialize( data, control, inform )! Initialize controls
!        control%print_level = 1
!        control%subproblem_control%print_level = 1
...
!        control%subproblem_control%glrt_control%print_level = 3
         ...
         ELSE
           WRITE( 6, "( I1, ',', I2, 2( ',', I1 ), ': SNLS_solve exit status', &
          & ' = ', I6 )" ) rev, scaling, model, usew, inform%status
         END IF
         CALL SNLS_terminate( data, control, inform ) ! delete workspace
       END DO
       END DO
     END DO
   END DO
...
!    control%subproblem_control%print_level = 4
     ...
     IF ( inform%status == 0 ) THEN
      ...
      &  ' status = ', I6 )" ) i, inform%iter, inform%norm_c, inform%status
     ELSE
       WRITE( 6, "( I2, ': SNLS_solve exit status = ', I0 )" ) i, inform%status
     END IF
...
!      control%subproblem_control%print_level = 1
       ...
       ELSE
         WRITE( 6, "( A1, I1, ': SNLS_solve exit status = ', I0 )" )           &
...
!     U( 2 ) = 0.5_rp_ * V( 2 )
   ...
   END PROGRAM GALAHAD_SNLS_test_deck
