! THIS VERSION: GALAHAD 5.5 - 2026-01-14 AT 13:30 GMT.

#include "galahad_modules.h"

!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ S N L S  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  January 14th 2026

   MODULE GALAHAD_USESNLS_precision

     USE GALAHAD_KINDS_precision

!  This is the driver program for running SNLS for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ S N L S   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_SNLS( input )
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNSNLS'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNSNLS.SPC'
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: dfilename = 'SNLS.data'
     CHARACTER ( LEN = 30 ) :: rfilename = 'SNLSRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'SNLSSOL.d'
     CHARACTER ( LEN = 30 ) :: wfilename = 'SNLSSAVE.d'
...
!  Output file characteristics
     ...
     CHARACTER ( LEN =  6 ) :: solv = 'SNLS '

!  ------------------ Open the specfile for snls ----------------
...
!  Set up control parameters prior to the next solution
     ...
     IF ( is_specfile ) CALL SNLS_read_specfile( control, input_specfile )
...
!  Read a previous solution file for a re-entry
     ...
     IF ( warm_start .AND. wfiledevice > 0 ) THEN
       ...
       IF ( name /= nlp%pname ) THEN
         WRITE( out, "( /, ' *** Exit from usesnls: re-entry requested with',  &
       ...
       IF ( i /= nlp%n ) THEN
         WRITE( out, "( /, ' *** Exit from usesnls: number of variables',      &
        &        ' changed from ', I0,' to ', I0,' on re-entry ' )" ) nlp%n, i
         STOP
       END IF
       READ( wfiledevice, "( I8 )" ) i
       IF ( i /= nlp%m_r ) THEN
         WRITE( out, "( /, ' *** Exit from usesnls: number of residuals',      &
       ...
       DO i = 1, nlp%n
         ...
         IF ( name /= nlp%VNAMES( i ) ) THEN
           WRITE( out, "( /, ' *** Exit from usesnls: variable named ',        &
...
!  end of function STRING_trim_integer_8
     ...
     END SUBROUTINE USE_SNLS

!  End of module USESNLS

   END MODULE GALAHAD_USESNLS_precision
