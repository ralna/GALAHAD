...
!-*-*-*-*-*-*-*-  G A L A H A D   U S E L 2 R T   M O D U L E  -*-*-*-*-*-*-*-*-
...
!-*-*-*-*-*-*-*-*-*-   U S E _ C Q P  S U B R O U T I N E   -*-*-*-*-*-*-*-*-
     ...
     SUBROUTINE USE_L2RT( input )

!  --------------------------------------------------------------------
!
!  Form a multilevel incomplete QR factorization of a rectangular
!  matrix A using the GALAHAD package GALAHAD_L2RT
!
!  --------------------------------------------------------------------
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 16 ) :: specname = 'RUNL2RT'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNL2RT.SPC'
      CHARACTER ( LEN = 4 ) :: rowcol

!  The default values for L2RT could have been set as:

! BEGIN RUNL2RT SPECIFICATIONS (DEFAULT)
!  write-problem-data                            NO
!  problem-data-file-name                        L2RT.data
...
!  write-solution                                NO
!  solution-file-name                            L2RTSOL.d
!  solution-file-device                          62
!  write-result-summary                          NO
!  result-summary-file-name                      L2RTRES.d
!  result-summary-file-device                    47
! END RUNL2RT SPECIFICATIONS
...
!     INTEGER ( KIND = ip_ ) :: lfiledevice = 78
      ...
      CHARACTER ( LEN = 30 ) :: dfilename = 'L2RT.data'
      CHARACTER ( LEN = 30 ) :: ifilename = 'INITIAL.SIF'
      CHARACTER ( LEN = 30 ) :: rfilename = 'L2RTRES.d'
      CHARACTER ( LEN = 30 ) :: sfilename = 'L2RTSOL.d'
...
!   Define the keywords
        ...
        spec( 23 )%keyword = 'order'
        spec( 24 )%keyword = 'weight'
        spec( 25 )%keyword = 'shift'
        spec( 26 )%keyword = ''
        spec( 27 )%keyword = ''
        spec( 28 )%keyword = ''
        spec( 29 )%keyword = ''
...
!   Interpret the result
        ...
        CALL SPECFILE_assign_real( spec( 23 ), order, errout )
        CALL SPECFILE_assign_real( spec( 24 ), weight, errout )
        CALL SPECFILE_assign_real( spec( 25 ), shift, errout )
      END IF
...
!  Determine the names of the problem, variables and constraints.
      ...
      CALL CUTEST_probname_r( cutest_status, pname )
...
!  Set all default values, and override defaults if requested
      ...
      CALL L2RT_initialize( L2RT_data, L2RT_control, L2RT_inform )

      IF ( is_specfile )                                                       &
        CALL L2RT_read_specfile( L2RT_control, input_specfile )

      printo = out > 0 .AND. L2RT_control%print_level > 0
      printe = out > 0 .AND. L2RT_control%print_level >= 0
...
!       solv = ' L2RT'
!       IF ( printo ) WRITE( out, " ( ' ** L2RT solver used ** ' ) " )
...
!  Iteration to find the minimizer
          ...
          L2RT_inform%status = 1
          U( : A_ls%m ) = B( : A_ls%m )

          DO
            CALL L2RT_solve( A_ls%m, A_ls%n, order, weight, shift,             &
                             X( : A_ls%n ), U( : A_ls%m ), V( : A_ls%n ),      &
                             L2RT_data, L2RT_control, L2RT_inform )
            branch = L2RT_inform%status
...
!  Compute the residuals for checking
              ...
              IF ( printe ) THEN
               ...
               &  ' 2nd pass L2RT iterations, overall time = ', F6.2 )" )      &
                  L2RT_inform%iter, L2RT_inform%iter_pass2, clockt - clocks
                WRITE( out, "( '  ||b||   calculated              =',          &
               &  ES15.8 )" ) SQRT( DOT_PRODUCT( B( : A_ls%m ), B( : A_ls%m)))
                WRITE( out, "( '  ||x||_P recurred and calculated =',          &
               &  2ES15.8 )" ) L2RT_inform%x_norm, x_norm
                WRITE( out, "( ' ||Ax-b|| recurred and calculated =',          &
               &  2ES15.8 )" ) L2RT_inform%r_norm,                             &
...
!  Error returns
            ...
            CASE DEFAULT
              CALL CPU_TIME( timet ) ; CALL CLOCK_time( clockt )
              IF ( printe ) WRITE( out, "( ' L2RT_solve exit status = ', I0 )")&
                L2RT_inform%status
              EXIT
...
!  Iteration to find the minimizer
          ...
          DO
            CALL L2RT_solve( m_used, n_used, order, weight, shift, X, U, V,    &
                             L2RT_data, L2RT_control, L2RT_inform )
            branch = L2RT_inform%status
...
!          pname, n, m, iter, qfval, status, clockt
        IF ( L2RT_inform%status >= 0 ) THEN
          WRITE( rfiledevice, "( A10, 2I8, I3, L2, I10, I8, 2F12.2, I4 )" )    &
            pname, m_used, n_used, preconditioner, consistent, n_fact,         &
            L2RT_inform%iter, clockp - clocks,                                 &
            clockt - clocks, L2RT_inform%status
        ELSE
          WRITE( rfiledevice, "( A10, 2I8, I3, L2, I10, I8, 2F12.2, I4 )" )    &
            pname, m_used, n_used, preconditioner, consistent, - n_fact,       &
            - L2RT_inform%iter,                                                &
            - ( clockp - clocks ), - ( clockt - clocks ), - L2RT_inform%status
        END IF
      END IF
      CALL L2RT_terminate( L2RT_data, L2RT_control, L2RT_inform )
      GO TO 600
...
!  End of subroutine USE_L2RT

     END SUBROUTINE USE_L2RT

!  End of module USEL2RT

   END MODULE GALAHAD_USEL2RT_precision
