...
!  return status. See L2RT_solve for details
...
!  the multiplier, mu + sigma * ||x||^(p-2) * sqrt(||Ax-b||^2 + mu ||x||^2)
...
!-*-*-*-*-*-  L 2 R T _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-*-
      ...
      SUBROUTINE L2RT_initialize( data, control, inform )
...
!---------------------------------
      ...
      REAL ( KIND = rp_ ), INTENT( IN ) :: p, sigma, mu
...
!---------------------------------
      ...
      INTEGER ( KIND = ip_ ) :: i
      REAL ( KIND = rp_ ) :: alpha_1, dec_tol, rho_tilde, phi_tilde
...
!                 0.9_rp_ * data%LAMBDA( inform%iter - 1 )
                data%LAMBDA( inform%iter ) = data%LAMBDA( inform%iter - 1 )
              END IF
            ELSE
              data%LAMBDA( inform%iter ) = data%LAMBDA( inform%iter - 1 )
...
!-----------------------------------------------
      ...
      REAL ( KIND = rp_ ), INTENT( IN ) :: p, sigma, mu, beta, error_tol
      ...
      REAL ( KIND = rp_ ), INTENT( OUT ), DIMENSION( n + 1 ) :: RES
...
!-----------------------------------------------
      ...
      REAL ( KIND = rp_ ) :: h_norm, error, power
      REAL ( KIND = rp_ ) :: phi, phi_prime, psi, psi_prime, dl_phi, dl_psi
      REAL ( KIND = rp_ ) :: pi, pi_prime, nu, nu_prime, omega, prod
...
!-----------------------------------------------
     ...
     INTEGER ( KIND = ip_ ), INTENT( IN ) :: m, n
     REAL ( KIND = rp_ ), INTENT( IN ) :: power, weight, shift
...
!-*-*-*-*-*-  End of G A L A H A D _ L 2 R T  double  M O D U L E  *-*-*-*-*-*-
   ...
   END MODULE GALAHAD_L2RT_precision
