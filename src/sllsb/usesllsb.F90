...
!-*-*-*-*-*-*-*-  G A L A H A D   U S E S L L S B   M O D U L E  -*-*-*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
    ...
    MODULE GALAHAD_USESLLSB_precision

!     ---------------------------------------------------------------------
!    | CUTEst/AMPL interface to SLLSB, an interior-point crossover         |
!    | algorithm for simplex-constrained linear least-squares optimization |
!     ---------------------------------------------------------------------
...
!-*-*-*-*-*-*-*-*-*-   U S E _ S L L S B  S U B R O U T I N E   -*-*-*-*-*-*-*-

     SUBROUTINE USE_SLLSB( input, close_input )
...
!                    x_l <=  x <= x_u
!
!  (with contrived A_o and b) using the GALAHAD package GALAHAD_SLLSB
!
!  --------------------------------------------------------------------
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 8 ) :: specname = 'RUNSLLSB'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 12 ) :: runspec = 'RUNSLLSB.SPC'

!  The default values for SLLSB could have been set as:

! BEGIN RUNSLLSB SPECIFICATIONS (DEFAULT)
!  write-problem-data                        NO
!  problem-data-file-name                    SLLSB.data
...
!  Default values for specfile-defined parameters
      ...
      CHARACTER ( LEN = 30 ) :: dfilename = 'SLLSB.data'
      CHARACTER ( LEN = 30 ) :: ifilename = 'INITIAL.SIF'
      CHARACTER ( LEN = 30 ) :: pfilename = 'PRESOLVE.SIF'
      CHARACTER ( LEN = 30 ) :: qfilename = 'SCALED.SIF'
      CHARACTER ( LEN = 30 ) :: rfilename = 'SLLSBRES.d'
      CHARACTER ( LEN = 30 ) :: sfilename = 'SLLSBSOL.d'
      CHARACTER ( LEN = 30 ) :: cfilename = 'SLLSB.checkpoints'
...
!  Set all default values, and override defaults if requested
      ...
      IF ( is_specfile )                                                       &
        CALL SLLSB_read_specfile( control, input_specfile )
      IF ( scale /= 0 )                                                        &
...
!  =================
        ...
        solv = ' SLLSB'
        IF ( printo ) WRITE( out, " ( ' ** SLLSB solver used ** ' ) " )
        CALL SLLSB_solve( prob, data, control, inform )
...
!     DEALLOCATE( RES )
      ...
      IF ( control%print_level > 5 ) THEN
        ...
        WRITE( 6, "( ' c')" )
        WRITE( 6, "( ( 5ES16.8 ) )" ) prob%c
        WRITE( 6, "( ' y')" )
        WRITE( 6, "( ( 5ES16.8 ) )" ) prob%y
        WRITE( 6, "( ' z')" )
...
!  End of subroutine USE_SLLSB

     END SUBROUTINE USE_SLLSB

!  End of module USESLLSB

   END MODULE GALAHAD_USESLLSB_precision
