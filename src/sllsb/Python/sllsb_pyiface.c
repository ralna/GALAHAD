//* \file sllsb_pyiface.c */

/*
 * THIS VERSION: GALAHAD 5.5 - 2026-03-06 AT 13:00 GMT.
 *
 *-*-*-*-*-*-*-*-*-  GALAHAD_SLLSB PYTHON INTERFACE  *-*-*-*-*-*-*-*-*-*-
 *
 *  Copyright reserved, Gould/Orban/Toint, for GALAHAD productions
 *  Principal author: Jaroslav Fowkes & Nick Gould
 *
 *  History -
 *   originally released GALAHAD Version 5.5. January 19th 2026
 *
...
static struct sllsb_control_type control;  // control struct
static struct sllsb_inform_type inform;    // inform struct
...
static PyObject* sllsb_make_time_dict(const struct sllsb_time_type *time){
...
PyObject* sllsb_make_inform_dict(const struct sllsb_inform_type *inform){
    ...
    PyDict_SetItemString(py_inform, "time",
                         sllsb_make_time_dict(&inform->time));
...
static PyObject* py_sllsb_initialize(PyObject *self){

    // Call sllsb_initialize
    sllsb_initialize(&data, &control, &status);

    // Record that SLLSB has been initialised
    init_called = true;
...
static PyObject* py_sllsb_load(PyObject *self, PyObject *args, PyObject *keywds){
    ...
    sllsb_reset_control(&control, &data, &status);

    // Update SLLSB control options
    if(!sllsb_update_control(&control, py_options))
        return NULL;
...
static PyObject* py_sllsb_information(PyObject *self){

    // Check that package has been initialised
    if(!check_init(init_called))
        return NULL;

    // Call sllsb_information
    sllsb_information(&data, &inform, &status);
...
static PyObject* py_sllsb_terminate(PyObject *self){

    // Check that package has been initialised
    if(!check_init(init_called))
        return NULL;

    // Call sllsb_terminate
    sllsb_terminate(&data, &control, &inform);
...
//  *-*-*-*-*-*-*-*-*-*-   INITIALIZE SLLSB PYTHON MODULE    -*-*-*-*-*-*-*-*-*-*

/* sllsb python module method table */
static PyMethodDef sllsb_module_methods[] = {
    {"initialize", (PyCFunction) py_sllsb_initialize, METH_NOARGS, NULL},
    {"load", (PyCFunction) py_sllsb_load, METH_VARARGS | METH_KEYWORDS, NULL},
    {"solve", (PyCFunction) py_sllsb_solve, METH_VARARGS | METH_KEYWORDS, NULL},
    {"information", (PyCFunction) py_sllsb_information, METH_NOARGS, NULL},
    {"terminate", (PyCFunction) py_sllsb_terminate, METH_NOARGS, NULL},
    {NULL, NULL, 0, NULL}  /* Sentinel */
};

/* sllsb python module documentation */

PyDoc_STRVAR(sllsb_module_doc,

"The sllsb package uses a primal-dual interior-point method to solve the\n"
...
"\n"
"See $GALAHAD/html/Python/sllsb.html for argument lists, call order\n"
"and other details.\n"
"\n"
);

/* sllsb python module definition */
static struct PyModuleDef module = {
   PyModuleDef_HEAD_INIT,
   "sllsb",               /* name of module */
   sllsb_module_doc,      /* module documentation, may be NULL */
   -1,                  /* size of per-interpreter state of the module,or -1
                           if the module keeps state in global variables */
   sllsb_module_methods   /* module methods */
};

/* Python module initialization */
PyMODINIT_FUNC PyInit_sllsb(void) { // must be same as module name above
