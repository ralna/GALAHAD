...
!-*-*-*-*-*-*-*-  G A L A H A D _  S L L S B    C   I N T E R F A C E  -*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_SLLSB_precision_ciface
    USE GALAHAD_KINDS_precision
    USE GALAHAD_common_ciface
    USE GALAHAD_SLLSB_precision, ONLY:                                         &
        f_sllsb_control_type   => SLLSB_control_type,                          &
        f_sllsb_time_type      => SLLSB_time_type,                             &
        f_sllsb_inform_type    => SLLSB_inform_type,                           &
        f_sllsb_full_data_type => SLLSB_full_data_type,                        &
        f_sllsb_initialize     => SLLSB_initialize,                            &
        f_sllsb_read_specfile  => SLLSB_read_specfile,                         &
        f_sllsb_import         => SLLSB_import,                                &
        f_sllsb_reset_control  => SLLSB_reset_control,                         &
        f_sllsb_solve_given_a  => SLLSB_solve_given_a,                         &
        f_sllsb_information    => SLLSB_information,                           &
        f_sllsb_terminate      => SLLSB_terminate
...
!-------------------------------------------------
    ...
    TYPE, BIND( C ) :: sllsb_control_type
    ...
    END TYPE sllsb_control_type

    TYPE, BIND( C ) :: sllsb_time_type
    ...
    END TYPE sllsb_time_type

    TYPE, BIND( C ) :: sllsb_inform_type
...
!  copy C control parameters to fortran
    ...
    TYPE ( sllsb_control_type ), INTENT( IN ) :: ccontrol
    TYPE ( f_sllsb_control_type ), INTENT( OUT ) :: fcontrol
...
!  copy fortran control parameters to C
    ...
    TYPE ( f_sllsb_control_type ), INTENT( IN ) :: fcontrol
    TYPE ( sllsb_control_type ), INTENT( OUT ) :: ccontrol
...
!  copy C time parameters to fortran
    ...
    TYPE ( sllsb_time_type ), INTENT( IN ) :: ctime
    TYPE ( f_sllsb_time_type ), INTENT( OUT ) :: ftime
...
!  copy fortran time parameters to C
    ...
    TYPE ( f_sllsb_time_type ), INTENT( IN ) :: ftime
    TYPE ( sllsb_time_type ), INTENT( OUT ) :: ctime
...
!  copy C inform parameters to fortran
    ...
    TYPE ( sllsb_inform_type ), INTENT( IN ) :: cinform
    TYPE ( f_sllsb_inform_type ), INTENT( OUT ) :: finform
    INTEGER ( KIND = ip_ ) :: i
...
!  copy fortran inform parameters to C
    ...
    TYPE ( f_sllsb_inform_type ), INTENT( IN ) :: finform
    TYPE ( sllsb_inform_type ), INTENT( OUT ) :: cinform
    INTEGER ( KIND = ip_ ) :: i, l
  ...
  END MODULE GALAHAD_SLLSB_precision_ciface

!  -------------------------------------
!  C interface to fortran sllsb_initialize
!  -------------------------------------

  SUBROUTINE sllsb_initialize( cdata, ccontrol, status ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE
...
!  local variables
  ...
  TYPE ( f_sllsb_full_data_type ), POINTER :: fdata
  TYPE ( f_sllsb_control_type ) :: fcontrol
  TYPE ( f_sllsb_inform_type ) :: finform
  LOGICAL :: f_indexing
...
!  initialize required fortran types
  ...
  CALL f_sllsb_initialize( fdata, fcontrol, finform )
...
!  ----------------------------------------
!  C interface to fortran sllsb_read_specfile
!  ----------------------------------------

  SUBROUTINE sllsb_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( sllsb_control_type ), INTENT( INOUT ) :: ccontrol
...
!  read control parameters from the specfile
  ...
  CALL f_sllsb_read_specfile( fcontrol, device )
...
!  ---------------------------------
!  C interface to fortran sllsb_inport
!  ---------------------------------

  SUBROUTINE sllsb_import( ccontrol, cdata, status, n, o, m, caotype, aone,    &
...
!  dummy arguments
  ...
  INTEGER ( KIND = ipc_ ), INTENT( IN ), DIMENSION( m ), OPTIONAL :: cohort
...
!  import the problem data into the required SLLSB structure

  CALL f_sllsb_import( fcontrol, fdata, status, n, o, m,                       &
                       faotype, aone, aorow, aocol, aoptr, COHORT = cohort )
...
!  ----------------------------------------
!  C interface to fortran sllsb_reset_control
!  ----------------------------------------

  SUBROUTINE sllsb_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_sllsb_reset_control( fcontrol, fdata, status )
  RETURN

  END SUBROUTINE sllsb_reset_control

!  ------------------------------------
!  C interface to fortran sllsb_solve_sllsb
!  ------------------------------------

  SUBROUTINE sllsb_solve_given_a( cdata, status, n, o, m, aone, aoval, b,      &
                                  regularization_weight, x, y, z, r, xstat,    &
                                  w, x_s ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE
...
!  solve the qp
  ...
  CALL f_sllsb_solve_given_a( fdata, status, aoval, b, regularization_weight,  &
                              x, y, z, r, xstat, w, x_s )
...
!  --------------------------------------
!  C interface to fortran sllsb_information
!  --------------------------------------

  SUBROUTINE sllsb_information( cdata, cinform, status ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( sllsb_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain SLLSB solution information

  CALL f_sllsb_information( fdata, finform, status )
...
!  ------------------------------------
!  C interface to fortran sllsb_terminate
!  ------------------------------------

  SUBROUTINE sllsb_terminate( cdata, ccontrol, cinform ) BIND( C )
  USE GALAHAD_SLLSB_precision_ciface
  IMPLICIT NONE
...
!  deallocate workspace
  ...
  CALL f_sllsb_terminate( fdata, fcontrol, finform )
