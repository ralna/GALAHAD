...
!-*-*-*-*-*-*-*-*-*-  G A L A H A D   U S E _ A P S  -*-*-*-*-*-*-*-*-*-*-

!  Nick Gould, for GALAHAD productions
!  Copyright reserved
!  March 15th 2018

   MODULE GALAHAD_USEAPS_double

!  This is the driver program for running APS for a variety of computing
...
!-*-*-*-*-*-*-*-*-*-  U S E _ A P S   S U B R O U T I N E  -*-*-*-*-*-*-*-

     SUBROUTINE USE_APS( input )
...
!    LOGICAL :: write_result_summary = .FALSE.
     ...
     CHARACTER ( LEN = 30 ) :: rfilename = 'APSRES.d'
     CHARACTER ( LEN = 30 ) :: sfilename = 'APSSOL.d'
...
!  Specfile characteristics
     ...
     CHARACTER ( LEN = 16 ) :: specname = 'RUNAPS'
     TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
     CHARACTER ( LEN = 16 ) :: runspec = 'RUNAPS.SPC'

!  ------------------ Open the specfile for aps ----------------
...
!   Define the keywords
       ...
       spec( 3 )%keyword  = 'aps-result-summary-file-name'
       spec( 4 )%keyword = 'aps-result-summary-file-device'
       spec( 7 )%keyword  = 'print-full-solution'
       spec( 8 )%keyword  = 'write-solution'
       spec( 9 )%keyword  = 'aps-solution-file-name'
       spec( 10 )%keyword  = 'aps-solution-file-device'
       spec( 11 )%keyword  = 'trust-region-subproblem'
       spec( 13 )%keyword = 'trus-region-radius'
...
!  Set up data for next problem
     ...
     IF ( is_specfile ) CALL APS_read_specfile( control, input_specfile )
...
!  Solve the problem
       ...
       IF ( control%print_level > 0 .AND. control%out > 0 )                    &
         WRITE( control%out, "( ' APS used ', / )" )
       CALL APS_solve( n, radius, f, G, H, X, data, control, inform )
       IF ( control%print_level > 0 .AND. control%out > 0 )                    &
         WRITE( control%out, "( /, ' APS used ' )" )
       IF ( control%print_level > 0 .AND. control%out > 0 )                    &
...
!  If required, append results to a file,
       ...
       IF ( write_result_summary ) THEN
         ...
         ELSE
           WRITE( rfiledevice, 2040 ) pname, n, inform%obj, inform%multiplier, &
             inform%factorizations, - inform%time%clock_total, inform%status
         END IF
       END IF
...
!  If required, write the solution
       ...
       IF ( write_solution .AND.                                               &
        ...
        &       A, /, ' Objective:', ES24.16 )" ) pname, solv, inform%obj
         ...
         END DO
       END IF
       CALL APS_terminate( data, control, inform )
...
!  End of subroutine USE_APS

     END SUBROUTINE USE_APS

!  End of module USEAPS_double

   END MODULE GALAHAD_USEAPS_double
