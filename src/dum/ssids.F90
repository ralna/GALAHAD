...
! in datatypes.f90
   type SSIDS_options
     ...
     integer(long_) :: small_subtree_threshold = 4*10**6_long_
...
contains
 ...
 subroutine SSIDS_factor_precision(posdef, val, akeep, fkeep, options, inform, &
                                   scale, ptr, row)
 ...
 subroutine SSIDS_solve_one_precision(x1, akeep, fkeep, options, inform, job)
   real(rp_), dimension(:), intent(inout) :: x1
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), intent(inout) :: fkeep
   type(ssids_options), intent(in) :: options
   type(SSIDS_inform), intent(out) :: inform
