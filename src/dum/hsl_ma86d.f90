...
!$ use omp_lib
   ...
   interface MA86_solve
      module procedure MA86_solve_one_double, MA86_solve_mult_double
   end interface
...
subroutine MA86_factor_double(n, ptr, row, val, order, keep, control, info)
   USE GALAHAD_SYMBOLS
   integer, intent(in) :: n ! order of A
   integer, intent(in) :: row(:) ! row indices of lower triangular part
   integer, intent(in) :: ptr(:) ! col pointers for lower triangular part
...
subroutine MA86_factor_solve_one_double(n, ptr, row, val, order, keep, control,&
      info, x)
   USE GALAHAD_SYMBOLS
   integer, intent(in) :: n ! order of A
   integer, intent(in) :: row(:) ! row indices of lower triangular part
...
subroutine MA86_factor_solve_mult_double(n, ptr, row, val, order, keep, &
...
subroutine MA86_solve_mult_double(nrhs,lx,x,order,keep, control,info,job)
   USE GALAHAD_SYMBOLS
   integer, intent(in) :: nrhs ! number of right-hand sides to solver for.
   integer, intent(in) :: lx ! first dimension of x
   real(wp), intent(inout) :: x(lx,nrhs) ! On entry, x must
