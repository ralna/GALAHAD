...
!  use hsl_ma64_double
  ...
  type MA77_control
    ...
    real(dp_) :: small = tiny(one)
    real (dp_) :: static = zero
  ...
  type MA77_keep
    ...
    real(dp_), allocatable :: aelt(:)
    real(dp_), allocatable :: arow(:)
    ...
    real(dp_),allocatable :: rmain(:)
    real(dp_),allocatable :: rwork(:)
    real(dp_),allocatable :: rwdelay(:)
...
!****************************************************************************
  ...
  subroutine MA77_input_reals_double(index,length,reals,keep,control,info)
    ...
    real (dp_), intent (in) :: reals(length)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_control), intent (in) :: control
    type (MA77_info), intent (inout) :: info
  ...
  subroutine MA77_factor_double(pos_def,keep,control,info,scale)
    ...
    type (MA77_control), intent (in) :: control
    type (MA77_info), intent (inout) :: info
    real(dp_), intent(in), optional :: scale(:)
  ...
  subroutine MA77_factor_solve_double(pos_def,keep,control,info,nrhs, &
    ...
    integer(ip_) :: lx
    integer(ip_) :: nrhs
    real (dp_), intent(inout) :: x(lx,nrhs)
   ...
   subroutine MA77_resid_double(nrhs,lx,x,lresid,resid,keep,control,info,anorm)
    ...
    integer(ip_) :: lx
    integer(ip_) :: lresid
    real(dp_), intent(in) :: x(lx,nrhs)
    real(dp_), intent(inout) :: resid(lresid,nrhs)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_control), intent (in) :: control
    type (MA77_info), intent (inout) :: info
    real(dp_),optional :: anorm
  ...
  subroutine MA77_solve_fredholm_double( nrhs, flag_out, lx, x,                &
   ...
   USE GALAHAD_SYMBOLS
    ...
    integer(ip_), intent (in) :: lx
    real (dp_), intent (inout) :: x(lx,2*nrhs)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_control), intent (in) :: control
    type (MA77_info), intent (inout) :: info
    real(dp_), intent(in), optional :: scale(:)
  ...
  subroutine MA77_lmultiply_double(trans,k,lx,x,ly,y,keep,control,info,scale)
    ...
    real (dp_), intent (inout) :: x(lx,k) ! On entry, x must
    real (dp_), intent (out) :: y(ly,k) ! On exit,
    type (MA77_keep), intent (inout) :: keep
    type (MA77_control), intent (in) :: control
    type (MA77_info), intent (inout) :: info
    real(dp_), intent(in), optional :: scale(:)
...
!************************************************************************
  ...
  subroutine MA77_read_real(rfile,keep_array,loc,length,read_array, &
   ...
   real(dp_), allocatable :: keep_array(:)
   integer(long_), intent(in) :: loc
   integer(ip_), intent(in) :: length
   real(dp_), dimension(:), intent(inout) :: read_array
   integer(ip_) :: flag
  ...
  subroutine MA77_write_real(rfile,size_array,keep_array,loc,length, &
   ...
   integer(long_), intent(in) :: loc
   integer(ip_), intent(in) :: length
   real(dp_), intent(in) :: write_array(:)
   integer(ip_) :: flag
   type (of01_rdata), intent(inout) :: data
   integer(ip_), intent(in) :: lp
