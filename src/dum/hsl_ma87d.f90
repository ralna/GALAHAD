...
!$    integer(kind=omp_lock_kind) :: lock
   ...
   type MA87_control
      ...
      real(dp_) :: diag_zero_plus = 0.0
      real(dp_) :: diag_zero_minus = 0.0
   ...
   type MA87_info
      real(dp_) :: detlog = 0            ! Holds logarithm of abs det A (or 0)
...
subroutine MA87_sparse_fwd_solve_double(nbi,bindex,b,order,invp,               &
   ...
   real(dp_), intent(in) :: b(:) ! If bindex(i)=k, b(k) must hold the k-th
   ...
   real(dp_), intent(inout) :: x(:) ! If index(i)=k, x(k) holds the k-th
      ! nonzero component of solution; all other entries of x are set to zero.
   ! For details of keep, control, info : see derived type description
   real(dp_), intent(out) :: w(:) ! work array of size n at least n.
   type(MA87_keep), intent(inout) :: keep
   type(MA87_control), intent(in) :: control
