...
!$ use omp_lib
 ...
 ! Parameters
   ...
   interface SSIDS_analyse
      module procedure analyse_quadruple
   end interface SSIDS_analyse

   interface SSIDS_analyse_coord
      module procedure SSIDS_analyse_coord_quadruple
   end interface SSIDS_analyse_coord

   interface SSIDS_factor
      module procedure SSIDS_factor_quadruple
   end interface SSIDS_factor

   interface SSIDS_solve
      module procedure SSIDS_solve_one_quadruple
      module procedure SSIDS_solve_mult_quadruple
   end interface SSIDS_solve

   interface SSIDS_free
      module procedure free_akeep_quadruple
      module procedure free_fkeep_quadruple
      module procedure free_both_quadruple
   end interface SSIDS_free

   interface SSIDS_enquire_posdef
      module procedure SSIDS_enquire_posdef_quadruple
   end interface SSIDS_enquire_posdef

   interface SSIDS_enquire_indef
      module procedure SSIDS_enquire_indef_quadruple
   end interface SSIDS_enquire_indef

   interface SSIDS_alter
      module procedure SSIDS_alter_quadruple
...
! in akeep.f90
   type ssids_akeep
      ...
      real(qpc_), dimension(:), allocatable :: scaling
...
! in datatypes.f90
   type SSIDS_options
     ...
     real(qpc_) :: small = 1e-20_qpc_
     real(qpc_) :: u = 0.01
     integer(ip_) :: nstream = 1
     real(qpc_) :: multiplier = 1.1
...
contains
 ...
 subroutine analyse_quadruple(check, n, ptr, row, akeep, options, inform,      &
      order, val, topology)
   ...
   real(qpc_), optional, intent(in) :: val(:)
 ...
 end subroutine analyse_quadruple

 subroutine SSIDS_analyse_coord_quadruple(n, ne, row, col, akeep, options, &
 ...
 end subroutine SSIDS_analyse_coord_quadruple

 subroutine SSIDS_factor_quadruple(posdef, val, akeep, fkeep, options, inform, &
      scale, ptr, row)
   logical, intent(in) :: posdef
   real(qpc_), dimension(*), target, intent(in) :: val
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), intent(inout) :: fkeep
   type(ssids_options), intent(in) :: options
   type(SSIDS_inform), intent(out) :: inform
   real(qpc_), dimension(:), optional, intent(inout) :: scale
...
end subroutine SSIDS_factor_quadruple

 subroutine SSIDS_solve_one_quadruple(x1, akeep, fkeep, options, inform, job)
   real(qpc_), dimension(:), intent(inout) :: x1
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), intent(inout) :: fkeep
   type(ssids_options), intent(in) :: options
   type(SSIDS_inform), intent(out) :: inform
 ...
 end subroutine SSIDS_solve_one_quadruple

 subroutine SSIDS_solve_mult_quadruple(nrhs, x, ldx, akeep, fkeep, options, &
      inform, job)
   integer(ip_), intent(in) :: nrhs
   integer(ip_), intent(in) :: ldx
   real(qpc_), dimension(ldx,nrhs), intent(inout), target :: x
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), intent(inout) :: fkeep
   type(ssids_options), intent(in) :: options
   type(SSIDS_inform), intent(out) :: inform
 ...
 end subroutine SSIDS_solve_mult_quadruple

 subroutine SSIDS_enquire_posdef_quadruple(akeep, fkeep, options, inform, d)
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), target, intent(in) :: fkeep
   type(ssids_options), intent(in) :: options
   type(SSIDS_inform), intent(out) :: inform
   real(qpc_), dimension(*), intent(out) :: d
 ...
 end subroutine SSIDS_enquire_posdef_quadruple

 subroutine SSIDS_enquire_indef_quadruple(akeep, fkeep, options, inform, &
      piv_order, d)
   type(SSIDS_akeep), intent(in) :: akeep
   type(SSIDS_fkeep), target, intent(in) :: fkeep
   type(ssids_options), intent(in) :: options
   ...
   real(qpc_), dimension(2,*), optional, intent(out) :: d
 ...
 end subroutine SSIDS_enquire_indef_quadruple

 subroutine SSIDS_alter_quadruple(d, akeep, fkeep, options, inform)
   real(qpc_), dimension(2,*), intent(in) :: d
   type(SSIDS_akeep), intent(in) :: akeep
 ...
 end subroutine SSIDS_alter_quadruple

 subroutine free_akeep_quadruple(akeep, flag)
   type(SSIDS_akeep), intent(inout) :: akeep
   integer(ip_), intent(out) :: flag
   flag = GALAHAD_error_unknown_solver
 end subroutine free_akeep_quadruple

 subroutine free_fkeep_quadruple(fkeep, cuda_error)
   type(SSIDS_fkeep), intent(inout) :: fkeep
   integer(ip_), intent(out) :: cuda_error
   cuda_error = GALAHAD_error_unknown_solver
 end subroutine free_fkeep_quadruple

 subroutine free_both_quadruple(akeep, fkeep, cuda_error)
   type(SSIDS_akeep), intent(inout) :: akeep
   type(SSIDS_fkeep), intent(inout) :: fkeep
   integer(ip_), intent(out) :: cuda_error
   cuda_error = GALAHAD_error_unknown_solver
