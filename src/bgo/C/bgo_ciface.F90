...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_BGO_precision_ciface
    ...
    USE GALAHAD_LHS_precision_ciface, ONLY:                                    &
        lhs_inform_type,                                                       &
        lhs_control_type,                                                      &
...
!-------------------------------------------------
    ...
    TYPE, BIND( C ) :: bgo_control_type
      ...
      INTEGER ( KIND = ipc_ ) :: attempts_max
      INTEGER ( KIND = ipc_ ) :: max_evals
      INTEGER ( KIND = ipc_ ) :: sampling_strategy
      INTEGER ( KIND = ipc_ ) :: hypercube_discretization
...
!  non-reentrant (one active solve per process).
    ...
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_f = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_g = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_h = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_hprod = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_prec = C_NULL_FUNPTR
    TYPE ( C_FUNPTR ) :: bgo_saved_ceval_shprod = C_NULL_FUNPTR
    INTEGER ( KIND = ipc_ ) :: bgo_saved_n
    TYPE ( C_PTR ) :: bgo_saved_cuserdata
    INTEGER ( KIND = ipc_ ) :: bgo_saved_ne
    LOGICAL ( KIND = C_BOOL ) :: bgo_saved_f_indexing
  CONTAINS
...
!  call C interoperable eval_f
    ...
    PROCEDURE( eval_f ), POINTER :: bgo_saved_eval_f
    CALL C_F_PROCPOINTER( bgo_saved_ceval_f, bgo_saved_eval_f )
    status = bgo_saved_eval_f( bgo_saved_n, x, f, bgo_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_g
    PROCEDURE( eval_g ), POINTER :: bgo_saved_eval_g
    CALL C_F_PROCPOINTER( bgo_saved_ceval_g, bgo_saved_eval_g )
    status = bgo_saved_eval_g( bgo_saved_n, x, g, bgo_saved_cuserdata )
    RETURN
...
!  Call C interoperable eval_h
    PROCEDURE( eval_h ), POINTER :: bgo_saved_eval_h
    CALL C_F_PROCPOINTER( bgo_saved_ceval_h, bgo_saved_eval_h )
    status = bgo_saved_eval_h( bgo_saved_n, bgo_saved_ne, x, hval, bgo_saved_cuserdata )
    RETURN
...
! Call C interoperable eval_hprod
    ...
    PROCEDURE( eval_hprod ), POINTER :: bgo_saved_eval_hprod
    CALL C_F_PROCPOINTER( bgo_saved_ceval_hprod, bgo_saved_eval_hprod )
    IF ( PRESENT( fgot_h ) ) THEN
      cgot_h = fgot_h
    ELSE
      cgot_h = .FALSE.
    END IF
    status = bgo_saved_eval_hprod( bgo_saved_n, x, u, v, cgot_h, bgo_saved_cuserdata )
    RETURN
...
!  call C interoperable eval_prec
    ...
    PROCEDURE( eval_prec ), POINTER :: bgo_saved_eval_prec
    CALL C_F_PROCPOINTER( bgo_saved_ceval_prec, bgo_saved_eval_prec )
    status = bgo_saved_eval_prec( bgo_saved_n, x, u, v, bgo_saved_cuserdata )
    RETURN
...
!  call C interoperable eval_shprod
    ...
    PROCEDURE( eval_shprod ), POINTER :: bgo_saved_eval_shprod
    CALL C_F_PROCPOINTER( bgo_saved_ceval_shprod, bgo_saved_eval_shprod )
    IF ( PRESENT( fgot_h ) ) THEN
      cgot_h = fgot_h
    ELSE
      cgot_h = .false.
    ...
    IF ( bgo_saved_f_indexing ) then
      status = bgo_saved_eval_shprod( bgo_saved_n, x, nnz_v, index_nz_v, v, nnz_u, index_nz_u,    &
                             u, cgot_h, bgo_saved_cuserdata )
    ELSE ! handle C sparse matrix indexing
      status = bgo_saved_eval_shprod(bgo_saved_n, x, nnz_v, index_nz_v - 1, v, nnz_u, index_nz_u, &
                             u, cgot_h, bgo_saved_cuserdata )
  ...
  END MODULE GALAHAD_BGO_precision_ciface
...
!  -------------------------------------
  ...
  USE GALAHAD_BGO_precision_ciface
  IMPLICIT NONE
...
!  associate data pointer
  ...
  bgo_saved_n = n
  bgo_saved_cuserdata = cuserdata
  bgo_saved_ne = ne

!  associate procedure pointers

  bgo_saved_ceval_f = ceval_f
  bgo_saved_ceval_g = ceval_g
  bgo_saved_ceval_h = ceval_h
  IF ( C_ASSOCIATED( ceval_hprod ) ) THEN
    bgo_saved_ceval_hprod = ceval_hprod
  ELSE
    bgo_saved_ceval_hprod = C_NULL_FUNPTR
  END IF
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    bgo_saved_ceval_prec = ceval_prec
  ELSE
    bgo_saved_ceval_prec = C_NULL_FUNPTR
  END IF

  IF ( C_ASSOCIATED( bgo_saved_ceval_prec ) .AND. C_ASSOCIATED( bgo_saved_ceval_hprod ) ) THEN
    CALL f_bgo_solve_with_mat( fdata, fuserdata, status, x, g,                 &
                               wrap_eval_f, wrap_eval_g, wrap_eval_h,          &
                               eval_HPROD = wrap_eval_hprod,                   &
                               eval_PREC = wrap_eval_prec )
  ELSE IF ( C_ASSOCIATED( bgo_saved_ceval_prec ) ) THEN
    CALL f_bgo_solve_with_mat( fdata, fuserdata, status, x, g,                 &
                               wrap_eval_f, wrap_eval_g, wrap_eval_h,          &
                               eval_PREC = wrap_eval_prec )
  ELSE IF ( C_ASSOCIATED( bgo_saved_ceval_hprod ) ) THEN
    CALL f_bgo_solve_with_mat( fdata, fuserdata, status, x, g,                 &
                               wrap_eval_f, wrap_eval_g, wrap_eval_h,          &
                               eval_HPROD = wrap_eval_hprod )
  ELSE
...
!  associate procedure pointers
  ...
  bgo_saved_ceval_g = ceval_g
  bgo_saved_ceval_hprod = ceval_hprod
  bgo_saved_ceval_shprod = ceval_shprod
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    bgo_saved_ceval_prec = ceval_prec
  ELSE
    bgo_saved_ceval_prec = C_NULL_FUNPTR
  ...
  bgo_saved_n = n
  bgo_saved_cuserdata = cuserdata
  bgo_saved_f_indexing = f_indexing

!  solve the problem when the Hessian is only available via products

  IF ( C_ASSOCIATED( bgo_saved_ceval_prec ) ) THEN
    CALL f_bgo_solve_without_mat( fdata, fuserdata, status, x, g,              &
