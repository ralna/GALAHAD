...
#include "galahad_modules.h"
   ...
   TYPE ( BGO_full_data_type ) :: data
   TYPE ( GALAHAD_userdata_type ) :: userdata
!  EXTERNAL :: FUN, GRAD, HESS, HESSPROD, PREC
   INTEGER ( KIND = ip_ ) :: n, ne, nnz_v, nnz_u
   INTEGER ( KIND = ip_ ) :: i, s, status, data_storage_type, eval_status
   LOGICAL :: alive
...
!    control%ugo_control%print_level = 1
 ...
 !   control%random_multistart = .TRUE.
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       st = 'C'
       CALL BGO_import( control, data, status, n, X_l, X_u,                    &
                        'coordinate', ne, H_row, H_col, H_ptr )
       CALL BGO_solve_with_mat( data, userdata, status, X, G,                  &
     ...
     CASE ( 4 ) ! diagonal
       ...
       CALL BGO_solve_with_mat( data, userdata, status, X, G, FUN_diag,        &
                                GRAD_diag, HESS_diag, HESSPROD_diag, PREC )
     CASE ( 5 ) ! access by products
       st = 'P'
       CALL BGO_import( control, data, status, n, X_l, X_u,                    &
                        'absent', ne, H_row, H_col, H_ptr )
       CALL BGO_solve_without_mat( data, userdata, status, X, G,               &
                                   FUN, GRAD, HESSPROD, SHESSPROD, PREC )
     END SELECT
     CALL BGO_information( data, inform, status )
     ...
     ELSE
       WRITE( 6, "( A1, ': BGO_solve exit status = ', I0 ) " ) st, inform%status
     END IF
!    WRITE( 6, "( ' X ', 3ES12.5 )" ) X
!    WRITE( 6, "( ' G ', 3ES12.5 )" ) G
...
!    control%random_multistart = .TRUE.
     ...
     CASE ( 1 ) ! sparse co-ordinate storage
       ...
       DO ! reverse-communication loop
         CALL BGO_solve_reverse_with_mat( data, status, eval_status,           &
                                          X, f, G, H_val, U, V )
         SELECT CASE ( status )
     ...
     CASE ( 5 ) ! access by products
       ...
       CALL BGO_import( control, data, status, n, X_l, X_u,                    &
                        'absent', ne, H_row, H_col, H_ptr )
       DO ! reverse-communication loop
         CALL BGO_solve_reverse_without_mat( data, status, eval_status,        &
...
CONTAINS
   ...
   END PROGRAM GALAHAD_BGO_interface_test
