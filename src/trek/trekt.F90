...
#include "galahad_modules.h"
   PROGRAM GALAHAD_TREK_test_program
   USE GALAHAD_KINDS_precision, ONLY: rp_, ip_
   USE GALAHAD_TREK_precision, ONLY: SMT_type, SMT_put, TREK_control_type,     &
   ...
   INTEGER ( KIND = ip_ ) :: id, ir, is, data_storage_type
...
!    control%trs_control%error = 23 ; control%trs_control%out = 23
...
!  Iteration to find the minimizer
     ...
     CALL TREK_solve( nn, H, C, radius, X, data, control, inform, S = S )

     WRITE( 6, "( ' pass  ', I3, ': TREK_solve exit status = ', I6 )" )        &
            pass, inform%status
...
!      DO id = 0, 0 ! new data?
         ...
         DO ir = 1, 2 ! resolve with smaller radius?
           control%new_radius = ir == 2
!        WRITE( 6, "( ' format ', A1, I1, A1, ':' )" ) st, ir, id, sa
!write(6,*) 'ir = ', ir, 'id = ', id, 'is = ', is, H%type, ' ', S%type
           IF ( control%new_radius ) THEN
        ...
        &    ' TREK_solve exit status = ', I4 )" ) st, ir, id, sa, inform%status
!stop
         END DO
       END DO
     END DO
...
!  ==============
   ...
   DO pass = 1, 7
     ...
     if(.false.)then
       control%error = 6 ; control%out = 6 ; control%print_level = 1
       control%trs_control%error = 6 ; control%trs_control%out = 6
       control%trs_control%print_level = 1
       control%stop_check_all_orders = .TRUE.
     endif
     CALL TREK_solve( n, H, C, radius, X, data, control, inform, S = S )
...
#include "galahad_sls_defaults_dls.h"
   ...
   END PROGRAM GALAHAD_TREK_test_program
