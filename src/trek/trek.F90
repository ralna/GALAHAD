...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
   ...
   MODULE GALAHAD_TREK_precision
     ...
     USE GALAHAD_MOP_precision, ONLY: TREK_Mv => MOP_Ax
...
!----------------------
     ...
     INTERFACE TREK_initialize
       MODULE PROCEDURE TREK_initialize, TREK_full_initialize
     END INTERFACE TREK_initialize

     INTERFACE TREK_terminate
       MODULE PROCEDURE TREK_terminate, TREK_full_terminate
     END INTERFACE TREK_terminate
...
!  resolve a previously solved problem with a smaller radius

       LOGICAL :: new_radius = .FALSE.
...
!  ...................
      ...
      TYPE, PUBLIC :: TREK_data_type
        ...
        REAL ( KIND = rp_ ) :: c_norm, last_radius, last_shift, shift_val, ztu
...
!  initialize TRS components

      control%TRS_control = trs_control
      control%TRS_control%print_level = - 1
      control%TRS_control%prefix = '" - TRS:"                     '
      inform%TRS_inform = trs_inform
...
!  The default values as given by TREK_initialize could (roughly)
!  have been set as:
...
!  Local variables
      ...
      INTEGER ( KIND = ip_ ), PARAMETER :: reduction = f + 1
      ...
      INTEGER ( KIND = ip_ ), PARAMETER :: stop_check_all_orders               &
                                             = perturb_c + 1
      INTEGER ( KIND = ip_ ), PARAMETER :: new_radius                          &
                                             = stop_check_all_orders + 1
      INTEGER ( KIND = ip_ ), PARAMETER :: new_values = new_radius + 1
...
!  Logical key-words
      ...
      spec( new_radius )%keyword = 'new-radius'
...
!  Set logical values
     ...
     CALL SPECFILE_assign_value( spec( stop_check_all_orders ),                &
                                 ...
                                 control%error )
     CALL SPECFILE_assign_value( spec( new_radius ),                           &
                                 control%new_radius,                           &
                                 control%error )
...
!    REAL ( KIND = rp_ ) :: e11, e12, e22
     REAL ( KIND = rp_ ) :: violation, shift, error_r, s_norm
     ...
     LOGICAL :: initial, restart, new_vals, shifted_structure, interior, s_ok
...
!  see if this is an initial- or a re-entry
      ...
      initial = .NOT. control%new_radius
...
!  check initial radius value is positive
     ...
     IF ( new_vals ) THEN
       IF ( radius <= zero ) THEN
         IF ( printe ) WRITE( error, "( A, ' non-positive radius provided' )" )&
           prefix
         inform%status = GALAHAD_error_restrictions ; GO TO 920
       END IF
...
!  on reentry, also check the new radius value is smaller than the previous one
     ...
     ELSE
       IF ( radius <= zero .OR. radius >= data%last_radius ) THEN
         IF ( printe ) WRITE( error, "( A, ' inappropriate radius provided' )")&
           prefix
         inform%status = GALAHAD_error_restrictions ; GO TO 920
       END IF
...
!  record the iteration limit
       ...
       IF ( data%control%TRS_control%print_level < 0 )                         &
         data%control%TRS_control%print_level = control%print_level - 1
...
!  if necessary, compute the Gershgorin shift, shift_val
       ...
       IF ( data%shifted ) THEN
         CALL CPU_time( time_record ) ; CALL CLOCK_time( clock_record )
         IF ( data%unit_s ) THEN
           CALL TREK_find_shift( H, data%shift_val, data%U, shift_status )
         ELSE
           CALL TREK_find_shift( H, data%shift_val, data%U, shift_status,      &
...
!  compute u = S x (if necessary) and ||x||_S
       ...
       IF ( data%unit_s ) THEN
         x_norm = TWO_NORM( X )
       ELSE
         CALL TREK_Mv( one, S, X, zero, data%U, out, error,                    &
                       symmetric = .TRUE. )
         x_norm = SQRT( DOT_PRODUCT( data%U, X ) )
       END IF

!  if the Newton step is inside the trust region, record the step, and prepare
!  to exit

       interior = x_norm <= radius .AND. .NOT. data%shifted
       IF ( interior ) THEN
         data%k_exit = 1
         data%last_shift = zero
         IF ( data%perturb_c ) THEN
           inform%obj = half * DOT_PRODUCT( X, data%C_pert ) + control%f
         ELSE
           inform%obj = half * DOT_PRODUCT( X, C ) + control%f
         END IF
         inform%multiplier = zero
         inform%error = zero
         inform%n_vec = 0

!  compute the next potential radius so that ||x|| > next_radius

         inform%next_radius = radius
         DO
           inform%next_radius = inform%next_radius * data%control%reduction
           IF ( x_norm > inform%next_radius ) EXIT
         END DO
       ELSE
         inform%next_radius = radius * data%control%reduction
       END IF
...
!  set w = S u
         ...
         CALL TREK_Mv( one, S, data%U, zero, data%W, out, error,               &
                       symmetric = .TRUE. )
...
!  if the Newton step is inside the trust region, exit
       ...
       IF ( interior ) THEN
         IF ( printd ) WRITE( out, "( A, ' *** exit segment 0' )" ) prefix
...
!  set u = H v_{-k}
             ...
             IF ( data%shifted ) THEN
               CALL TREK_Mv( one, data%H_shift, data%V( 1 : n, - k ), zero,    &
                             data%U, out, error, symmetric = .TRUE. )
             ELSE
               CALL TREK_Mv( one, H, data%V( 1 : n, - k ), zero,               &
                             data%U, out, error, symmetric = .TRUE. )
             END IF
...
!    min 1/2 x'^T D x' + c'^T x' : ||x'|| <= radius
           ...
           data%control%TRS_control%equality_problem = data%shifted
           CALL TRS_solve_diagonal( k2m1, radius, control%f,                   &
...
!  if the solution is interior but we have shifted H, make sure that
!  we do not exit

           IF ( ABS( violation ) >= small * radius .AND. data%shifted )        &
             error_r = MAX( one, data%c_norm )
...
!          IF ( .TRUE. ) THEN
           IF ( printd ) THEN
             ...
             IF ( data%unit_s ) THEN
               R = R + shift * SOL
             ELSE
               CALL TREK_Mv( shift, S, SOL, one, R, out, error,                &
                             symmetric = .TRUE. )
             END IF
             CALL TREK_Mv( one, H, SOL, one, R, out, error, symmetric = .TRUE. )
           ...
           IF ( printi ) WRITE( out, 2010 ) prefix, k, k2m1,                   &
                s_norm, radius, shift, error_r, inform%obj

!  check for termination

           error_r = error_r / MAX( one, data%c_norm )
           IF ( error_r < data%control%stop_residual .AND.                     &
...
!  orthogonalise u wrt the remaining vectors if desired
           ...
           IF ( control%reorthogonalize ) THEN
             ...
             ELSE
               DO i = mkp1, km1 - 1
                 CALL TREK_Mv( one, S, data%V( 1 : n, i ), zero, data%Z,       &
                               out, error, symmetric = .TRUE. )
                 beta = DOT_PRODUCT( data%Z, data%U )
                 data%U = data%U - beta * data%V( 1 : n, i )
               END DO
               CALL TREK_Mv( one, S, data%U, zero, data%Z,                     &
                             out, error, symmetric = .TRUE. )
           ...
           IF ( printd ) THEN
             IF ( data%unit_s ) THEN
               WRITE( out, "( ' p ', 3ES22.14 )" )                             &
                 ...
                 DOT_PRODUCT( data%V( 1 : n, k ), data%V( 1 : n, k ) )
             ELSE
               CALL TREK_Mv( one, S, data%V( 1 : n, k ), zero, data%Z,         &
                             out, error, symmetric = .TRUE. )
               WRITE( out, "( ' p ', 3ES22.14 )" )                             &
...
!    min 1/2 x'^T D x' + c'^T x' : ||x'|| <= radius
           ...
           data%control%TRS_control%equality_problem = data%shifted
           data%control%trs_control%initial_multiplier = shift
           CALL TRS_solve_diagonal( k2, radius, control%f,                     &
                                    data%C, data%D, data%X,                    &
                                    data%control%TRS_control,                  &
                                    inform%TRS_inform )
           shift = inform%TRS_inform%multiplier
...
!          IF ( .TRUE. ) THEN
           ...
           IF ( printh ) WRITE( out, 2000 ) prefix
           IF ( printi ) WRITE( out, 2010 ) prefix, k, k2, s_norm, radius,     &
...
!   set z = S v_{-m-1}
               ...
               ELSE
                 data%V( 1 : n, mkm1 ) = data%U / data%delta_mk
                 CALL TREK_Mv( one, S, data%V( 1 : n, mkm1 ), zero, data%Z,    &
                               out, error, symmetric = .TRUE. )
                 WRITE( out, "( ' p ', 3ES22.14 )" )                           &
                   DOT_PRODUCT( data%Z, data%V( 1 : n, - k ) ),                &
                   DOT_PRODUCT( data%Z, data%V( 1 : n, k ) ),                  &
...
!      DO ii = 1, inform%n_vec
       DO ii = 1, p_dim
         ...
         END IF
         IF ( data%shifted ) THEN
           CALL TREK_Mv( one, data%H_shift, data%V( 1 : n, i ), zero,          &
                         data%U, out, error, symmetric = .TRUE. )
         ELSE
           CALL TREK_Mv( one, H, data%V( 1 : n, i ), zero,                     &
                         data%U, out, error, symmetric = .TRUE. )
         END IF
         DO jj = 1, ii
           IF ( jj == 1 ) THEN
...
2000 FORMAT( A, '    k d(K)   ||x||     radius      shift      error   ',      &
             '         f')
...
!-*-*-*-*-   T R E K _ H _ f i n d _ s h i f t   S U B R O U T I N E   -*-*-*-*-
     ...
     SUBROUTINE TREK_find_shift( H, shift, H_low, status, S, S_diag, S_offd )
...
!  Deallocate all remaing allocated arrays
      ...
      array_name = 'trek: data%H_shift%ptr'
      ...
      array_name = 'trek: data%H_shift%col'
      ...
      array_name = 'trek: data%H_shift%val'
...
!-*-*-*-  G A L A H A D -  T R E K _ i m p o r t _ S U B R O U T I N E -*-*-*-*-

     SUBROUTINE TREK_import( control, data, status, n,                         &
...
!   comments to TREK_solve
!
!  data is a scalar variable of type TREK_full_data_type used for internal data
!
...
!  copy control to data
     ...
     data%trek_control = control

     error = data%trek_control%error
     space_critical = data%trek_control%space_critical
     deallocate_error_fatal = data%trek_control%space_critical
...
!  set H appropriately in its storage type
     ...
     CASE ( 'coordinate', 'COORDINATE' )
       IF ( .NOT. ( PRESENT( H_row ) .AND. PRESENT( H_col ) ) ) THEN
         data%trek_inform%status = GALAHAD_error_optional
         GO TO 900
       END IF
       CALL SMT_put( data%H%type, 'COORDINATE', data%trek_inform%alloc_status )
       data%H%n = n ; data%H%ne = H_ne

       array_name = 'trek: data%H%row'
       CALL SPACE_resize_array( data%H%ne, data%H%row,                         &
              data%trek_inform%status, data%trek_inform%alloc_status,          &
              array_name = array_name,                                         &
              deallocate_error_fatal = deallocate_error_fatal,                 &
              exact_size = space_critical,                                     &
              bad_alloc = data%trek_inform%bad_alloc, out = error )
       IF ( data%trek_inform%status /= 0 ) GO TO 900
     ...
     CASE ( 'dense', 'DENSE' )
       CALL SMT_put( data%H%type, 'DENSE', data%trek_inform%alloc_status )
     ...
     CASE ( 'diagonal', 'DIAGONAL' )
       CALL SMT_put( data%H%type, 'DIAGONAL', data%trek_inform%alloc_status )
     ...
     CASE DEFAULT
       data%trek_inform%status = GALAHAD_error_unknown_storage
       GO TO 900
     END SELECT
...
!  copy control to data
     ...
     WRITE( data%trek_control%out, "('')", ADVANCE = 'no' ) ! prevents ifort bug
     error = data%trek_control%error
     space_critical = data%trek_control%space_critical
     deallocate_error_fatal = data%trek_control%space_critical
...
!  set S appropriately in its storage type
     ...
     CASE ( 'coordinate', 'COORDINATE' )
       IF ( .NOT. ( PRESENT( S_ne ) .AND. PRESENT( S_row ) .AND.               &
         ...
         GO TO 900
       END IF
       CALL SMT_put( data%S%type, 'COORDINATE', data%trek_inform%alloc_status )
       data%S%n = n ; data%S%ne = S_ne

       array_name = 'trek: data%S%row'
     ...
     CASE ( 'dense', 'DENSE' )
       CALL SMT_put( data%S%type, 'DENSE', data%trek_inform%alloc_status )
     ...
     CASE ( 'diagonal', 'DIAGONAL' )
       CALL SMT_put( data%S%type, 'DIAGONAL', data%trek_inform%alloc_status )
     ...
     CASE ( 'identity', 'IDENTITY' )
       CALL SMT_put( data%S%type, 'IDENTITY', data%trek_inform%alloc_status )
...
!-  G A L A H A D -  T R E K _ r e s e t _ c o n t r o l   S U B R O U T I N E -
     ...
     SUBROUTINE TREK_reset_control( control, data, status )

!  reset control parameters after import if required.
!  See TREK_solve for a description of the required arguments
...
!  end of subroutine TREK_reset_control

     END SUBROUTINE TREK_reset_control

!-  G A L A H A D -  T R E K _ s o l v e _ p r o b l e m  S U B R O U T I N E  -

     SUBROUTINE TREK_solve_problem( data, status, H_val, C, radius, X, S_val )

!  solve the trust-region problem whose structure was previously
!  imported. See TREK_solve for a description of the required arguments.
...
!       status and a string containing the name of the offending array
!       are held in inform.alloc_status and inform.bad_alloc respectively.
!   -3. The restriction radius > 0, or requirement that the types contain a 
!       relevant string 'DENSE', 'COORDINATE', 'SPARSE_BY_ROWS',
!       'DIAGONAL' or 'IDENTITY' has been violated.
!
...
!   the Hessian matrix H in the storage scheme specified in trek_import.
!
...
!   the scaling matrix S in the storage scheme specified in trek_import. This
!   need not be given if S is the identity matrix
!
!  resolve is an optional scalar of type default logical that should be
!   .true. if the previous problem is to be resolved with a smaller radius
!
...
!  call the solver
     ...
     IF ( .NOT. data%use_s ) THEN
       CALL TREK_solve( n, data%H, C, radius, X, data%trek_data,               &
                        data%trek_control, data%trek_inform )
     ELSE
       ...
       CALL TREK_solve( n, data%H, C, radius, X, data%trek_data,               &
                        data%trek_control, data%trek_inform, S = data%S )
     END IF
...
!  End of subroutine TREK_solve_problem

     END SUBROUTINE TREK_solve_problem

!-  G A L A H A D -  T R E K _ i n f o r m a t i o n   S U B R O U T I N E  -

     SUBROUTINE TREK_information( data, inform, status )

!  return solver information during or after solution by TREK
!  See TREK_solve for a description of the required arguments
...
!  end of subroutine TREK_information
