...
C 20th December 2005 Version 1.1.0. REAL variables in MA61CD changed
C      to DOUBLE to avoid warnings of accuracy loss.
   ...
   10 CONTINUE
      RETURN
      END
      SUBROUTINE MA61AD(N,NZ,A,INI,INJ,IAI,IAJ,IK,IW,W,C,ICNTL,CNTL,
     +                  INFO,KEEP)
      DOUBLE PRECISION ZERO,ONE
      PARAMETER (ZERO=0.0D0,ONE=1.0D0)
      DOUBLE PRECISION C
      INTEGER IAI,IAJ,N,NZ
      DOUBLE PRECISION A(IAJ),W(N,3)
 ...
 9000    FORMAT (/,/,' Warning: more than one diagonal entry in row',I5)
   ...
   70 CONTINUE
      CALL MA61ED(INI,INJ,NZ0,IW,N,A)
      KK = 0
      DO 120 IR = 1,N
         KPP = IW(IR,1)
         IW(IR,1) = KK + 1
 ...
 9010       FORMAT (/,/,' Warning: there is more than one entry in row',
  ...
  180 CONTINUE
      ...
      KEEP(6) = NCP
      KEEP(8) = IPD
      CALL MA61CD(N,NZ0,W(1,2),A(NZP1),INI,INJ(NZP1),IAI,IAJNZ,IK,IW,
 ...
 9020 FORMAT (/,/,' Error return from MA61AD because',
     +    ' N is out of range.')
      IFLAG = -1
      GO TO 280
  230 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9030)
 9030 FORMAT (/,/,' Error return from MA61AD because',
     +   ' NZ is out of range.')
      IFLAG = -2
      GO TO 280
  240 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9040)
 9040 FORMAT (/,/,' Error return from MA61AD because',
     +   ' IAI is out of range.')
      IFLAG = -3
      GO TO 280
  250 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9050)
 9050 FORMAT (/,/,' Error return from MA61AD because',
     +   ' IAJ is out of range.')
      IFLAG = -4
      GO TO 280
  260 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9060) K,I,J
 9060 FORMAT (/,/,' Error return from MA61AD because',
     +   ' element',I7,' is in row',I5,' and column',I5)
      IFLAG = -5
      GO TO 280
  270 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9070) I
 9070 FORMAT (/,/,' Error return from MA61AD because diagonal element',
  ...
  310 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE MA61CD(N,NZ,D,A,INI,INJ,IAI,IAJ,IK,IP,IW,W,IFLAG,C,
     +                  ICNTL,CNTL,KEEP)
      DOUBLE PRECISION ZERO,ONE
      PARAMETER (ZERO=0.0D0,ONE=1.0D0)
      DOUBLE PRECISION EPSTOL,U,CMAX
      PARAMETER (EPSTOL=5.0D-16,U=1.0D2,CMAX=1.0D20)
      DOUBLE PRECISION C
      INTEGER IAI,IAJ,IFLAG,N,NZ
      DOUBLE PRECISION A(IAJ),D(N),W(N)
      INTEGER IK(N,3),INI(IAI),INJ(IAJ),IP(N,2),IW(N,2)
      DOUBLE PRECISION CNTL(3)
      INTEGER ICNTL(5),KEEP(12)
      DOUBLE PRECISION AA,AL,C0
      DOUBLE PRECISION ALFA,B1,B2,DJP,LFULDD,LFULL,PFILL,PIVT
     ...
     +        NZI
      ...
      LOGICAL CHANGE
      EXTERNAL MA61DD
      INTRINSIC ABS,MAX,INT,SQRT,DBLE
      NURL = KEEP(1)
      NUCL = KEEP(2)
      NUAL = KEEP(3)
      LROW = KEEP(4)
  ...
  110    CONTINUE
         ...
         CALL MA61DD(A,INJ,IAJ,N,IK,IP,.TRUE.,NCP,NUCL,NUAL)
  120    CONTINUE
         KP = IP(JP,1)
         KL = IK(JP,1) + KP - 1
         IP(JP,1) = NURL + 1
 ...
 9000    FORMAT (/,/,'  Warning: modification of zero or negative',
  ...
  410          CONTINUE
               ...
               CALL MA61DD(A,INI,IAI,N,IK(1,2),IP(1,2),.FALSE.,NCP,NUCL,
     +                     NUAL)
               K = IP(J,2)
               KL1 = K + NZ - 1
  420          CONTINUE
 ...
 9010    FORMAT (/,/,' Warning: available space used at pivot step',I7)
  500    CONTINUE
         ...
         PFILL = DBLE(LROW-NZ0)/DBLE(NFILL)
  ...
  740 CONTINUE
   ...
   50 CONTINUE
      RETURN
      END
      SUBROUTINE MA61ED(IN1,IN2,NZ,IP,N,A)
      INTEGER N,NZ
   ...
   40 CONTINUE
      RETURN
      END
      SUBROUTINE MA61GD(N,A,INJ,IAJ,D,IK,B,LROW)
      INTEGER IAJ,N,LROW
   ...
   60 CONTINUE
      RETURN
      END
      SUBROUTINE MA61BD(N,NZ,A,INI,INJ,IAJ,IK,B,W,W1,KMAX,EPS,ICNTL,
     +                  INFO,KEEP)
      INTEGER IAJ,N,NZ
      DOUBLE PRECISION A(*),B(*),EPS(2),W(N,3),W1(N,3)
      INTEGER IK(N,3),INI(*),INJ(*),KMAX(2)
      INTEGER ICNTL(5),INFO(10),KEEP(12)
      INTEGER I,IFJ,NZ0,NZP1,IFLAG,LROW,ND
      EXTERNAL MA61FD
      LROW = KEEP(4)
      ND = KEEP(7)
      IFLAG = KEEP(9)
      ...
      DO 10 I = 1,N
         B(I) = B(I)*W(I,1)
   10 CONTINUE
      CALL MA61FD(N,NZ0,A,INI,INJ,IFJ,A(NZP1),W(1,2),INJ(NZP1),IK,B,
 ...
 9010 FORMAT (' Error return from MA61BD because',
     +    ' N is out of range.')
      IFLAG = -1
      GO TO 70
   40 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9020)
 9020 FORMAT (' Error return from MA61BD because',
     +   ' NZ is out of range.')
      IFLAG = -2
      GO TO 70
   50 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9030)
 9030 FORMAT (' Error return from MA61BD because',
     +    ' IAJ is out of range.')
      IFLAG = -4
      GO TO 70
   60 CONTINUE
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=9040)
 9040 FORMAT (' Error return from MA61BD because',
     +    ' of error in previous call to MA61AD.')
   70 CONTINUE
   80 CONTINUE
      INFO(1) = IFLAG
      RETURN
      END
      SUBROUTINE MA61FD(N,NZ,A,INI,INJ,IAF,AF,DF,INJF,IK,B,R,E,F,G,KMAX,
     +                  EPS,LROW)
      DOUBLE PRECISION ZERO
      PARAMETER (ZERO=0.0D0)
      DOUBLE PRECISION EPS
      INTEGER IAF,KMAX,N,NZ,LROW
      DOUBLE PRECISION A(*),AF(*),B(*),DF(*),E(*),F(*),G(*),R(*)
      INTEGER IK(N,2),INI(*),INJ(*),INJF(*)
      DOUBLE PRECISION BB,D0,D1,L,R1
      INTEGER I,KITR
      EXTERNAL MA61GD,MA61HD
      INTRINSIC SQRT
      D0 = ZERO
      EPS = EPS**2
      DO 10 I = 1,N
         E(I) = B(I)
   10 CONTINUE
      CALL MA61GD(N,AF,INJF,IAF,DF,IK,E,LROW)
      CALL MA61HD(A,INI,INJ,NZ,N,E,R)
      R1 = ZERO
      DO 20 I = 1,N
         R(I) = R(I) - B(I)
         R1 = R1 + R(I)**2
   ...
   20 CONTINUE
      ...
      CALL MA61GD(N,AF,INJF,IAF,DF,IK,G,LROW)
      DO 30 I = 1,N
         E(I) = -G(I)
         D0 = D0 + R(I)*G(I)
   30 CONTINUE
   40 CONTINUE
      KITR = KITR + 1
      CALL MA61HD(A,INI,INJ,NZ,N,E,F)
      L = ZERO
      DO 50 I = 1,N
         L = L + E(I)*F(I)
   50 CONTINUE
   ...
   90 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE MA61HD(A,INI,INJ,NZ,N,B,Z)
      INTEGER N,NZ
