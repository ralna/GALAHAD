...
C  19/3/03. Array ICNTL in MA27G made assumed size.
C  17/9/09. Bug corrected in MA27H.  Also some tidying of code MA27H

      SUBROUTINE MA27I(ICNTL,CNTL)
      INTEGER, INTENT( OUT ) :: ICNTL(30)
      REAL, INTENT( OUT ) :: CNTL(5)
      INTEGER IFRLVL
      PARAMETER ( IFRLVL=5 )
      ICNTL(1) = 6
      ICNTL(2) = 6
      ...
      RETURN
      END
      SUBROUTINE MA27A(N,NZ,IRN,ICN,IW,LIW,IKEEP,IW1,NSTEPS,IFLAG,
     +                 ICNTL,CNTL,INFO,OPS)
      ...
      EXTERNAL MA27G,MA27H,MA27J,MA27K,MA27L,MA27M,MA27U
      INTRINSIC MIN
      DO 5 I = 1,15
        INFO(I) = 0
    5 CONTINUE
   ...
   10 FORMAT(/,/,' ENTERING MA27A  WITH      N     NZ      LIW  IFLAG',
     +       /,21X,I7,I7,I9,I7)
      NSTEPS = 0
      K = MIN(8,NZ)
   ...
   40 IF (N.LT.1 .OR. N.GT.ICNTL(4)) GO TO 70
      ...
      CALL MA27G(N,NZ,IRN,ICN,IW,LLIW,IW1,IW1(1,2),IW(L1),IWFR,
     +           ICNTL,INFO)
      CALL MA27H(N,IW1,IW,LLIW,IWFR,IW(L1),IW(L2),IKEEP(1,2),IKEEP(1,3),
     +           IKEEP,ICNTL(4),INFO(11),CNTL(2))
      GO TO 60
   50 IF (LIW.LT.NZ+3*N+1) GO TO 120
      CALL MA27J(N,NZ,IRN,ICN,IKEEP,IW,LLIW,IW1,IW1(1,2),IW(L1),IWFR,
     +           ICNTL,INFO)
      CALL MA27K(N,IW1,IW,LLIW,IWFR,IKEEP,IKEEP(1,2),IW(L1),IW(L2),
     +           INFO(11))
   60 CALL MA27L(N,IW1,IW(L1),IKEEP,IKEEP(1,2),IKEEP(1,3),IW(L2),NSTEPS,
     +           ICNTL(5))
      IF(NZ.GE.1) IW(1) = IRN(1) + 1
      CALL MA27M(N,NZ,IRN,ICN,IKEEP,IKEEP(1,3),IKEEP(1,2),IW(L2),NSTEPS,
     +           IW1,IW1(1,2),IW,INFO,OPS)
      GO TO 160
   70 INFO(1) = -1
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=80) INFO(1)
   80 FORMAT (' **** ERROR RETURN FROM MA27A  **** INFO(1)=',I3)
  ...
  170 FORMAT (/,' LEAVING MA27A  WITH NSTEPS  INFO(1)    OPS IERROR',
     +          ' NRLTOT NIRTOT',
     +        /,20X,2I7,F7.0,3I7,
  ...
  200 CONTINUE
      RETURN
      END
      SUBROUTINE MA27B(N,NZ,IRN,ICN,A,LA,IW,LIW,IKEEP,NSTEPS,MAXFRT,IW1,
     +                 ICNTL,CNTL,INFO)
   ...
   60 IF (N.LT.1 .OR. N.GT.ICNTL(4)) GO TO 70
      ...
      CALL MA27N(N,NZ,NZ1,A,LA,IRN,ICN,IW,LIW,IKEEP,IW1,ICNTL,INFO)
      IF (INFO(1).EQ.-3) GO TO 130
      IF (INFO(1).EQ.-4) GO TO 160
      CALL MA27O(N,NZ1,A,LA,IW,LIW,IKEEP,IKEEP(1,3),NSTEPS,MAXFRT,
     +           IKEEP(1,2),IW1,ICNTL,CNTL,INFO)
      ...
      END IF
   65 FORMAT (' *** WARNING MESSAGE FROM SUBROUTINE MA27B',
     +        '  *** INFO(1) =',I2,
     +        /,5X,'MATRIX IS SINGULAR. RANK=',I5)
      GO TO 220
   70 INFO(1) = -1
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=80) INFO(1)
   80 FORMAT (' **** ERROR RETURN FROM MA27B  **** INFO(1)=',I3)
      IF (ICNTL(1).GT.0) WRITE (ICNTL(1),FMT=90) N
   90 FORMAT (' VALUE OF N OUT OF RANGE ... =',I10)
      GO TO 220
  100 INFO(1) = -2
  ...
  310 RETURN
     ...
     + ICNTL,INFO)
      ...
      REAL, INTENT( IN ), DIMENSION( la ) :: A
      REAL, INTENT( OUT ), DIMENSION( maxfrt ) :: W
      REAL, INTENT( INOUT ), DIMENSION( n ) :: RHS
      INTEGER, INTENT( IN ), DIMENSION( 30 ) :: ICNTL
      INTEGER, INTENT( OUT ), DIMENSION( 20 ) :: INFO
      INTEGER I,IAPOS,IBLK,IPOS,IROWS,J1,J2,JJ,K,KBLK,LATOP,LEN,NBLK,
     +        NCOLS,NROWS
      EXTERNAL MA27Q,MA27R
      INTRINSIC ABS,MIN
      INFO(1) = 0
      IF (ICNTL(3).LE.0 .OR. ICNTL(2).LE.0) GO TO 110
      WRITE (ICNTL(2),FMT=10) N,LA,LIW,MAXFRT,NSTEPS
   10 FORMAT (/,/,' ENTERING MA27C  WITH      N     LA    LIW MAXFRT',
     +       '  NSTEPS',/,21X,5I7)
      KBLK = ABS(IW(1)+0)
  ...
  100 FORMAT (' RHS',1P,5E13.3,/, (4X,1P,5E13.3))
  ...
  120 CONTINUE
      GO TO 150
  130 NBLK = -IW(1)
  140 CALL MA27Q(N,A,LA,IW(2),LIW-1,W,MAXFRT,RHS,IW1,NBLK,LATOP,ICNTL)
      CALL MA27R(N,A,LA,IW(2),LIW-1,W,MAXFRT,RHS,IW1,NBLK,LATOP,ICNTL)
  150 IF (ICNTL(3).LE.0 .OR. ICNTL(2).LE.0) GO TO 170
      WRITE (ICNTL(2),FMT=160)
  160 FORMAT (/,/,' LEAVING MA27C  WITH')
      IF (N.GT.0) WRITE (ICNTL(2),FMT=100) (RHS(I),I=1,K)
  170 CONTINUE
      RETURN
      END
      SUBROUTINE MA27G(N,NZ,IRN,ICN,IW,LW,IPE,IQ,FLAG,IWFR,
     +                 ICNTL,INFO)
   ...
   10 CONTINUE
      ...
      DO 110 K = 1,NZ
        ...
        END IF
   60   FORMAT (' *** WARNING MESSAGE FROM SUBROUTINE MA27A',
     +          '  *** INFO(1) =',I2)
  ...
  310 RETURN
      END
      SUBROUTINE MA27H(N,IPE,IW,LW,IWFR,NV,NXT,LST,IPD,FLAG,IOVFLO,
     +                 NCMPA,FRATIO)
      REAL FRATIO
...
CCC         IF (IWFR.LT.LW) GO TO 100
              ...
              IPE(IE) = JP
              IW(JP) = LN - JP1
              CALL MA27U(N,IPE,IW,IP-1,LWFR,NCMPA)
              JP2 = IWFR - 1
              IWFR = LWFR
  ...
  370 CONTINUE
      ...
      END
      SUBROUTINE MA27U(N,IPE,IW,LW,IWFR,NCMPA)
   ...
   70 RETURN
      END
      SUBROUTINE MA27J(N,NZ,IRN,ICN,PERM,IW,LW,IPE,IQ,FLAG,IWFR,
     +                 ICNTL,INFO)
      INTEGER IWFR,LW,N,NZ
  ...
  250 RETURN
      END
      SUBROUTINE MA27K(N,IPE,IW,LW,IWFR,IPS,IPV,NV,FLAG,NCMPA)
      INTEGER IWFR,LW,N,NCMPA
  ...
  100 CONTINUE
      RETURN
      END
      SUBROUTINE MA27L(N,IPE,NV,IPS,NE,NA,ND,NSTEPS,NEMIN)
  ...
  160 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE MA27M(N,NZ,IRN,ICN,PERM,NA,NE,ND,NSTEPS,LSTKI,LSTKR,
     +                 IW,INFO,OPS)
      REAL OPS
      INTEGER N,NSTEPS,NZ
  ...
  100 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE MA27N(N,NZ,NZ1,A,LA,IRN,ICN,IW,LIW,PERM,IW2,ICNTL,
     +                 INFO)
      REAL ZERO
   ...
   30   INFO(1) = 1
        ...
        ENDIF
   40   FORMAT (' *** WARNING MESSAGE FROM SUBROUTINE MA27B',
     +          '  *** INFO(1) =',I2)
        IF (INFO(2).LE.10 .AND. ICNTL(2).GT.0) THEN
  ...
  230 RETURN
      END
      SUBROUTINE MA27O(N,NZ,A,LA,IW,LIW,PERM,NSTK,NSTEPS,MAXFRT,NELIM,
     +                 IW2,ICNTL,CNTL,INFO)
      REAL ZERO,HALF,ONE
      ...
      INTEGER ICNTL(30),INFO(20)
      REAL AMAX,AMULT,AMULT1,AMULT2,DETPIV,RMAX,SWOP,THRESH,TMAX,UU
     ...
     +        POSPV2
      INTEGER NTWO,NEIG,NCMPBI,NCMPBR,NRLBDU,NIRBDU,IXDUM,IYDUM
      EXTERNAL MA27P
  ...
  150   CONTINUE
        ...
        CALL MA27P(A,IW,ISTK,ISTK2,IINPUT,2,NCMPBR,NCMPBI)
        IF (NEWEL+NFRONT.LT.ISTK) GO TO 160
  ...
  180   CALL MA27P(A,IW,ASTK,ASTK2,AINPUT,1,NCMPBR,NCMPBI)
...
CBUG        APOS = POSFAC + IDIAG(NFRONT-NPIV,IPIV-NPIV)
            IXDUM = NFRONT-NPIV
            IYDUM = IPIV-NPIV
            APOS = POSFAC + IDIAG(IXDUM,IYDUM)
  ...
  300       IF (A(APOS).GT.ZERO .AND. ISNPIV.EQ.1) GO TO 560
            ...
            END IF
  310       FORMAT (' *** WARNING MESSAGE FROM SUBROUTINE MA27B',
     +              '  *** INFO(1) =',I2,/,' PIVOT',I6,
...
CBUG        APOS2 = POSFAC + IDIAG(NFRONT-NPIV,JMAX-NPIV)
            IXDUM = NFRONT-NPIV
            IYDUM = JMAX-NPIV
            APOS2 = POSFAC + IDIAG(IXDUM,IYDUM)
...
CDIR$ IVDEP
  ...
  810 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE MA27P(A,IW,J1,J2,ITOP,IREAL,NCMPBR,NCMPBI)
   ...
   50 RETURN
      END
      SUBROUTINE MA27Q(N,A,LA,IW,LIW,W,MAXFNT,RHS,IW2,NBLK,LATOP,ICNTL)
      INTEGER IFRLVL
      PARAMETER ( IFRLVL=5 )
      INTEGER LA,LATOP,LIW,MAXFNT,N,NBLK
      REAL A(LA),RHS(N),W(MAXFNT)
  ...
  150 LATOP = APOS - 1
      RETURN
      END
      SUBROUTINE MA27R(N,A,LA,IW,LIW,W,MAXFNT,RHS,IW2,NBLK,LATOP,ICNTL)
      INTEGER IFRLVL
      PARAMETER ( IFRLVL=5 )
      INTEGER LA,LATOP,LIW,MAXFNT,N,NBLK
      REAL A(LA),RHS(N),W(MAXFNT)
