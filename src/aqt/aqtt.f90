...
!  OPEN( UNIT = 23 )
   DO pass = 1, 11
      IF ( pass /= 4 .AND. pass /= 7 .AND. pass /= 8 )                         &
           CALL AQT_initialize( data, control, info )
...
!  Iteration to find the minimizer
      ...
      DO                                     
         CALL AQT_solve( n, radius, f, X, R, VECTOR, data, control, info )
...
!  Error returns
      ...
      WRITE( 6, "( ' pass ', I3, ' AQT_solve exit status = ', I6 )" )         &
             pass, info%status
...
!               info%iter + info%iter_pass2, f, info%multiplier
      IF ( pass /= 3 .AND. pass /= 6 .AND. pass /= 7 )                         &
        CALL AQT_terminate( data, control, info ) !  delete internal workspace
   END DO
...
!  Iteration to find the minimizer
      ...
      DO                                     
         IF ( pass /= 4 ) THEN
           CALL AQT_solve( nn, radius, f, X( : nn ), R( : nn ),                &
                            VECTOR( : nn ), data, control, info )
         ELSE
           CALL AQT_solve( nn, radius, f, X0, R0,                              &
...
!  STOP
   END PROGRAM GALAHAD_AQT_test_deck
