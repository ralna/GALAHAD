...
!  History -
!    originally released GALAHAD Version 3.3. December 20th 2021

!  For full documentation, see
!   http://galahad.rl.ac.uk/galahad-www/specs.html

  MODULE GALAHAD_BQPB_precision_ciface
    USE GALAHAD_KINDS
    USE GALAHAD_common_ciface
    USE GALAHAD_BQPB_precision, ONLY:                                          &
        f_bqpb_control_type   => BQPB_control_type,                            &
        f_bqpb_time_type      => BQPB_time_type,                               &
        f_bqpb_inform_type    => BQPB_inform_type,                             &
        f_bqpb_full_data_type => BQPB_full_data_type,                          &
        f_bqpb_initialize     => BQPB_initialize,                              &
        f_bqpb_read_specfile  => BQPB_read_specfile,                           &
        f_bqpb_import         => BQPB_import,                                  &
        f_bqpb_reset_control  => BQPB_reset_control,                           &
        f_bqpb_solve_qp       => BQPB_solve_qp,                                &
        f_bqpb_solve_sldqp    => BQPB_solve_sldqp,                             &
        f_bqpb_information    => BQPB_information,                             &
        f_bqpb_terminate      => BQPB_terminate
...
!-------------------------------------------------
    ...
    TYPE, BIND( C ) :: bqpb_control_type
      ...
      REAL ( KIND = rp_ ) :: stop_abs_p
      REAL ( KIND = rp_ ) :: stop_rel_p
      REAL ( KIND = rp_ ) :: stop_abs_d
      REAL ( KIND = rp_ ) :: stop_rel_d
      REAL ( KIND = rp_ ) :: stop_abs_c
      REAL ( KIND = rp_ ) :: stop_rel_c
      REAL ( KIND = rp_ ) :: perturb_h
      REAL ( KIND = rp_ ) :: prfeas
      REAL ( KIND = rp_ ) :: dufeas
      REAL ( KIND = rp_ ) :: muzero
      REAL ( KIND = rp_ ) :: tau
      REAL ( KIND = rp_ ) :: gamma_c
    ...
    TYPE, BIND( C ) :: bqpb_inform_type
      ...
      REAL ( KIND = rp_ ) :: dual_infeasibility
      REAL ( KIND = rp_ ) :: complementary_slackness
      REAL ( KIND = rp_ ) :: init_primal_infeasibility
      REAL ( KIND = rp_ ) :: init_dual_infeasibility
      REAL ( KIND = rp_ ) :: init_complementary_slackness
      ...
      REAL ( KIND = rp_ ), DIMENSION( 16 ) :: checkpointsTime
...
!  copy C control parameters to fortran
    ...
    TYPE ( bqpb_control_type ), INTENT( IN ) :: ccontrol
    TYPE ( f_bqpb_control_type ), INTENT( OUT ) :: fcontrol
...
!  copy fortran control parameters to C
    ...
    TYPE ( f_bqpb_control_type ), INTENT( IN ) :: fcontrol
    TYPE ( bqpb_control_type ), INTENT( OUT ) :: ccontrol
...
!  copy C time parameters to fortran
    ...
    TYPE ( bqpb_time_type ), INTENT( IN ) :: ctime
    TYPE ( f_bqpb_time_type ), INTENT( OUT ) :: ftime
...
!  copy fortran time parameters to C
    ...
    TYPE ( f_bqpb_time_type ), INTENT( IN ) :: ftime
    TYPE ( bqpb_time_type ), INTENT( OUT ) :: ctime
...
!  copy C inform parameters to fortran
    ...
    TYPE ( bqpb_inform_type ), INTENT( IN ) :: cinform
    TYPE ( f_bqpb_inform_type ), INTENT( OUT ) :: finform
    INTEGER ( KIND = ip_ ) :: i
...
!  copy fortran inform parameters to C
    ...
    TYPE ( f_bqpb_inform_type ), INTENT( IN ) :: finform
    TYPE ( bqpb_inform_type ), INTENT( OUT ) :: cinform
    INTEGER ( KIND = ip_ ) :: i, l
  ...
  END MODULE GALAHAD_BQPB_precision_ciface
...
!  initialize required fortran types
  ...
  CALL f_bqpb_initialize( fdata, fcontrol, finform )
...
!  ----------------------------------------
!  C interface to fortran bqpb_read_specfile
!  ----------------------------------------

  SUBROUTINE bqpb_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_BQPB_precision_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( bqpb_control_type ), INTENT( INOUT ) :: ccontrol
...
!  ---------------------------------
!  C interface to fortran bqpb_inport
!  ---------------------------------

  SUBROUTINE bqpb_import( ccontrol, cdata, status, n,                          &
...
!  ---------------------------------------
!  C interface to fortran bqpb_reset_control
!  ----------------------------------------

  SUBROUTINE bqpb_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_BQPB_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_BQPB_reset_control( fcontrol, fdata, status )
  RETURN

  END SUBROUTINE bqpb_reset_control

!  ------------------------------------
!  C interface to fortran bqpb_solve_bqpb
!  ------------------------------------

  SUBROUTINE bqpb_solve_qp( cdata, status, n, hne, hval, g, f,                 &
                            xl, xu, x, z, xstat ) BIND( C )
  USE GALAHAD_BQPB_precision_ciface
  IMPLICIT NONE
...
!  solve the qp
  ...
  CALL f_bqpb_solve_qp( fdata, status, hval, g, f, xl, xu, x, z, xstat )
  RETURN

  END SUBROUTINE bqpb_solve_qp

!  --------------------------------------
!  C interface to fortran bqpb_solve_sldqp
!  --------------------------------------

  SUBROUTINE bqpb_solve_sldqp( cdata, status, n, w, x0, g, f, xl, xu,          &
                               x, z, xstat ) BIND( C )
  USE GALAHAD_BQPB_precision_ciface
  IMPLICIT NONE
...
!  solve the qp
  ...
  CALL f_bqpb_solve_sldqp( fdata, status, w, x0, g, f, xl, xu, x, z, xstat )
  RETURN
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( bqpb_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain BQPB solution information

  CALL f_bqpb_information( fdata, finform, status )
...
!  deallocate workspace
  ...
  CALL f_bqpb_terminate( fdata, fcontrol, finform )
