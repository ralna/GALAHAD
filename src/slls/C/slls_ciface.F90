...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_SLLS_precision_ciface
    USE GALAHAD_KINDS_precision
    USE GALAHAD_common_ciface
    USE GALAHAD_SLLS_precision, ONLY:                                          &
        f_slls_control_type         => SLLS_control_type,                      &
        f_slls_time_type            => SLLS_time_type,                         &
        f_slls_inform_type          => SLLS_inform_type,                       &
        f_slls_full_data_type       => SLLS_full_data_type,                    &
        f_slls_initialize           => SLLS_initialize,                        &
        f_slls_read_specfile        => SLLS_read_specfile,                     &
        f_slls_import               => SLLS_import,                            &
        f_slls_import_without_a     => SLLS_import_without_a,                  &
        f_slls_reset_control        => SLLS_reset_control,                     &
        f_slls_solve_given_a        => SLLS_solve_given_a,                     &
        f_slls_solve_reverse_a_prod => SLLS_solve_reverse_a_prod,              &
        f_slls_information          => SLLS_information,                       &
...
!  slls_solve_given_a. A module wrapper + module state (rather than a wrapper
...
!  AddressSanitizer and by modern linkers / hardened runtimes.
    ...
    TYPE ( C_FUNPTR ) :: slls_saved_ceval_prec = C_NULL_FUNPTR
    INTEGER ( KIND = ipc_ ) :: slls_saved_n
    TYPE ( C_PTR ) :: slls_saved_cuserdata
...
!  copy C control parameters to fortran
    ...
    TYPE ( slls_control_type ), INTENT( IN ) :: ccontrol
    TYPE ( f_slls_control_type ), INTENT( OUT ) :: fcontrol
...
!  copy fortran control parameters to C
    ...
    TYPE ( f_slls_control_type ), INTENT( IN ) :: fcontrol
    TYPE ( slls_control_type ), INTENT( OUT ) :: ccontrol
...
!  copy C time parameters to fortran
    ...
    TYPE ( slls_time_type ), INTENT( IN ) :: ctime
    TYPE ( f_slls_time_type ), INTENT( OUT ) :: ftime
...
!  copy fortran time parameters to C
    ...
    TYPE ( f_slls_time_type ), INTENT( IN ) :: ftime
    TYPE ( slls_time_type ), INTENT( OUT ) :: ctime
...
!  copy C inform parameters to fortran
    ...
    TYPE ( slls_inform_type ), INTENT( IN ) :: cinform
    TYPE ( f_slls_inform_type ), INTENT( OUT ) :: finform
    INTEGER ( KIND = ip_ ) :: i
...
!  copy fortran inform parameters to C
    ...
    TYPE ( f_slls_inform_type ), INTENT( IN ) :: finform
    TYPE ( slls_inform_type ), INTENT( OUT ) :: cinform
    INTEGER ( KIND = ip_ ) :: i, l
...
!  trampoline / executable stack); per-call state is in the slls_saved_* module
!  variables set by slls_solve_given_a. NB: non-reentrant (one solve/process).
...
!  call the C interoperable eval_prec
    PROCEDURE( eval_prec ), POINTER :: slls_saved_eval_prec
    CALL C_F_PROCPOINTER( slls_saved_ceval_prec, slls_saved_eval_prec )
    status = slls_saved_eval_prec( slls_saved_n, v, p, slls_saved_cuserdata )
    RETURN

    END SUBROUTINE wrap_eval_prec

  END MODULE GALAHAD_SLLS_precision_ciface

!  --------------------------------------
!  C interface to fortran slls_initialize
!  --------------------------------------

  SUBROUTINE slls_initialize( cdata, ccontrol, status ) BIND( C )
...
!  local variables
  ...
  TYPE ( f_slls_inform_type ) :: finform
  LOGICAL :: f_indexing
...
!  initialize required fortran types
  ...
  CALL f_slls_initialize( fdata, fcontrol, finform )
...
!  -----------------------------------------
!  C interface to fortran slls_read_specfile
!  -----------------------------------------

  SUBROUTINE slls_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_SLLS_precision_ciface
  IMPLICIT NONE
...
!  read control parameters from the specfile
  ...
  CALL f_slls_read_specfile( fcontrol, device )
...
!  -----------------------------------------
!  C interface to fortran slls_reset_control
!  -----------------------------------------

  SUBROUTINE slls_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_SLLS_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_SLLS_reset_control( fcontrol, fdata, status )
  RETURN
...
!  associate procedure pointers
  ...
  IF ( C_ASSOCIATED( ceval_prec ) ) THEN
    slls_saved_ceval_prec = ceval_prec
  ELSE
    slls_saved_ceval_prec = C_NULL_FUNPTR
  END IF
  slls_saved_n = n
  slls_saved_cuserdata = cuserdata
...
!  ---------------------------------------
!  C interface to fortran slls_information
!  ---------------------------------------

  SUBROUTINE slls_information( cdata, cinform, status ) BIND( C )
  USE GALAHAD_SLLS_precision_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( slls_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain SLLS solution information

  CALL f_slls_information( fdata, finform, status )
...
!  -------------------------------------
!  C interface to fortran slls_terminate
!  -------------------------------------

  SUBROUTINE slls_terminate( cdata, ccontrol, cinform ) BIND( C )
  USE GALAHAD_SLLS_precision_ciface
  IMPLICIT NONE
...
!  deallocate workspace
  ...
  CALL f_slls_terminate( fdata, fcontrol, finform )
