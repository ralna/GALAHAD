! THIS VERSION: GALAHAD 5.4 - 2025-11-23 AT 15:00 GMT.

#include "galahad_modules.h"
#include "galahad_cfunctions.h"

!-*-*-*-*-*-*-*-  G A L A H A D _  N R E K    C   I N T E R F A C E  -*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
  ...
  MODULE GALAHAD_NREK_precision_ciface
...
!   USE GALAHAD_common_ciface, ONLY:  strlen, opt_strlen, cstr_to_fchar
    ...
    USE GALAHAD_NREK_precision, ONLY:                                          &
        f_nrek_control_type => NREK_control_type,                              &
        f_nrek_time_type => NREK_time_type,                                    &
        f_nrek_inform_type => NREK_inform_type,                                &
        f_nrek_full_data_type => NREK_full_data_type,                          &
        f_nrek_initialize => NREK_initialize,                                  &
        f_nrek_read_specfile => NREK_read_specfile,                            &
        f_nrek_import => NREK_import,                                          &
        f_nrek_s_import => NREK_s_import,                                      &
        f_nrek_solve_problem => NREK_solve_problem,                            &
        f_nrek_reset_control => NREK_reset_control,                            &
        f_nrek_information => NREK_information,                                &
        f_nrek_terminate => NREK_terminate
...
!-------------------------------------------------
    ...
    TYPE, BIND( C ) :: nrek_control_type
      ...
      LOGICAL ( KIND = C_BOOL ) :: new_weight
...
!  copy C control parameters to fortran
    ...
    TYPE ( nrek_control_type ), INTENT( IN ) :: ccontrol
    TYPE ( f_nrek_control_type ), INTENT( OUT ) :: fcontrol
    ...
    fcontrol%increase = ccontrol%increase
    ...
    fcontrol%new_weight = ccontrol%new_weight
...
!  copy fortran control parameters to C
    ...
    TYPE ( f_nrek_control_type ), INTENT( IN ) :: fcontrol
    TYPE ( nrek_control_type ), INTENT( OUT ) :: ccontrol
    ...
    ccontrol%increase = fcontrol%increase
    ...
    ccontrol%new_weight = fcontrol%new_weight
...
!  copy C time parameters to fortran
    ...
    TYPE ( nrek_time_type ), INTENT( IN ) :: ctime
    TYPE ( f_nrek_time_type ), INTENT( OUT ) :: ftime
...
!  copy fortran time parameters to C
    ...
    TYPE ( f_nrek_time_type ), INTENT( IN ) :: ftime
    TYPE ( nrek_time_type ), INTENT( OUT ) :: ctime
...
!  copy C inform parameters to fortran
    ...
    TYPE ( nrek_inform_type ), INTENT( IN ) :: cinform
    TYPE ( f_nrek_inform_type ), INTENT( OUT ) :: finform
    INTEGER ( KIND = ip_ ) :: i
    ...
    finform%next_weight = cinform%next_weight
...
!  copy fortran inform parameters to C
    ...
    TYPE ( f_nrek_inform_type ), INTENT( IN ) :: finform
    TYPE ( nrek_inform_type ), INTENT( OUT ) :: cinform
    INTEGER ( KIND = ip_ ) :: i, l
    ...
    cinform%next_weight = finform%next_weight
  ...
  END MODULE GALAHAD_NREK_precision_ciface

!  --------------------------------------
!  C interface to fortran nrek_initialize
!  --------------------------------------

  SUBROUTINE nrek_initialize( cdata, ccontrol, status ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  local variables
  ...
  TYPE ( f_nrek_full_data_type ), POINTER :: fdata
  TYPE ( f_nrek_control_type ) :: fcontrol
  TYPE ( f_nrek_inform_type ) :: finform
  LOGICAL :: f_indexing
...
!  initialize required fortran types
  ...
  CALL f_nrek_initialize( fdata, fcontrol, finform )
...
!  -----------------------------------------
!  C interface to fortran nrek_read_specfile
!  -----------------------------------------

  SUBROUTINE nrek_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( nrek_control_type ), INTENT( INOUT ) :: ccontrol
...
!  read control parameters from the specfile
  ...
  CALL f_nrek_read_specfile( fcontrol, device )
...
!  ----------------------------------
!  C interface to fortran nrek_inport
!  ----------------------------------

  SUBROUTINE nrek_import( ccontrol, cdata, status, n,                          &
...
!  import the problem data into the required NREK structure

  CALL f_nrek_import( fcontrol, fdata, status, n,                              &
...
!  -----------------------------------
!  C interface to fortran nrek_inport_s
!  -----------------------------------

  SUBROUTINE nrek_s_import( cdata, status, n, cstype, sne, srow, scol,         &
                            sptr ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  import the problem data into the required NREK structure
  ...
  CALL f_nrek_s_import( fdata, status, fstype, sne, srow, scol, sptr )
  RETURN

  END SUBROUTINE nrek_s_import

!  -----------------------------------------
!  C interface to fortran nrek_reset_control
!  -----------------------------------------

  SUBROUTINE nrek_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_nrek_reset_control( fcontrol, fdata, status )
  RETURN

  END SUBROUTINE nrek_reset_control

!  -----------------------------------------
!  C interface to fortran nrek_solve_problem
!  -----------------------------------------

  SUBROUTINE nrek_solve_problem( cdata, status, n, hne, hval, c,               &
                                 power, weight, x, sne, sval ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  solve the problem
  ...
  CALL f_NREK_solve_problem( fdata, status, hval, c, power, weight, x, sval )
  RETURN

  END SUBROUTINE nrek_solve_problem

!  ---------------------------------------
!  C interface to fortran nrek_information
!  ---------------------------------------

  SUBROUTINE nrek_information( cdata, cinform, status ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( nrek_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain NREK solution information

  CALL f_nrek_information( fdata, finform, status )
...
!  ------------------------------------
!  C interface to fortran nrek_terminate
!  ------------------------------------

  SUBROUTINE nrek_terminate( cdata, ccontrol, cinform ) BIND( C )
  USE GALAHAD_NREK_precision_ciface
  IMPLICIT NONE
...
!  deallocate workspace
  ...
  CALL f_nrek_terminate( fdata, fcontrol, finform )
