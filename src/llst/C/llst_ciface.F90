...
!-*-*-*-*-*-*-*-  G A L A H A D _  L L S T    C   I N T E R F A C E  -*-*-*-*-*-

!  Copyright reserved, Gould/Orban/Toint, for GALAHAD productions
!  Principal authors: Jaroslav Fowkes & Nick Gould

!  History -
!    originally released GALAHAD Version 4.1. May 21st 2023

!  For full documentation, see
!   http://galahad.rl.ac.uk/galahad-www/specs.html

  MODULE GALAHAD_LLST_precision_ciface
    USE iso_c_binding
    USE GALAHAD_common_ciface
    USE GALAHAD_LLST_precision, ONLY:                                          &
        f_llst_control_type => LLST_control_type,                              &
        f_llst_time_type => LLST_time_type,                                    &
        f_llst_history_type => LLST_history_type,                              &
        f_llst_inform_type => LLST_inform_type,                                &
        f_llst_full_data_type => LLST_full_data_type,                          &
        ...
        f_llst_terminate => LLST_terminate
...
!-------------------------------------------------
    ...
    END TYPE llst_control_type

    TYPE, BIND( C ) :: llst_time_type
    ...
    END TYPE llst_time_type

    TYPE, BIND( C ) :: llst_history_type
      REAL ( KIND = rpc_ ) :: lambda
      REAL ( KIND = rpc_ ) :: x_norm
      REAL ( KIND = rpc_ ) :: r_norm
    END TYPE llst_history_type

    TYPE, BIND( C ) :: llst_inform_type
      ...
      TYPE ( llst_history_type ), DIMENSION( 100 ) :: history
      TYPE ( sbls_inform_type ) :: sbls_inform
      TYPE ( sls_inform_type ) :: sls_inform
      TYPE ( ir_inform_type ) :: ir_inform
    END TYPE llst_inform_type
...
!  copy fortran inform parameters to C
  ...
  END MODULE GALAHAD_LLST_precision_ciface
...
!  initialize required fortran types
  ...
  CALL f_llst_initialize( fdata, fcontrol, finform )
...
!  ----------------------------------------
!  C interface to fortran llst_read_specfile
!  ----------------------------------------

  SUBROUTINE llst_read_specfile( ccontrol, cspecfile ) BIND( C )
  USE GALAHAD_LLST_precision_ciface
  IMPLICIT NONE

!  dummy arguments

  TYPE ( llst_control_type ), INTENT( INOUT ) :: ccontrol
...
!  ---------------------------------
  ...
  SUBROUTINE llst_import( ccontrol, cdata, status, m, n,                       &
...
!  import the problem data into the required LLST structure
  ...
  CALL f_llst_import( fcontrol, fdata, status, m, n,                           &
...
!  ----------------------------------------
  ...
  SUBROUTINE llst_reset_control( ccontrol, cdata, status ) BIND( C )
  USE GALAHAD_LLST_precision_ciface
  IMPLICIT NONE
...
!  import the control parameters into the required structure
  ...
  CALL f_LLST_reset_control( fcontrol, fdata, status )
  RETURN

  END SUBROUTINE llst_reset_control

!  ----------------------------------------
!  C interface to fortran llst_solve_problem
!  ----------------------------------------

  SUBROUTINE llst_solve_problem( cdata, status, m, n, radius, ane, aval, b,    &
                                 x, sne, sval ) BIND( C )
  USE GALAHAD_LLST_precision_ciface
  IMPLICIT NONE
...
!  dummy arguments
  ...
  TYPE ( C_PTR ), INTENT( INOUT ) :: cdata
  TYPE ( llst_inform_type ), INTENT( INOUT ) :: cinform
  INTEGER ( KIND = ipc_ ), INTENT( OUT ) :: status
...
!  obtain LLST solution information

  CALL f_llst_information( fdata, finform, status )
