...
!-*-*-*-*-*-*-*-  G A L A H A D   U S E D L P   M O D U L E  -*-*-*-*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
    ...
    MODULE GALAHAD_USEDLP_precision

!     -------------------------------------------------------
!    | CUTEst/AMPL interface to DLP, a dual                  |
!    |  gradient-projection algorithm for linear programming |
!     -------------------------------------------------------
...
!-*-*-*-*-*-*-*-*-*-   U S E _ D L P  S U B R O U T I N E   -*-*-*-*-*-*-*-*-

     SUBROUTINE USE_DLP( input )
...
!  Scalars
      ...
      INTEGER ( KIND = ip_ ) :: i, j, l, nea, n, m, ir, ic, la, liw, iores
      INTEGER ( KIND = ip_ ) :: smt_stat, status
...
!  Default values for specfile-defined parameters
      ...
      INTEGER ( KIND = ip_ ) :: objective_type = 0
...
!   Define the keywords
        ...
        spec( 25 )%keyword = 'objective-type'
        spec( 26 )%keyword = ''
...
!   Interpret the result
        ...
        CALL SPECFILE_assign_integer( spec( 25 ), objective_type, errout )
      END IF
...
!  The objective specified by the SIF file will be removed

      IF ( objective_type == 1 ) prob%gradient_kind = 0
...
!  If required, print out the (raw) problem data
      ...
      IF ( write_problem_data ) THEN
        ...
        IF ( prob%gradient_kind == 0 ) THEN
          WRITE( dfiledevice, "( ' no objective' )" )
        ELSE
...
!     SCALE_control%print_level = DLP_control%print_level
...
!if(j == 10228) WRITE(6,*) i, prob%A%val( l ), prob%Y( i )
      ...
      END DO
!     DO i = 1, n
!       WRITE(6,"( i6, 2ES12.4 )" ) i, prob%Z( i ), AY( i )
!     END DO
!     WRITE(6,"( ( 5ES12.4 ) ) " ) MAXVAL( ABS( prob%Z ) )
!     WRITE(6,"( ' G ', /, ( 5ES12.4 ) )" ) prob%G( : n )
      res_k = MAXVAL( ABS( - prob%Z( : n ) + AY( : n ) ) )
      DEALLOCATE( AY )
...
!  End of subroutine USE_DLP

     END SUBROUTINE USE_DLP

!  End of module USEDLP

   END MODULE GALAHAD_USEDLP_precision
