...
function lstr_solve_problem(::Type{Float32}, ::Type{Int32}, data, status, m, n, radius, x,
                            u, v)
  @ccall libgalahad_single.lstr_solve_problem_s(data::Ptr{Ptr{Cvoid}}, status::Ptr{Int32},
...
function run_sif(::Val{:lstr}, ::Val{:single}, path_libsif::String, path_outsdif::String)
  cmd = setup_env_lbt(`$(GALAHAD_jll.runlstr_sif_single()) $path_libsif $path_outsdif`)
  run(cmd)
  return nothing
end

function run_sif(::Val{:lstr}, ::Val{:double}, path_libsif::String, path_outsdif::String)
