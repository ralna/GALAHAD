...
const runlpb_sif_single = joinpath(galahad_bindir, "runlpb_sif_single$(exeext)")

function run_sif(::Val{:lpb}, ::Val{:single}, path_libsif::String, path_outsdif::String)
  return run(`$runlpb_sif_single $path_libsif $path_outsdif`)
end

const runlpb_sif_double = joinpath(galahad_bindir, "runlpb_sif_double$(exeext)")

function run_sif(::Val{:lpb}, ::Val{:double}, path_libsif::String, path_outsdif::String)
  return run(`$runlpb_sif_double $path_libsif $path_outsdif`)
end

const runlpb_sif_quadruple = joinpath(galahad_bindir, "runlpb_sif_quadruple$(exeext)")

function run_sif(::Val{:lpb}, ::Val{:quadruple}, path_libsif::String, path_outsdif::String)
  return run(`$runlpb_sif_quadruple $path_libsif $path_outsdif`)
end
...
function run_qplib(::Val{:lpb}, ::Val{:single}, path_qplib::String)
  open(path_qplib, "r") do io
    process = pipeline(`$runlpb_qplib_single`; stdin=io)
    return run(process)
  end
end
...
function run_qplib(::Val{:lpb}, ::Val{:double}, path_qplib::String)
  open(path_qplib, "r") do io
    process = pipeline(`$runlpb_qplib_double`; stdin=io)
    return run(process)
  end
end
...
function run_qplib(::Val{:lpb}, ::Val{:quadruple}, path_qplib::String)
  open(path_qplib, "r") do io
    process = pipeline(`$runlpb_qplib_quadruple`; stdin=io)
    return run(process)
  end
end
