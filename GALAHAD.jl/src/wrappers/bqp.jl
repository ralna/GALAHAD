...
function bqp_read_specfile(control, specfile)
    @ccall libgalahad_double.bqp_read_specfile(control::Ref{bqp_control_type},
                                               specfile::Ptr{Cchar})::Cvoid
end

function bqp_import(control, data, status, n, H_type, ne, H_row, H_col, H_ptr)
    @ccall libgalahad_double.bqp_import(control::Ref{bqp_control_type},
...
function bqp_import_without_h(control, data, status, n)
    @ccall libgalahad_double.bqp_import_without_h(control::Ref{bqp_control_type},
                                                  data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint},
                                                  n::Cint)::Cvoid
end

function bqp_reset_control(control, data, status)
    @ccall libgalahad_double.bqp_reset_control(control::Ref{bqp_control_type},
                                               data::Ptr{Ptr{Cvoid}},
                                               status::Ptr{Cint})::Cvoid
end
...
function bqp_terminate(data, control, inform)
    @ccall libgalahad_double.bqp_terminate(data::Ptr{Ptr{Cvoid}},
                                           control::Ref{bqp_control_type},
                                           inform::Ref{bqp_inform_type})::Cvoid
end
