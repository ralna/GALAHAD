...
function test_lpa(::Type{T}, ::Type{INT}) where {T,INT}
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{lpa_control_type{T,INT}}()
  inform = Ref{lpa_inform_type{T,INT}}()
  ...
  for d in 1:3
    # Initialize LPA
    lpa_initialize(T, INT, data, control, status)
    ...
    if d == 1
      st = 'C'
      lpa_import(T, INT, control, data, status, n, m,
                 "coordinate", A_ne, A_row, A_col, C_NULL)

      lpa_solve_lp(T, INT, data, status, n, m, g, f,
    ...
    lpa_information(T, INT, data, inform, status)
    ...
    lpa_terminate(T, INT, data, control, inform)
  end
...
@testset "LPA" begin
  @test test_lpa(Float32, Int32) == 0
  @test test_lpa(Float64, Int32) == 0
  @test test_lpa(Float128, Int32) == 0
end
