...
function test_dqp(::Type{T}, ::Type{INT}) where {T,INT}
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{dqp_control_type{T,INT}}()
  inform = Ref{dqp_inform_type{T,INT}}()
  ...
  for d in 1:6
    ...
    if d == 1
      st = 'C'
      dqp_import(T, INT, control, data, status, n, m,
                 "coordinate", H_ne, H_row, H_col, C_NULL,
                 "coordinate", A_ne, A_row, A_col, C_NULL)

      dqp_solve_qp(T, INT, data, status, n, m, H_ne, H_val, g, f,
    ...
    if d == 3
      ...
      dqp_solve_qp(T, INT, data, status, n, m, H_dense_ne, H_dense, g, f,
    ...
    dqp_information(T, INT, data, inform, status)
    ...
    dqp_terminate(T, INT, data, control, inform)
  end
  ...
  for d in 1:1
    ...
    if d == 1
      ...
      dqp_solve_sldqp(T, INT, data, status, n, m, w, x_0, g, f,
                      A_ne, A_val, c_l, c_u, x_l, x_u, x, c, y, z,
                      x_stat, c_stat)
    end
...
@testset "DQP" begin
  @test test_dqp(Float32, Int32) == 0
  @test test_dqp(Float64, Int32) == 0
  @test test_dqp(Float128, Int32) == 0
end
