...
function test_arc(::Type{T}, ::Type{INT}; mode::String="reverse", sls::String="sytr", dls::String="potr") where {T,INT}
  ...
  ne_dense = INT(6)  # Dense Hesssian elements
  ...
  if mode == "direct"
    ...
    for d in 1:5
      ...
      if d == 4
        st = 'I'
        arc_import(T, INT, control, data, status, n, "diagonal", ne, C_NULL, C_NULL, C_NULL)
        arc_solve_with_mat(T, INT, data, userdata_ptr, status, n, x, g, ne, fun_diag_ptr, grad_diag_ptr, hess_diag_ptr, prec_ptr)
      end
...
for (T, INT, libgalahad) in ((Float32 , Int32, GALAHAD.libgalahad_single      ),
  ...
  if isfile(libgalahad)
    @testset "ARC -- $T -- $INT" begin
      @test test_arc(T, INT; mode="reverse") == 0
      @test test_arc(T, INT; mode="direct") == 0
    end
  end
end
