...
function test_ugo(::Type{T}, ::Type{INT}; mode::String="reverse") where {T,INT}
  ...
  function eval_fgh(x::Ptr{T}, f::Ptr{T}, g::Ptr{T}, h::Ptr{T}, userdata::Ptr{Cvoid})::INT
    _x = unsafe_wrap(Vector{T}, x, 1)
    ...
    _f[1] = objf(_x[1], _userdata)
    _g[1] = gradf(_x[1], _userdata)
    _h[1] = hessf(_x[1], _userdata)
    return INT(0)
  end

  eval_fgh_ptr = @eval @cfunction($eval_fgh, $INT, (Ptr{$T}, Ptr{$T}, Ptr{$T}, Ptr{$T}, Ptr{Cvoid}))
...
for (T, INT, libgalahad) in ((Float32 , Int32, GALAHAD.libgalahad_single      ),
  ...
  if isfile(libgalahad)
    @testset "UGO -- $T -- $INT" begin
      @test test_ugo(T, INT; mode="reverse") == 0
      # @test test_ugo(T, INT; mode="direct") == 0
    end
  end
end
