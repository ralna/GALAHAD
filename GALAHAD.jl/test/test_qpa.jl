...
function test_qpa(::Type{T}, ::Type{INT}; sls::String="sytr") where {T,INT}
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{qpa_control_type{T,INT}}()
  inform = Ref{qpa_inform_type{T,INT}}()
  ...
  for d in 1:7
    ...
    if d == 1
      st = 'C'
      qpa_import(T, INT, control, data, status, n, m,
                 "coordinate", H_ne, H_row, H_col, C_NULL,
                 "coordinate", A_ne, A_row, A_col, C_NULL)

      qpa_solve_qp(T, INT, data, status, n, m, H_ne, H_val, g, f,
    ...
    if d == 3
      ...
      qpa_solve_qp(T, INT, data, status, n, m, H_dense_ne, H_dense, g, f,
    ...
    qpa_information(T, INT, data, inform, status)
    ...
    qpa_terminate(T, INT, data, control, inform)
  end
  ...
  qpa_solve_l1qp(T, INT, data, status, n, m, H_ne, H_val, g, f, rho_g, rho_b,
                 A_ne, A_val, c_l, c_u, x_l, x_u, x, c, y, z,
                 x_stat, c_stat)
  ...
  qpa_solve_bcl1qp(T, INT, data, status, n, m, H_ne, H_val, g, f, rho_g,
                   A_ne, A_val, c_l, c_u, x_l, x_u, x, c, y, z,
                   x_stat, c_stat)
...
for (T, INT, libgalahad) in ((Float32 , Int32, GALAHAD.libgalahad_single      ),
  ...
  if isfile(libgalahad)
    @testset "QPA -- $T -- $INT" begin
      @test test_qpa(T, INT) == 0
    end
  end
end
