...
function test_rqs(::Type{T}, ::Type{INT}) where {T,INT}
  ...
  inform = Ref{rqs_inform_type{T,INT}}()
    ...
    for m_is in 0:1 # include a scaling matrix?
      ...
      for storage_type in 1:4
        ...
        if storage_type == 1
          ...
          rqs_import(T, INT, control, data, status, n,
                     "coordinate", H_ne, H_row, H_col, C_NULL)
          if m_is == 1
            rqs_import_m(T, INT, data, status, n,
                         "coordinate", M_ne, M_row, M_col, C_NULL)
          end
          if a_is == 1
            rqs_import_a(T, INT, data, status, m,
                         "coordinate", A_ne, A_row, A_col, C_NULL)
          end

          # solve the problem
          if (a_is == 1) && (m_is == 1)
            rqs_solve_problem(T, INT, data, status, n,
        ...
        if storage_type == 3
          ...
          # solve the problem
          if (a_is == 1) && (m_is == 1)
            rqs_solve_problem(T, INT, data, status, n, power, weight,
        ...
        rqs_information(T, INT, data, inform, status)
...
@testset "RQS" begin
  @test test_rqs(Float32, Int32) == 0
  @test test_rqs(Float64, Int32) == 0
  @test test_rqs(Float128, Int32) == 0
end
