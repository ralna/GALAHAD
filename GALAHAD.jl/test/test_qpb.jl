...
function test_qpb(::Type{T}, ::Type{INT}; sls::String="sytr", dls::String="potr") where {T,INT}
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{qpb_control_type{T,INT}}()
  inform = Ref{qpb_inform_type{T,INT}}()
  ...
  for d in 1:7
    # Initialize QPB
    qpb_initialize(T, INT, data, control, status)
    ...
    @reset control[].lsqp_control.fdc_control.use_sls = true
    @reset control[].lsqp_control.fdc_control.symmetric_linear_solver = galahad_linear_solver(sls)
    @reset control[].lsqp_control.sbls_control.symmetric_linear_solver = galahad_linear_solver(sls)
    @reset control[].lsqp_control.sbls_control.definite_linear_solver = galahad_linear_solver(dls)
    ...
    if d == 1
      st = 'C'
      qpb_import(T, INT, control, data, status, n, m,
                 "coordinate", H_ne, H_row, H_col, C_NULL,
                 "coordinate", A_ne, A_row, A_col, C_NULL)

      qpb_solve_qp(T, INT, data, status, n, m, H_ne, H_val, g, f,
    ...
    if d == 3
      ...
      qpb_solve_qp(T, INT, data, status, n, m, H_dense_ne, H_dense, g, f,
    ...
    qpb_information(T, INT, data, inform, status)
...
for (T, INT, libgalahad) in ((Float32 , Int32, GALAHAD.libgalahad_single      ),
  ...
  if isfile(libgalahad)
    @testset "QPB -- $T -- $INT" begin
      @test test_qpb(T, INT) == 0
    end
  end
end
