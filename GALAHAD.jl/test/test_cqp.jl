...
function test_cqp(::Type{T}, ::Type{INT}) where {T,INT}
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{cqp_control_type{T,INT}}()
  inform = Ref{cqp_inform_type{T,INT}}()
  ...
  for d in 1:7
    # Initialize CQP
    cqp_initialize(T, INT, data, control, status)
    ...
    if d == 1
      st = 'C'
      cqp_import(T, INT, control, data, status, n, m,
                 "coordinate", H_ne, H_row, H_col, C_NULL,
                 "coordinate", A_ne, A_row, A_col, C_NULL)

      cqp_solve_qp(T, INT, data, status, n, m, H_ne, H_val, g, f,
    ...
    if d == 3
      ...
      cqp_solve_qp(T, INT, data, status, n, m, H_dense_ne, H_dense, g, f,
    ...
    cqp_information(T, INT, data, inform, status)
  ...
  for d in 1:1
    ...
    if d == 1
      ...
      cqp_solve_sldqp(T, INT, data, status, n, m, w, x_0, g, f,
                      A_ne, A_val, c_l, c_u, x_l, x_u, x, c, y, z,
                      x_stat, c_stat)
    end
...
@testset "CQP" begin
  @test test_cqp(Float32, Int32) == 0
  @test test_cqp(Float64, Int32) == 0
  @test test_cqp(Float128, Int32) == 0
end
