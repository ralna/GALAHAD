...
function test_dgo(::Type{T}, ::Type{INT}; mode::String="reverse", sls::String="sytr", dls::String="potr") where {T,INT}
  ...
  # Derived types
  data = Ref{Ptr{Cvoid}}()
  control = Ref{dgo_control_type{T,INT}}()
  inform = Ref{dgo_inform_type{T,INT}}()

  # Set user data
  pass_userdata = true
  userdata = userdata_dgo{T}(4, 10, 1000, pass_userdata, fun, grad, hess, hessprod, nothing, prec)
  userdata_ptr = pointer_from_objref(userdata)
  userdata_dense = userdata_dgo{T}(4, 10, 1000, pass_userdata, fun, grad, hess_dense, hessprod, nothing, prec)
  userdata_dense_ptr = pointer_from_objref(userdata_dense)
  userdata_diag = userdata_dgo{T}(4, 10, 1000, pass_userdata, fun_diag, grad_diag, hess_diag, hessprod_diag, nothing, prec)
  userdata_diag_ptr = pointer_from_objref(userdata_diag)
  userdata_nh = userdata_dgo{T}(4, 10, 1000, pass_userdata, fun, grad, nothing, hessprod, shessprod, prec)
  ...
  if mode == "direct"
    ...
    for d in 1:5
      ...
      if d == 3
        ...
        dgo_solve_with_mat(T, INT, data, userdata_dense_ptr, status, n, x, g,
                           ne_dense, obj_ptr, grad_ptr, hess_ptr,
                           hessprod_ptr, prec_ptr)
      end
      ...
      if d == 4
        ...
        dgo_solve_with_mat(T, INT, data, userdata_diag_ptr, status, n, x, g,
                           n, obj_ptr, grad_ptr, hess_ptr,
                           hessprod_ptr, prec_ptr)
      end
      ...
      if d == 5
        ...
        dgo_solve_without_mat(T, INT, data, userdata_nh_ptr, status, n, x, g,
                              obj_ptr, grad_ptr, hessprod_ptr, shessprod_ptr, prec_ptr)
      end

      # Record solution information
      dgo_information(T, INT, data, inform, status)

      if inform[].status == 0
        @printf("%c:%6i evaluations. Optimal objective value = %.2f, status = %1i\n",
                st, inform[].f_eval, inform[].obj, inform[].status)
      elseif inform[].status == -99
        @printf("%c:%6i evaluations. Best objective value = %.2f status = %1i\n",
                st, inform[].f_eval, inform[].obj, inform[].status)
      else
  ...
  if mode == "reverse"
    ...
    for d in 1:5
      ...
      if d == 5
        ...
        terminated = false
        while !terminated # reverse-communication loop
          dgo_solve_reverse_without_mat(T, INT, data, status, eval_status,
