...
/*
 * THIS VERSION: GALAHAD 4.2 - 2021-12-21 AT 11:20 GMT.
 *
...
\endmanonly
  ...
  \f$\Delta v = \Delta (x, c, z, z^l, z^u)\f$
  ...
  - \link bqpb_terminate \endlink - deallocate data structures
...
// *-*-*-*-*-*-*-*-*-*-*-*-    B Q P B  _ I M P O R T   -*-*-*-*-*-*-*-*-*-*

void bqpb_import( struct bqpb_control_type *control,
                  void **data,
                  ipc_ *status,
                  ipc_ n,
                  const char H_type[],
...
// *-*-*-*-*-*-*-    B Q P B  _ R E S E T _ C O N T R O L   -*-*-*-*-*-*-*

void bqpb_reset_control( struct bqpb_control_type *control,
                         void **data,
                         ipc_ *status );
...
/*!<
 ...
 @param[in,out] status is a scalar variable of type int, that gives
  ...
  \li -2. A deallocation error occurred.  A message indicating the offending
       ...
       status and a string containing the name of the offending array
       are held in inform.alloc_status and inform.bad_alloc respectively.
  \li -3. The restriction n > 0 or requirement that h_type contains
       its relevant string 'dense', 'coordinate', 'sparse_by_rows',
       'diagonal', 'scaled_identity', 'identity', 'zero' or 'none'
        has been violated.
  \li -5. The simple-bound constraints are inconsistent.
...
void bqpb_information( void **data,
