...
/*! \mainpage GALAHAD C package sllsb

  \section sllsb_intro Introduction

  \subsection sllsb_purpose Purpose
...
\endmanonly
  ...
  \subsection sllsb_authors Authors
  ...
  \subsection sllsb_date Originally released

  January 2026.

  \subsection sllsb_terminology Terminology
...
\endmanonly
  ...
  \f[(A_o^T A_o + \sigma I ) x = A_o^T b + z (2a)\f]
\manonly
  \n
   ( A_o^T A_o + sigma I ) x = A_o^T b + z (2a)
  \n
\endmanonly
...
\endmanonly
  ...
  \subsection sllsb_method Method
  ...
  \subsection sllsb_references Reference
  ...
  \subsection sllsb_call_order Call order

  To solve a given problem, functions from the sllsb package must be called
  in the following order:

  - \link sllsb_initialize \endlink - provide default control parameters and
      set up initial data structures
  - \link sllsb_read_specfile \endlink (optional) - override control values
      by reading replacement values from a file
  - \link sllsb_import \endlink - set up problem data structures and fixed
      values
  - \link sllsb_reset_control \endlink (optional) - possibly change control
      parameters if a sequence of problems are being solved
  - \link sllsb_solve_sllsb \endlink - solve the east-squares problem
  - \link sllsb_information \endlink (optional) - recover information about
    the solution and solution process
  - \link sllsb_terminate \endlink - deallocate data structures
...
// include guard
#ifndef GALAHAD_SLLSB_H
#define GALAHAD_SLLSB_H
...
struct sllsb_control_type {
 ...
 /// \brief
    ...
    /// \brief
    /// control parameters for SLS used by SLLSB_pounce
...
struct sllsb_inform_type {
    ...
    /// estimate of the solution determined by SLLSB_solve
    rpc_ obj;

    /// \brief
    /// the value of the objective function q(x) at the best 
    /// estimate of the solution determined by SLLSB_solve
    rpc_ ls_obj;
    ...
    /// \brief
    /// inform parameters for SLS using SLLSB_pounce
...
// -*-*-*-*-*-*-*-*-*-    S L L S B  _ I N I T I A L I Z E    -*-*-*-*-*-*-*-*-

void sllsb_initialize( void **data,
                      struct sllsb_control_type *control,
                      ipc_ *status );
...
// -*-*-*-*-*-*-*-*-    S L L S B  _ R E A D _ S P E C F I L E   -*-*-*-*-*-*-*-

void sllsb_read_specfile( struct sllsb_control_type *control,
                         const char specfile[] );

/*!<
  Read the content of a specification file, and assign values associated
  with given keywords to the corresponding control parameters.
  By default, the spcification file will be named RUNSLLSB.SPC and
  lie in the current directory.
  Refer to Table 2.1 in the fortran documentation provided in
  $GALAHAD/doc/sllsb.pdf for a list of keywords that may be set.
...
// -*-*-*-*-*-*-*-*-*-*-*-    S L L S B  _ I M P O R T   -*-*-*-*-*-*-*-*-*-*-

void sllsb_import( struct sllsb_control_type *control,
                  void **data,
                  ipc_ *status,
                  ipc_ n,
                  ipc_ o,
...
/*!<
 ...
 @param[in] control is a struct whose members provide control
  paramters for the remaining prcedures (see sllsb_control_type)
...
//  -*-*-*-*-*-*-*-*-   S L L S B _ R E S E T _ C O N T R O L   -*-*-*-*-*-*-*-

void sllsb_reset_control( struct sllsb_control_type *control,
                         void **data,
                         ipc_ *status );
...
//  -*-*-*-*-*-*-*-   S L L S B _ S O L V E _ G I V E N _ A   -*-*-*-*-*-*-*-*-

void sllsb_solve_given_a( void **data,
                          ipc_ *status,
                          ipc_ n,
                          ipc_ o,
                          ipc_ m,
...
/*!<
 ...
 @param[in] x_s is a one-dimensional array of size n and type rpc_, that
   ...
   If the weights are all one, x_s can be set to NULL.

*/


// *-*-*-*-*-*-*-*-*-*-    S L L S B  _ I N F O R M A T I O N   -*-*-*-*-*-*-*-*

void sllsb_information( void **data,
                       struct sllsb_inform_type *inform,
                       ipc_ *status );
...
// *-*-*-*-*-*-*-*-*-*-    S L L S B  _ T E R M I N A T E   -*-*-*-*-*-*-*-*-*-*

void sllsb_terminate( void **data,
                     struct sllsb_control_type *control,
                     struct sllsb_inform_type *inform );
