...
/*! \mainpage GALAHAD C package trek

  \section trek_intro Introduction

  \subsection trek_purpose Purpose
 ...
 \f$\frac{1}{2} x^T H  x + c^T x\f$,
  where the vector \f$x\f$ is required to satisfy
  the constraint \f$\|x\|_S \leq \Delta\f$,
  and where the \f$S\f$-norm of \f$x\f$ is \f$\|x\|_S = \sqrt{x^T S x}\f$.
  ...
  \subsection trek_authors Authors
  ...
  \subsection trek_date Originally released

  November 2025.

  \subsection trek_method Method
  ...
  \subsection trek_references Reference
  ...
  \subsection trek_call_order Call order

  To solve a given problem, functions from the trek package must be called
  in the following order:

  - \link trek_initialize \endlink - provide default control parameters and
      set up initial data structures
  - \link trek_read_specfile \endlink (optional) - override control values
  ...
  - \link trek_solve_problem \endlink - solve the trust-region problem
  - \link trek_information \endlink (optional) - recover information about
    the solution and solution process
  - \link trek_terminate \endlink - deallocate data structures
...
// include guard
#ifndef GALAHAD_TREK_H
#define GALAHAD_TREK_H
...
struct trek_inform_type {
    ...
    ///     the argument control.new_radius.
...
/*!<
  ...
  By default, the spcification file will be named RUNTREK.SPC and
  lie in the current directory.
  Refer to Table 2.1 in the fortran documentation provided in
  $GALAHAD/doc/trek.pdf for a list of keywords that may be set.
...
/*!<
 ...
 @param[in] control is a struct whose members provide control
  paramters for the remaining prcedures (see trek_control_type)
...
/*!<
 ...
 @param[in,out] status is a scalar variable of type ipc_, that gives
  ...
  \li -2. A deallocation error occurred.  A message indicating the offending
       ...
       status and a string containing the name of the offending array
       are held in inform.alloc_status and inform.bad_alloc respectively.
  \li -3. The restrictions n > 0 or radius > 0 or
