...
/*
 * THIS VERSION: GALAHAD 5.5 - 2026-04-15 AT 09:10 GMT.
 *
 *-*-*-*-*-*-*-*-*-  GALAHAD_BNLS C INTERFACE  *-*-*-*-*-*-*-*-*-*-
 *
 *  Copyright reserved, Gould/Orban/Toint, for GALAHAD productions
 *  Principal author: Jaroslav Fowkes & Nick Gould
 *
 *  History -
 *   originally released GALAHAD Version 5.5. April 15th 2026
 *
 *  For full documentation, see
 *   http://galahad.rl.ac.uk/galahad-www/specs.html
 */

/*! \mainpage GALAHAD C package bnls

  \section bnls_intro Introduction

  \subsection bnls_purpose Purpose

  This package uses a regularized Gauss-Newton method to solve the
   <b>bound-constrained nonlinear least-squares problem</b>
...
\manonly
  ...
  \n
\endmanonly
  where \f$x\f$ is required to lie in the box
  \f[x^l_j \leq x_j \leq x^u_j, \;\;\; j = 1, \ldots , n,\f]
\manonly
  \n
   x^l_j \[<=] x_j \[<=] x^u_j, ... , n,
  \n
\endmanonly
 ...
 \f$R^o\f$, and the \f$R^o\f$ non-negative diagonal weights\f$\w\f$ are given.
  ...
  \subsection bnls_authors Authors
  ...
  \subsection bnls_date Originally released

  April 2026.

  \subsection bnls_terminology Terminology
...
\endmanonly
  ...
  \f[(x-x_l)^T z_l + (x-x_u)^T z_u = 0,\hspace{12mm} \f]
\manonly
  \n
  (x-x_l)^T z_l + (x-x_u)^T z_u = 0,
  \n
\endmanonly
  where the vector inequalities hold component-wise.

  \subsection bnls_method Method

  Blah, blah, blah (see pdf docs)

  \subsection bnls_references Reference
  ...
  \subsection bnls_call_order Call order

  To solve a given problem, functions from the bnls package must be called
  in the following order:

  - \link bnls_initialize \endlink - provide default control parameters and
      set up initial data structures
  - \link bnls_read_specfile \endlink (optional) - override control values
      by reading replacement values from a file
  - set up problem data structures and fixed values by caling one of
     - \link bnls_import \endlink - in the case that \f$J_r(x)\f$ is explicitly
        available
     - \link bnls_import_without_jac \endlink - in the case that only the
        effect of applying \f$J_r\f$ and its transpose to a vector is possible
  - \link bnls_reset_control \endlink (optional) - possibly change control
      parameters if a sequence of problems are being solved
  - solve the problem by calling one of
     - \link bnls_solve_with_jac \endlink - solve using function calls to
       evaluate residual and Jacobian values
     - \link bnls_solve_with_jacprod \endlink - solve using function calls to
       evaluate residual values and Jacobian-vector products
     - \link bnls_solve_reverse_with_jac \endlink - solve returning to the
       calling program to obtain residual and Jacobian values, or
     - \link bnls_solve_reverse_with_jacprod \endlink - solve returning to the
       calling prorgram to obtain residual values and Jacobian-vector products
  - \link bnls_information \endlink (optional) - recover information about
    the solution and solution process
  - \link bnls_terminate \endlink - deallocate data structures
...
// include guard
#ifndef GALAHAD_BNLS_H
#define GALAHAD_BNLS_H
...
struct bnls_inform_type {
    ...
    /// at the best estimate the solution, x, determined by bnls_solve
    rpc_ obj;
    ...
    /// estimate, x, of the solution determined by bnls_solve
    rpc_ norm_pg;
...
// *-*-*-*-*-*-*-*-*-*-    B N L S  _ I N I T I A L I Z E    -*-*-*-*-*-*-*-*-*

void bnls_initialize( void **data,
...
/*!<
 Set default control values and initialize private data
  ...
  @param[out] control is a struct containing control information
              (see bnls_control_type)

  @param[out] inform  is a struct containing output information
              (see bnls_inform_type)

*/

// *-*-*-*-*-*-*-*-*-    B N L S  _ R E A D _ S P E C F I L E   -*-*-*-*-*-*-*

void bnls_read_specfile( struct bnls_control_type *control,
                        const char specfile[] );

/*!<
  Read the content of a specification file, and assign values associated
  with given keywords to the corresponding control parameters.
  By default, the spcification file will be named RUNBNLS.SPC and
  lie in the current directory.
  Refer to Table 2.1 in the fortran documentation provided in
  $GALAHAD/doc/bnls.pdf for a list of keywords that may be set.
...
// *-*-*-*-*-*-*-*-*-*-*-*-    B N L S  _ I M P O R T   -*-*-*-*-*-*-*-*-*-*

void bnls_import( struct bnls_control_type *control,
                  void **data,
                  ipc_ *status,
                  ipc_ n,
                  ipc_ m_r,
...
/*!<
 ...
 @param[in] control is a struct whose members provide control
  paramters for the remaining prcedures (see bnls_control_type)
...
// *-*-*-*-*-*-*-    B N L S  _ I M P O R T _ W I T H O U T _ A   -*-*-*-*-*-*

void bnls_import_without_jac( struct bnls_control_type *control,
                              void **data,
                              ipc_ *status,
                              ipc_ n,
                              ipc_ m_r);
...
*/
// *-*-*-*-*-*-*-    B N L S  _ R E S E T _ C O N T R O L   -*-*-*-*-*-*-*

void bnls_reset_control( struct bnls_control_type *control,
                         void **data,
                         ipc_ *status );
...
/*!<
 ...
 @param[in,out] status is a scalar variable of type ipc_, that gives
  ...
  \li  1. The import was succesful, and the package is ready for the solve phase
*/
//  *-*-*-*-*-*-*-*-*-  B N L S _ S O L V E _ W I T H _ J A C   -*-*-*-*-*-*-*-*

void bnls_solve_with_jac( void **data,
                          void *userdata,
                          ipc_ *status,
                          ipc_ n,
                          ipc_ m_r,
...
/*!<
 ...
 @param eval_jr is a user-supplied function that must have the following
   ...
   must be assigned to jr_val in the same order as presented to bnls_import, and
...
//  -*-*-*-*-*-*-   B N L S _ S O L V E _ W I T H _ J A C P R O D   -*-*-*-*-*-

void bnls_solve_with_jacprod( void **data,
                              void *userdata,
                              ipc_ *status,
                              ipc_ n,
                              ipc_ m_r,
...
//  -*-*-*-*-   B N L S _ S O L V E _ R E V E R S E _ W I T H  _ J A C  -*-*-*-

void bnls_solve_reverse_with_jac( void **data,
                                  ipc_ *status,
                                  ipc_ *eval_status,
                                  ipc_ n,
                                  ipc_ m_r,
...
*/
//  -*-   B N L S _ S O L V E _ R E V E R S E _ W I T H  _ J A C P R O D   -*-

void bnls_solve_reverse_with_jacprod( void **data,
                                      ipc_ *status,
                                      ipc_ *eval_status,
                                      ipc_ n,
                                      ipc_ m_r,
...
/*!<
 ...
 @param status (continued)
  \li  2. The user should compute the vector of residuals \f$r(x)\f$ at
         the point \f$x\f$ indicated in x and then re-enter the function.
         The required value should be set in r, and eval_status should be
         set to 0, and bnls_solve_reverse_with_jacprod re-entered with all 
  ...
  \li  4. The product \f$p = J_r(x) v\f$ of the residual Jacobian 
       ...
       be formed, v need not be set, but bnls_solve_reverse_with_jacprod 
  ...
  \li  5. The product \f$J_r(x)^Tv\f$ of the transpose of the residual Jacobian
       ...
       and bnls_solve_reverse_with_jacprod re-entered with all other arguments 
...
// *-*-*-*-*-*-*-*-*-*-    B N L S  _ I N F O R M A T I O N   -*-*-*-*-*-*-*-*

void bnls_information( void **data,
                       struct bnls_inform_type *inform,
                       ipc_ *status );
...
// *-*-*-*-*-*-*-*-*-*-    B N L S  _ T E R M I N A T E   -*-*-*-*-*-*-*-*-*-*

void bnls_terminate( void **data,
                     struct bnls_control_type *control,
                     struct bnls_inform_type *inform );
