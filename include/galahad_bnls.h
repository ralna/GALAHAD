...
/*
 * THIS VERSION: GALAHAD 5.1 - 2024-07-14 AT 15:00 GMT.
 *
...
\manonly
  ...
  \n
\endmanonly
  of many variables \f$\mathbf{x}\f$, that satisfy the bounds
  \f$\mathbf{x^l \leq x \leq x^u}\f$, involving positive weights
...
\endmanonly
  ...
  (see the \ref bnls_terminology section), and normally a two-norm
  ...
  - \link bnls_import \endlink - set up problem data structures and fixed
      values
  - \link bnls_reset_control \endlink (optional) - possibly change control
      parameters if a sequence of problems are being solved
  - solve the problem by calling one of
     - \link bnls_solve_with_mat \endlink - solve using function calls to
       evaluate function, gradient and Hessian values
     - \link bnls_solve_without_mat \endlink - solve using function calls to
       evaluate function and gradient values and Hessian-vector products
     - \link bnls_solve_reverse_with_mat \endlink - solve returning to the
       calling program to obtain function, gradient and Hessian values, or
     - \link bnls_solve_reverse_without_mat \endlink - solve returning to the
 ...
 */
struct bnls_subproblem_control_type {
...
struct bnls_control_type {
    ...
    struct bnls_subproblem_control_type subproblem_control;
 ...
 */
struct bnls_subproblem_inform_type {
...
struct bnls_inform_type {
    ...
    struct bnls_subproblem_inform_type subproblem_inform;
};
...
void bnls_solve_with_mat( void **data,
                         void *userdata,
                         ipc_ *status,
                         ipc_ n,
                         ipc_ m,
...
/*!<
 ...
 @param eval_j is a user-supplied function that must have the following
   ...
   be assigned to j in the same order as presented to bnls_import, and the
 ...
 @param eval_h is a user-supplied function that must have the following
   ...
   be assigned to h in the same order as presented to bnls_import, and the
   function return value set to 0. If the evaluation is impossible at x,
   return should be set to a nonzero value.
...
void bnls_solve_without_mat( void **data,
                            void *userdata,
                            ipc_ *status,
                            ipc_ n,
                            ipc_ m,
...
void bnls_solve_reverse_with_mat( void **data,
                                 ipc_ *status,
                                 ipc_ *eval_status,
                                 ipc_ n,
                                 ipc_ m,
...
/*!<
 ...
 @param[in,out] status is a scalar variable of type ipc_, that gives
  ...
  \li   3. The user should compute the Jacobian of the vector of residual
         ...
         \f$J\f$ in the earlier call to bnls_import should be set in J_val[l],
 ...
 @param status (continued)
  \li   4. The user should compute the matrix
         ...
         stored according to the scheme specified for the remainder of
         \f$H\f$ in the earlier call to bnls_import should be set in H_val[l],
  ...
  \li   7. The user should compute the entries of the matrix \f$P\f$,
         ...
         stored according to the scheme specified for the remainder of
         \f$P\f$ in the earlier call to bnls_import should be set in P_val[l],
...
void bnls_solve_reverse_without_mat( void **data,
                                    ipc_ *status,
                                    ipc_ *eval_status,
                                    ipc_ n,
                                    ipc_ m,
