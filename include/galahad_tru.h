...
/*! \mainpage GALAHAD C package tru

  \section tru_intro Introduction

  \subsection tru_purpose Purpose

  The tru package uses a <b>trust-region method to find a (local)
  ...
  \subsection tru_authors Authors
  ...
  \subsection tru_date Originally released

  July 2008, C interface August 2021.

  \subsection tru_terminology Terminology
  ...
  \f$m_k(s)\f$ of \f$f(x_k + s)\f$ within a trust region
  \f$\|s_k\| \leq \Delta_k\f$  for some specified positive
  "radius" \f$\Delta_k\f$. The quality of the
  ...
  \f$\eta_s > 0\f$, and in this case \f$x_{k+1} = x_k + s_k\f$. Otherwise
  ...
  Either linear or quadratic models \f$m_k(s)\f$ may be used. The former
  will be taken as the first two terms \f$f(x_k) + s^T \nabla_x f(x_k)\f$
  ...
  Lagrange multiplier \f$\lambda_k\f$.
  ...
  \subsection tru_references Reference
  ...
  \section tru_call_order Call order

  To solve a given problem, functions from the tru package must be called
  in the following order:

  - \link tru_initialize \endlink - provide default control parameters and
      set up initial data structures
  - \link tru_read_specfile \endlink (optional) - override control values
  ...
  - solve the problem by calling one of
     ...
     - \link tru_solve_without_mat \endlink - solve using function calls to
       evaluate function and gradient values and Hessian-vector products
     - \link tru_solve_reverse_with_mat \endlink - solve returning to the
       calling program to obtain function, gradient and Hessian values, or
     - \link tru_solve_reverse_without_mat \endlink - solve returning to the
       calling prorgram to obtain function and gradient values and
       Hessian-vector products
  - \link tru_information \endlink (optional) - recover information about
    the solution and solution process
  - \link tru_terminate \endlink - deallocate data structures
...
struct tru_control_type {
    ...
    /// \brief the trust-region norm used.
    ...
    /// \f$f - f(x_{new})\f$ is larger than .eta_successful times that predicted
    ...
    /// \brief
    /// is a retrospective strategy to be used to update the trust-region
    /// radius?
...
/*!<
  ...
  By default, the spcification file will be named RUNTRU.SPC and
...
/*!<
 ...
 @param eval_h is a user-supplied function that must have the following
   ...
   as presented to tru_import, and the function return value set to 0.
...
//  *-*-*-*-*-*-*-*-   T R U _ S O L V E _ W I T H O U T _ M A T   -*-*-*-*-*-*

void tru_solve_without_mat( void **data,
                            void *userdata,
                            ipc_ *status,
                            ipc_ n,
                            rpc_ x[],
...
//  *-*-*-*-*-   T R U _ S O L V E _ R E V E R S E _ W I T H _ M A T   -*-*-*-*

void tru_solve_reverse_with_mat( void **data,
                                 ipc_ *status,
                                 ipc_ *eval_status,
                                 ipc_ n,
                                 rpc_ x[],
...
//  *-*-*-   T R U _ S O L V E _ R E V E R S E _ W I T H O U T _ M A T   -*-*-*

void tru_solve_reverse_without_mat( void **data,
                                    ipc_ *status,
                                    ipc_ *eval_status,
                                    ipc_ n,
                                    rpc_ x[],
