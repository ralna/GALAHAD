...
Thus a call to ``llsr_initialize`` below will instead be

.. ref-code-block:: cpp
	:class: doxyrest-title-code-block

	void llsr_initialize_s_64(void **data, struct :ref:`llsr_control_type_s_64<doxid-structllsr__control__type>`* control, 
                             int64_t *status)
...
.. list-table::
	...
	*
		...
		- is a one-dimensional array of type char that specifies the :ref:`unsymmetric storage scheme<details-llsr_storage__unsym>` used for the constraint Jacobian, $A$ if any. It should be one of 'coordinate', 'sparse_by_rows' or 'dense'; lower or upper case variants are allowed.
...
.. list-table::
	...
	*
		...
		- is a one-dimensional array of type char that specifies the :ref:`symmetric storage scheme<details-llsr_storage__sym>` used for the matrix $S$. It should be one of 'coordinate', 'sparse_by_rows', 'dense' or 'diagonal'; lower or upper case variants are allowed.
