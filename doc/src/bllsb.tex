...
The bound-constrained least-squares solution algorithm is called as follows:
\vspace*{1mm}
...
and set values for all components except,
if they are not required, {\tt prob\%W} and {\tt prob\%X\_s}.
...
content with merely setting {\tt prob\%X=0.0} and {\tt prob\%Z=0.0}.
%The component {\tt prob\%C} need not be set on entry.

On exit, the components {\tt prob\%X} , {\tt prob\%R} and {\tt prob\%Z}
will contain the best estimates of the primal variables $\bmx$,
the residuals $\bmr = \bmA_o \bmx - \bmb$,
and dual variables for the bound constraints $\bmz$, respectively.
...
\eqn{4.2a}{
 \bmA_o^T W ( \bmA_o \bmx - \bmb) + \sigma ( \bmx - \bmx_s ) = \bmz,}
where
\eqn{4.2b}{
 \bmz = \bmz^{l} + \bmz^{u}, \,\,
