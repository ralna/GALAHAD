...
\itt{model} is a scalar variable of type \integer, that specifies
...
\itt{norm} is a scalar variable of type \integer,
...
\itt{max\_dxg} is a scalar variable of type \integer,
...
\itt{lbfgs\_vectors} is a scalar variable of type \integer,
...
\itt{icfs\_vectors} is a scalar variable of type \integer,
...
\itt{mi28\_lsize} is a scalar variable of type \integer,
...
\itt{mi28\_rsize} is a scalar variable of type \integer,
...
\itt{advanced\_start} is a scalar variable of type \integer that
...
\itt{stop\_g\_absolute} is a scalar variable of type \realdp,
...
\itt{stop\_g\_relative} is a scalar variable of type \realdp,
...
\itt{cg\_iter} is a scalar variable of type \integer, that gives the
...
\itt{weight} is a scalar variable of type \realdp, that holds
...
user-supplied subroutine whose purpose is to evaluate the value of the
product $\bmP(\bmx)^{-1}\bmv$ of the inverse of the user's preconditioner 
with a given vector $\bmv$. See Section~\ref{pv} for details.
...
\itt{userdata} is a scalar \intentinout\ argument of type
{\tt SERDATA\_type} whose components may be used
to communicate user-supplied data to and from the
subroutines {\tt eval\_F}, {\tt eval\_G},
{\tt eval\_H}, {\tt eval\_HPROD} and {\tt eval\_PREC}
(see Section~\ref{typeuserdata}).
...
user is expected to provide a subroutine of that name to evaluate the
product $\bmu = \bmP(\bmx)^{-1} \bmv$ involving the inverse of the user's 
preconditioner $\bmP(\bmx)$ with a given vector $\bmv$. 
The symmetric matrix $\bmP(\bmx)$ should ideally be chosen so that the 
eigenvalues of $\bmP(\bmx) (\nabla_{xx} f(\bmx))^{-1}$ are clustered.
The routine must be specified as
...
\ittf{6.} The user should compute the product $\bmu = \bmP(\bmx)^{-1}\bmv$
     of the inverse of their preconditioner $\bmP(\bmx)$ at the point $\bmx$
     indicated in {\tt nlp\%X} with the vector $\bmv$.
     The vectors $\bmv$ is given in {\tt data\%V}, the
     resulting vector $\bmu = \bmP(\bmx)^{-1} \bmv$ should be set in 
     {\tt data\%U} and {\tt data\%eval\_status}
...
and
{\tt control\%GLRT\_control\%print\-\_\-level},
