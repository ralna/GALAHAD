...
\newcommand{\package}{nrek}
\newcommand{\packagename}{NREK}
...
a real $n$ vector $\bmc$ and scalars $f$, $p>2$  and $\sigma>0$, this package 
uses an {\bf extended Krylov-subspace method} to find a
{\bf global minimizer of the norm-regularized quadratic objective function
$\half \bmx^T \bmH \bmx  +  \bmc^T \bmx + f + 
\sfrac{1}{p} \sigma \|\bmx\|_{\bmS}^p$,}
...
{\tt \libraryname\_RQS}, {\tt \libraryname\_SLS}, 
{\tt \libraryname\_MOP}
...
\itt{increase} is a scalar variable of type \realdp, that holds the value
of the increase factor for suggested subsequent regularization weights
(see {\tt control\%next\_weight}).
The suggested weight will be {\tt reduction} times the
current weight.
The default is {\tt increase = 2.0}.
...
although only {\tt 'sytr'}, {\tt 'potr'}, {\tt 'pbtr'} and {\tt 'slblt'} are installed by default.
See the documentation for the \galahad\ package {\tt SLS} for further details.
The default is {\tt linear\_solver = 'slblt'}, but we recommend
...
{\tt RQS\_control\_type} that is used to control various aspects of the
diagonal norm-regularization solver from the package {\tt RQS}. See the 
documentation for {\tt \libraryname\_RQS} for more details.
...
\itt{obj\_regularized} is a scalar variable of type \realdp, that holds the
value of the regularized objective function 
$\half \bmx^T \bmH \bmx  + \bmc^T \bmx + \sfrac{1}{p}\sigma \|\bmx\|^p$.
...
value of the Lagrange multiplier $\lambda = \sigma \|\bmx\|^{p-2}$
associated with the regularization.

\itt{weight} is a scalar variable of type \realdp, that holds the value of
the current regularization weight.

\itt{next\_weight} is a scalar variable of type \realdp, that holds the value 
of the proposed next weight to be used if the current weight proves to be
too small (see {\tt inform\%increase}).
...
\itt{RQS\_inform} is a scalar variable of type {\tt RQS\_inform\_type},
that holds informational parameters concerning the 
diagonal norm-regularization subroutine
contained in the \galahad\ refinement package {\tt RQS}.
See the documentation for the package {\tt RQS} for details of the
derived type {\tt RQS\_inform\_type}.
...
The norm-regularization solution algorithm is called as follows:

\vspace*{2mm}
\hskip0.5in
{\tt CALL \packagename\_solve( n, H, C, power, weight, X, data, control, inform[, S])}
...
\itt{power} is a scalar \intentin\ variable of type \realdp,
that must be set on initial entry to the power of the regularization, $p$.
{\bf Restriction: } $p > 2$.

\itt{weight} is a scalar \intentin\ variable of type \realdp,
...
\hline
  ...
  {\tt weight-increase-factor} & {\tt \%increase} & real \\
...
\galrestrictions $n  >  0,\;  p > 2, \;  \sigma  >  0$.
...
where $\lambda_* = \sigma \|\bmx_*\|^{p-2}$ is a Lagrange
multiplier corresponding to the regularization.
...
(iv) solutions to the regularization subproblem restricted to elements
...
starting with a regularization weight $\sigma = 10$ and order $p = 3$,
but then gradually doubling the weight
