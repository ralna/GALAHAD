...
\def\baselinestretch{0.8}
{\tt \begin{verbatim}
       SUBROUTINE eval_ASPROD( status, userdata, V, P[, NZ_v, nz_v_start, nz_v_end,     &
                               NZ_p, nz_p_end] )
\end{verbatim} }
\def\baselinestretch{1.0}
\noindent whose arguments are as follows:
...
{\tt NZ\_p}
(see below) are \present,
only the nonzero components {\tt NZ\_p(1:nz\_p\_end)} of {\tt P}
need be assigned.
...
\itt{nz\_p\_end} is an \optional\ scalar variable of type default \integer
that, if \present, must be set to record the number of non-zeros in
$\bmA \bmv$.

\itt{NZ\_p} is an \optional\ rank-one allocatable array of dimension $m$
...
\ittf{1.} The user should compute ones of the sums $\bmp + \bmA \bmv$
...
\ittf{2.} The user should compute the nonzero components of the
     ...
     {\tt reverse\%NZ\_p(1:reverse\%nz\_p\_end)} of {\tt reverse\%P},
     and the components \linebreak
     {\tt reverse\%NZ\_p} and {\tt reverse\%nz\_p\_end} must be set.
