...
subject to the {\bf simple bound constraints}
\disp{x_{j}^{l}  \leq  x_{j}^{ } \leq  x_{j}^{u} , \;\;\; j = 1, \ldots , n,}
where the $o$ by $n$ real {\bf design matrix} $\bmA_o$, the vectors 
of {\bf observations} $\bmb$ and 
{\bf variable bounds} $\bmx^{l}$ and $\bmx^{u}$, the {\bf shifts} $\bmx_s$, 
and the non-negative {\bf weights} $\bmw > 0$ and $\sigma \geq 0$ are given, 
...
may be used to hold the starting position in {\tt IV} (see below)
of the list of indices of nonzero components of $\bmv$.

\itt{lvu} is a scalar variable of type \integer, that
may be used to hold the finishing position in {\tt IV} (see below)
of the list of indices of nonzero components of $\bmv$.
...
is used to record the finishing position in {\tt IP} (see below)
...
{\tt IP(1:lp)} of {\tt P} should then be nonzero.
...
Inappropriate initial values may be altered, so the user should
...
\ittf{4.} The user should compute the matrix-vector product $\bm = \bmA_o \bmv$
     using the vector $\bmv$ whose nonzero components are stored in positions
     {\tt reverse\%IV(reverse\%lvl:reverse\%lvu)}
     of {\tt reverse\%V}, and place the result in {\tt reverse\%P}.
...
\ittf{5.} The user should compute the nonzero components of the
     matrix-vector product $\bmp = \bmA_o \bmv$ using the vector $\bmv$ whose
     ...
     occupy positions
     {\tt reverse\%IP(1:reverse\%lp)} of {\tt reverse\%P},
     and the components
     {\tt reverse\%IP} and {\tt reverse\%lp} must be set.
     The component {\tt reverse\%eval\_status} should set to 0 unless the
     user is unable to evaluate the product, in which case
     {\tt reverse\%eval\_status} should be set to a non-zero value, and the
      remaining components left unaltered.
...
% {\tt prob\%Ao\_val}, {\tt prob\%Ao\_row} or {\tt prob\%Ao\_col},
% is not large enough to hold the original, or reordered, matrix $\bmA_o$.
...
\eqn{4.2a}{
 \bmA_o^T \bmW (\bmA_o \bmx - \bmb ) + \sigma \bmx = \bmz
 \tim{and}
 \bmz = \bmz^{l} + \bmz^{u} ,}
and
\eqn{4.2b}{
