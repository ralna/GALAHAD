...
\galmatrix
Both the objective Jacobian matrix $\bmA_o$ and
...
that holds the objective Jacobian matrix $\bmA_o$.
...
and involves an objective Jacobian we wish to store using the co-ordinate
storage scheme, we may simply
%\vspace*{-2mm}
{\tt
\begin{verbatim}
        CALL SMT_put( prob%A%type, 'COORDINATE', istat )
\end{verbatim}
}
%\vspace*{-4mm}
\noindent
...
holds the values of the entries of the Jacobian matrix $\bmA$ in any of the
...
default \integer, that holds the starting position of each row of $\bmA$, 
...
\hspace{8mm}
{\tt CALL \packagename\_solve( prob, data, control, inform[, w, regularization\_weight] )}
...
\disp{\bmA = \mat{ccc}{1 & 1 & \\ & 1 & 1 \\ 1 &  & 1  \\ & 1 & }, \;\;
\bmb = \vect{ 2 \\ 2 \\ 3 \\ 1 }, \;\;
\bmw = \vect{ 1 \\ 1 \\ 1 \\ 2 }, \;\;
...
! sparse row-wise storage format
   ...
   p%Ao%col = (/ 1, 2, 2, 3, 1, 3, 2 /)  ! Jacobian Ao                    
   p%Ao%ptr = (/ 1, 3, 5, 7, 8 /) 
