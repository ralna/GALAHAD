...
\eqn{qp}{\mbox{minimize}\;\; q(\bmx) = \half \| \bmA_o \bmx - \bmb\|_W^2
 + \half \sigma \| \bmx - \bmx_s\|^2}
subject to the {\bf general linear constraints}
\disp{c_{i}^{l}  \leq  \bma_{i}^{T}\bmx  \leq  c_{i}^{u}, \;\;\;
 i = 1, \ldots , m,}
and the {\bf simple bound constraints}
\disp{x_{j}^{l}  \leq  x_{j}^{ } \leq  x_{j}^{u} , \;\;\; j = 1, \ldots , n,}
where the norms $\|r\|_W = \sqrt{\sum_{i=1}^o w_i r_i^2}$
and $\|x\| = \sqrt{\sum_{i=1}^n x_i^2}$,
and the $o$ by $n$ real {\bf design matrix} $\bmA_o$, the
vector of {\bf observations} $\bmb$, as well as 
the {\bf constraint rows} $\bma_{i}$ and vectors of {\bf constraint bounds}
$\bmc^{l}$, $\bmc^{u}$, $\bmx^{l}$, $\bmx^{u}$, the {\bf shifts} $\bmx_s$, 
and the {\bf weights} $w > 0$ and $\sigma \geq 0$ 
are given. Any of the constraint bounds $c_{i}^{l}$, $c_{i}^{u}$,
