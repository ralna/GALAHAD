...
or even the general, regularized, linear-least squares objective,
\eqn{rls}{\half \| \bmA_o \bmx - \bmb\|_W^2
...
or the non-overlapping regular simplex constraints
\eqn{sc}{\bme_{\calC_i}^T \bmx_{\calC_i} = 1 \tim{and} \bmx_{\calC_i}
 \geq 0 \tim{for} i = 1,\ldots,m_c,}
where $\|\bmv\|_W^2 \equiv \sum_i w_i v_i^2$, and
the $n$ by $n$ symmetric matrix $\bmH$, the  $o$ by $n$ 
design matrix $\bmA_o$, the vectors $\bmg$, $\bmw$, $\bmx^{0}$,
$\bma_{i}$, $\bmc^{l}$, $\bmc^{u}$, $\bmx^{l}$, $\bmx^{u}$, $\bmb$
and $\bmx_s$, the scalars $f$ and $\sigma$, the non-overlapping
integer sets of cohorts $\calC_i \subseteq \{1,\ldots,n\}$, for which
$\calC_i \cap \calC_j = \emptyset$ for $1 \leq i < j \leq m_c$,
are given.
Full advantage is taken of any zero coefficients in the matrices 
$\bmH$, $\bmA_o$ or the vectors $\bma_{i}$.
...
\galmatrix
The Hessian matrix $\bmH$,
the constraint Jacobian $\bmA$, the matrix
whose rows are the vectors $\bma_{i}^{T}$, $i = 1, \ldots , m$,
and the design matrix $\bmA_o$
may be stored in a variety of input formats.

\subsubsection{Dense storage format}\label{dense}
The matrix $\bmA$ (and, if required, $\bmA_o$) is stored as a compact
...
The required solution $\bmx$ to \req{qp} or \req{lsqp}
...
For the problem \req{rls} with constraints \req{sc}, the primal
optimality conditions are instead
\disp{\bme_{\cal C_i}^T \bmx_{\cal C_i} = 1 \tim{and} \bmx_{\cal C_i}
 \geq 0 \tim{for} i = 1,\ldots,m,}
the dual optimality conditions become
\disp{\bmA_o^T \bmW (\bmA_o \bmx - \bmb ) + \sigma (\bmx -\bmx_s)  = 
...
that holds a string which indicates the storage scheme used.
...
\itt{new\_problem\_structure} is a scalar variable of type default \logical,
 ...
 the sparsity patterns of the Jacobian and design matrices $\bmA$ and
 $\bmA_o$ involved
...
\ittf{m} is a scalar variable of type \integer,
 that holds the number of general linear or simplex constraints, $m$.

\ittf{o} is a scalar variable of type \integer,
 that holds the number of residuals, $o$, if required.
...
\itt{2} In this case, a weighted least-distance problem of the form \req{lsqp}
...
\itt{WEIGHT} is a rank-one allocatable array of type \realdp, that
...
infinity. 
When the constraints are non-overlapping regular simplices, \req{sc},
{\tt C\_l} need not be allocated.
...
that holds the design matrix $\bmA_o$. The following components are used:
...
Just as for {\tt Ao\%type} above, the procedure {\tt SMT\_put}
may be used to allocate sufficient space and insert the required keyword
into {\tt Ao\%type}.
Once again, if {\tt prob} is of derived type {\tt \packagename\_problem\_type}
and involves a design matrix we wish to store using the sparse row-wise
storage scheme, we may simply
%\vspace*{-2mm}
{\tt
\begin{verbatim}
        CALL SMT_put( prob%Ao%type, 'SPARSE_BY_ROWS' )
\end{verbatim}
}
%\vspace*{-4mm}
\noindent
...
\itt{Ao\%val} is a rank-one allocatable array of type \realdp, that holds
the values of the entries of the design matrix $\bmA_o$ in any of the
appropriate storage schemes discussed in Section~\ref{galmatrix}.
...
\integer, that holds the starting position of each row of $\bmA_o$, as well
as the total number of entries plus one, in the sparse row-wise storage
scheme (see Section~\ref{rowwise}). It need not be allocated when the
other appropriate schemes are used.
...
filled with the value $b_{i}$ for $i = 1, \ldots , o$ for the
objective function \req{rls}. 
For other objectives, {\tt B} need not be allocated.
...
\realdp, that holds the values $\bmA \bmx$ of the constraints.
...
\realdp, that holds the values $\bmA_o \bmx - \bmb$ of the residuals.
The $i$-th component of {\tt R}, $i = 1,  \ldots ,  o$, contains
$(\bmA_o \bmx - \bmb)_{i}$.
...
\itt{COHORT} is a rank-one array of type \integer, that should
 be allocated to have length {\tt n} if the a simplex constraint set 
\req{sc} is used, and if every variable is not contained in a single
cohort. In this case, if variable $x_j$ is associated with cohort $\calC_i$, 
$1 \leq i \leq m$, {\tt COHORT(j)} should be set to {\tt i}, while 
if $x_j$ is unconstrained {\tt COHORT(j) = 0} should be assigned. 
At least one value {\tt COHORT(j)} for $j = 1,\ldots\,n$ is expected to 
take the value $i$ for every $1 \leq i \leq m$, that is no empty cohorts 
are allowed. If all the variables are contained in a single cohort, 
or if the constraint set is not \req{sc}, {\tt COHORT} need not be allocated.
...
in the objective \req{rls} is not one, and in this case
filled with the value $w_{i}$ for $i = 1, \ldots , o$.
If all the weights are one, and for other objectives, {\tt W} 
...
in the objective \req{rls} is nonzero, and in this case
filled with the value $(\bmx_s)_{i}$ for $i = 1, \ldots , n$.
If all the shifts are zero, and for other objectives, {\tt X\_s} 
need not be allocated.

\itt{infinity} is a scalar variable of type \realdp,
that holds the value of infinity, that is the value beyond which
problem bounds are judged to be infinite.
