...
Full advantage is taken of any zero coefficients in the vectors $\bma_{i}$. 
...
\itt{find\_dependent} is a scalar variable of type default \real, that gives
 the time spent detecting and removing linearly-dependent equality constraints
...
\itt{phase1\_total} is a scalar variable of type default \real, that gives
 the total time spent in the initial-feasible-point phase of the package.

\itt{phase1\_analyse} is a scalar variable of type default \real, that gives
 the time spent analysing the required matrices prior to factorization
 in the initial-feasible-point phase.

\itt{phase1\_factorize} is a scalar variable of type default \real, that gives
 the time spent factorizing the required matrices
 in the initial-feasible-point phase.

\itt{phase1\_solve} is a scalar variable of type default \real, that gives
 the time spent computing the search direction
 in the initial-feasible-point phase.
...
is appropriate for $\bmA$ for their application---different 
...
input format.
\restrictions {\tt p\%n} $> 0$, {\tt p\%m} $\geq 0$ and
              {\tt p\%A\%ne} $\geq -2$.
...
\itt{-1.} One of the restrictions {\tt prob\%n} $> 0$ or {\tt prob\%m} $\geq  0$
...
\itt{-3.} A deallocation error occured. A message indicating the offending 
array is written on unit {\tt control\%error} and the returned allocation 
status and a string containing the name of the offending array
are held in {\tt inform\%alloc\_\-status}
and {\tt inform\%bad\_alloc} respectively.
...
\itt{-11.} The objective function appears to be unbounded from below on the
    feasible set.

\itt{-12.} The CPU time limit has been reached. This may happen if
    {\tt cpu\_time\_limit} is too small, but may also be symptomatic of 
    a badly scaled problem.
...
{\tt GALAHAD\_SILS}, but options at this stage are
