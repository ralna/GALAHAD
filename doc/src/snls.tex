...
{\tt nlp\%Jr\%type} $\in \{
  \mbox{\tt 'DENSE'}, {\tt 'DENSE\_BY\_ROWS'}, {\tt 'DENSE\_BY\_COLUMNS'},
...
\[\bmJ_r^T(\bmx) \bmW \bmr(\bmx) = \sum_{i=1}^{m_c} \bme_{\calC_i} y_i + \bmz\]
necessarily hold at an optimal point $\bmx$ for some Lagrange multipliers 
$\bmy$ and dual variables $\bmz \geq 0$, where additionally $\bmx$ and $\bmz$ 
