...
\]
and possibly regular simplex constraints
...
function from $\Re^n$ into $\Re^m$, the inequalities are understood
componentwise, and the non-overlapping integer
sets of cohorts $\calC_i \subseteq \{1,\ldots,n\}$, for which
$\calC_i \cap \calC_j = \emptyset$ for $1 \leq i < j \leq m_c$, are given.
The vectors $\bmc^l \leq \bmc^u$ and $\bmx^l \leq \bmx^u$ are
...
when the problem is unconstrained ($m=0$), which allows us
to use the same symbol $\bmH$ for both cases. The package also supports
weighted least-squares objectives
\[f(\bmx) = \half \sum_{i=1}^{o} w_i r^2_i(\bmx) \equiv \half \|\bmr(\bmx)\|_W^2,
\]
and their associated $m_r$ by $n$ Jacobian 
$\bmJ_r(\bmx) \equiv \nabla_{\bmx} \bmr(\bmx)$, if necessary.
...
\galcalls {\tt GALAHAD\_TOOLS}, {\tt \libraryname\_SMT}.
\galdate May 2003.
...
\galmatrix
Both the Hessian matrix $\bmH$ and the Jacobians $\bmJ$ and $\bmJ_r$ may
each be stored in one of three input formats (the format for the matrices
being possibly different).

\subsubsection{Dense storage format}\label{dense}

The matrix $\bmJ$ (or $\bmJ_r$) is stored as a compact 
...
\subsubsection{The derived data type for holding optimization problems}
\label{typeprob}
...
\ittf{m\_a} is a scalar variable of type \integer, 
 that holds the number of general linear constraints if the user wishes
to distinguish these from $m$.
...
\ittf{m\_r} is a scalar variable of type \integer, 
 that holds the number of residuals, $m_r$, in a least-squares objective.

\ittf{r} is a rank-one allocatable array of dimension {\tt o} and type default 
\realdp, that holds the values $\bmr(\bmx)$ of the residuals in a least-squares
objective. The $i$-th component of {\tt r}, $i = 1,  \ldots ,  o$, contains 
$\bmr_{i}(\bmx)$.  

\ittf{w} is a rank-one allocatable array of dimension {\tt o} and type default 
\realdp, that holds the values $\bmw$ of the weights for a least-squares
objective. The $i$-th component of {\tt w}, $i = 1,  \ldots ,  o$, contains 
$\bmw_{i}$.  

\itt{Jr\_type} is a scalar variable of type \integer, 
that specifies the type of storage used for the least-squares Jacobian $\bmJ_r$.
Possible values are {\sym GALAHAD\_DENSE}, {\sym GALAHAD\_COORDINATE} 
or {\sym GALAHAD\_SPARSE\_BY\_ROWS}.
              
\itt{Jr\_ne} is a scalar variable of type \integer, 
that holds the number of non-zero entries in the 
least-squares Jacobian $\bmJ_r$.

\itt{Jr\_val} is a rank-one allocatable array of type \realdp, that holds
the values of the entries of the least-squares Jacobian matrix $\bmJ_r$ 
in any of the storage schemes discussed in Section~\ref{galmatrix}.

\itt{Jr\_row} is a rank-one allocatable array of type \integer,
that holds the row indices of $\bmJ_r$ in the sparse co-ordinate storage
scheme (see Section~\ref{coordinate}). 
It need not be allocated for either of the other two schemes.

\itt{Jr\_col} is a rank-one allocatable array variable of type \integer,
that holds the column indices of $\bmJ_r$ in either the sparse co-ordinate 
(see Section~\ref{coordinate}), or the sparse row-wise 
(see Section~\ref{rowwise}) storage scheme.
It need not be allocated when the dense storage scheme is used.

\itt{Jr\_ptr} is a rank-one allocatable array of dimension {\tt o+1} and type 
\integer, that holds the 
starting position of each row of $\bmJ_r$, as well
as the total number of entries plus one, in the sparse row-wise storage
scheme (see Section~\ref{rowwise}). It need not be allocated when the
other schemes are used.

\ittf{m\_c} is a scalar variable of type \integer, 
 that holds the number of simplex cohorts, $m_c$.

\itt{cohort} is a rank-one allocatable array of dimension {\tt n} and type 
 \integer, that lists the simplex cohort for each variable if there are
one or more non-overlapping simplex constraints of the form \req{sc}.
In this case, if variable $x_j$ is associated with cohort $\calC_i$, 
...
\itt{H, J, Jr} are scalar variable of type {\tt SMT\_TYPE} that provide
compact alternatives to the arrays {\tt H\_*}, {\tt J\_*} and {\tt Jr\_*}, 
described above.
...
\galmodules {\tt \packagename} calls the {\tt TOOLS} and {\tt SMT}
\galahad\ modules.
