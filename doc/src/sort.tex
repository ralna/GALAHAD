...
\hspace{8mm}
{\tt CALL \packagename\_heapsort\_build ( n, x, inform [, ix, rx, largest ] )}
...
\hspace{8mm}
{\tt CALL \packagename\_heapsort\_smallest ( m, x, inform [, ix, rx, largest ] )}
...
\itt{rx} is an optional rank-one \intentinout\ array argument of
dimension at least {\tt m} and type \realdp.
If {\tt rx} is present, exactly the same permutation is applied to the
components of {\tt rx} as to the components of {\tt x}.
...
\]
Suppose now that we wish to perform the following successive operations:
\begin{enumerate}
...
permutation,
\item invert this permutation (which yields the permutation used to sort the
