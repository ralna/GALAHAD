...
\disp{a_{i}^{l}  \leq  \bma_{i}^{T}\bmx  \leq  a_{i}^{u}, \;\;\; 
 i = 1, \ldots , m_a,}
...
\itt{verify\_level} is a scalar variable of type \integer\
...
\itt{f\_availability} is a scalar variable of type \integer\
...
\itt{c\_availability} is a scalar variable of type \integer\
...
\itt{J\_availability} is a scalar variable of type \integer\
...
\itt{H\_availability} is a scalar variable of type \integer\
...
\itt{numG\_wrong} is a scalar variable of type \integer\ that gives the
number of components of the gradient of the objective function that
appear to be wrong.

\itt{numJ\_wrong} is a scalar variable of type \integer\ that gives the
number of entries of the Jacobian of the constraint function that
appear to be wrong.

\itt{numH\_wrong} is a scalar variable of type \integer\ that gives the
...
\itt{Jval} is a rank-one allocatable array of type \realdp\ in
which the user places the entries of the Jacobian matrix evaluated at
{\tt X}, when required (see Section~\ref{reverse}). 

\itt{Hval} is a rank-one allocatable array of type \realdp\ in
...
\ittf{C} is a rank-one \intentout\ argument of type \realdp\
...
\itt{Jval} is a scalar \intentout\ argument of type \realdp,
...
\ittf{Y} is a rank-one \intentin\ array argument of type \realdp\
