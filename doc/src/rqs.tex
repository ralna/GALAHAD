...
finds a {\bf global minimizer of the norm-regularised quadratic objective
function
...
\itt{stop\_normal} and {\tt stop\_hard} are scalar variables of type \realdp,
...
\itt{len\_history} is a scalar variable of type \integer, that gives
...
\itt{p} is a scalar \intentin\ variable of type \realdp,
that must be set on initial entry
to the order of the regularisation, $p$.
{\bf Restriction: } $p > 2$.

\itt{sigma} is a scalar \intentin\ variable of type \realdp,
that must be set on initial entry
...
be positive definite, but in special ``hard'' cases singularity is 
a possibility.
...
$\lambda = \sigma \|\bmx(\lambda)\|_M^{p-2}$ with
...
efficient sparse Cholesky factorization with precautions to detect
indefinite $\bmH + \lambda \bmM$ is used. If $\bmA \bmx  = 0$ is required,
