...
\newcommand{\package}{trs}
\newcommand{\packagename}{TRS}
...
a real $n$ vector $\bmc$ and scalars $\Delta>0$ and $f$, this package finds a
{\bf global minimizer of the quadratic objective function
$\half \bmx^T \bmH \bmx  +  \bmc^T \bmx + f$, where the vector $\bmx$ is
required to satisfy the constraint $\|\bmx\|_{\bmM} \leq  \Delta$
and possibly $\bmA \bmx = \bmzero$}, and
...
$\ell_2$-trust-region case for which $\bmM = \bmI$, the $n$ by $n$
identity matrix.

\noindent
Factorization of matrices of the form $\bmH + \lambda \bmM$---or
\eqn{kkt}{\mat{cc}{ \bmH + \lambda \bmM & \bmA^T \\ \bmA & \bmzero}}
...
the purpose. The same applies to $\bmM$ if it is required, but again, 
...
no explicit entries needs be stored. The same is true for $\bmM$ but
not for the non-square $\bmA$.

\subsubsection{Zero-matrix storage format}\label{zero}
If $\bmH = \bmzero$ (i.e., $h_{ij} = 0$ for all $1 \leq i, j \leq n$),
no explicit entries needs be stored. The same applies both to $\bmM$ and $\bmA$,
although there would seem little real justification for using $\bmA = 0$.
...
\itt{dense\_factorization} is a scalar variable of type \integer,
...
\itt{new\_m} is a scalar variable of type \integer, that is used
to indicate how $\bmM$ (if required) has changed (if at all) since the previous
call to {\tt \packagename\_solve}. Possible values are:
\begin{description}
\itt{0} $\bmM$ is unchanged.
\itt{1} the values in $\bmM$ have changed, but its nonzero structure
is as before.
\itt{2} both the values and structure of $\bmM$ have changed.
\end{description}
...
\itt{max\_factorizations} is a scalar variable of type \integer,
...
\itt{inverse\_itmax} is a scalar variable of type \integer, that holds
...
\itt{taylor\_max\_degree} is a scalar variable of type \integer,
...
\itt{stop\_normal,} {\tt stop\_absolute\_normal}
...
The defaults are {\tt stop\_normal = \tt stop\_absolute\_normal = stop\_hard =}
$u^{0.75}$,
where $u$ is {\tt EPSILON(1.0)} ({\tt EPSILON(1.0D0)} in
{\tt \fullpackagename\_double}).

\itt{start\_invit\_tol} is a scalar variable of type \realdp, that holds the
...
\itt{start\_invitmax\_tol} is a scalar variables of type \realdp, that holds the
...
\itt{initialize\_approx\_eigenvector} is a scalar variable of type default
...
\itt{force\_Newton} is a scalar variable of type default \logical, that
may be set \true\ if the user wishes the package to ignore the
trust-region bound when $\bmH$ is positive definite, and thus to compute
the Newton step in this case, and \false\ otherwise.
The default is {\tt force\_Newton = \false}.
...
Current possible
choices are {\tt 'sils'}, {\tt 'ma27'}, {\tt 'ma57'}, {\tt 'ma77'},
{\tt 'ma86'}, {\tt 'ma87'}, {\tt 'ma97'}, {\tt slblt}, {\tt 'pardiso'}
and {\tt 'wsmp'},
although only {\tt 'sils'} and {\tt 'slblt'} are installed by default.
See the documentation for the \galahad\ package {\tt SLS} for further details.
...
are:
\begin{description}
\itt{lambda} is a scalar variable of type \realdp, that gives
 the value $\lambda$.
...
\itt{factorizations} is a scalar variable of type \integer, that
gives the number of factorizations of the matrix \req{kkt}
for different $\lambda$, performed during the calculation.

\itt{max\_entries\_factors} is a scalar variable of type \integer, that
gives the maximum number of entries in any of the matrix factorizations
performed during the calculation.

\itt{len\_history} is a scalar variable of type \integer, that gives the
number of $(\lambda,\|\bmx(\lambda)\|_{\bmM})$ pairs encountered during the
calculation.
...
\itt{pole} is a scalar variable of type \realdp, that holds a lower
...
\hskip0.5in
{\tt CALL \packagename\_solve( n, radius, f, C, H, X, data, control, inform[, M, A] )}
...
\ittf{f} is a scalar \intentin\ variable of type \realdp, that holds
...
\ittf{M} is an \optional\ scalar \intentin\ argument of type {\tt SMT\_TYPE}
...
\itt{M\%type} is an allocatable array of rank one and type default
\character, that
is used to indicate the storage scheme used. If the dense storage scheme
(see Section~\ref{dense}) is used,
...
for the scaled-identity matrix storage scheme
(see Section~\ref{scaled-identity}),
the first fifteen components of {\tt M\%type} must contain the
string {\tt SCALED\_IDENTITY},
for the identity matrix storage scheme
(see Section~\ref{identity}),
the first eight components of {\tt M\%type} must contain the
...
for the zero matrix storage scheme
(see Section~\ref{zero}),
the first four components of {\tt M\%type} must contain the
string {\tt ZERO}.
...
\itt{M\%ne} is a scalar variable of type \integer, that
holds the number of entries in the {\bf lower triangular} part of $\bmM$
in the sparse co-ordinate storage scheme (see Section~\ref{coordinate}).
It need not be set for any of the other three schemes.

\itt{M\%val} is a rank-one allocatable array of type \realdp, that holds
the values of the entries of the {\bf lower triangular} part
of the scaling matrix $\bmM$ in any of the
storage schemes discussed in Section~\ref{galmatrix}.

\itt{M\%row} is a rank-one allocatable array of type \integer,
that holds the row indices of the {\bf lower triangular} part of $\bmM$
in the sparse co-ordinate storage
scheme (see Section~\ref{coordinate}).
It need not be allocated for any of the other three schemes.

\itt{M\%col} is a rank-one allocatable array variable of type \integer,
that holds the column indices of the {\bf lower triangular} part of
...
\itt{M\%ptr} is a rank-one allocatable array of dimension {\tt n+1} and type
\integer, that holds the starting position of
...
\ittf{A} is an \optional\ scalar \intentin\ argument of type {\tt SMT\_TYPE}
...
string {\tt SPARSE\_BY\_ROWS},
and for the zero matrix storage scheme (see Section~\ref{zero}),
the first four components of {\tt A\%type} must contain the
string {\tt ZERO}.
...
\itt{A\%m} is a scalar variable of type \integer, that
...
\begin{verbatim}
  ( .. lines ignored by TRS_read_specfile .. )
    BEGIN TRS
       keyword    value
       .......    .....
       keyword    value
...
\begin{verbatim}
    BEGIN TRS SPECIFICATION
\end{verbatim}
and
\begin{verbatim}
    END TRS SPECIFICATION
\end{verbatim}
...
\def\baselinestretch{0.8} {\tt \begin{verbatim}
     CALL TRS_read_specfile( control, device )
\end{verbatim}}
...
\hline
  ...
  {\tt force-Newton-if-positive-definite} & {\tt \%force\_Newton} & logical \\
...
$\|\bmx\|_{\bmM} - \Delta$, the current estimate of the multiplier and
...
$\bmA \bmx_* = 0$, where $\lambda_* \geq 0$ is a Lagrange
multiplier corresponding to the constraint $\|\bmx\|_{\bmM}  \leq  \Delta$
and $\bmy_*$ are Lagrange multipliers for the linear constraints 
$\bmA \bmx = 0$, if any;
for the equality-constrained problem $\|\bmx\|_{\bmM} = \Delta$,
...
$\|\bmx(\lambda)\|_{\bmM}  \geq  \Delta$ is found.
Here $\bmx(\lambda)$ and its companion $\bmy(\lambda)$
are defined to be a solution of
\eqn{ls}{(\bmH + \lambda \bmM)\bmx(\lambda)
 + \bmA^T \bmy(\lambda) = - \bmc \tim{and} \bmA \bmx(\lambda) = \bmzero;}
along the way the possibility that $\bmH$ might be positive definite on
the null-space of $\bmA$ and
$\|\bmx(0)\|_{\bmM}  \leq  \Delta$ is examined, and if this transpires
...
``secular'' equation $\|\bmx(\lambda)\|_{\bmM}  =  \Delta$ with
...
\req{ls}. In the absence of linear constraints, an
...
a sparse symmetric, indefinite factorization of \req{kkt} is used
...
\tim{and}
f=1,}
with a radius $\Delta = 10$ but no other constraints.
...
($\bmM = \bmI$) trust-region, we may solve the
