...
{\bf approximate minimizer of the regularized linear-least-$\ell_2$-norm
objective function
$\sqrt{\| \bmA \bmx - \bmb\|_2^2 + \mu \| \bmx \|_2^2}
 + \sfrac{1}{p} \sigma \| \bmx \|_2^p$.}
This problem commonly occurs as a subproblem in nonlinear
optimization calculations involving quadratic or cubic regularization,
...
\ittf{mu} is a scalar \intentin\ variable of type default
\realdp,
...
\subsection{Weighted least-$\ell_2$-norms, scaled regularization and
preconditioning}
The package may also be used to solve a weighted regularization
least-$\ell_2$-norm problem
\disp{ \min \| \bmW( \bmA\bmx - \bmb)\|_2^{}
...
\disp{\lambda = \mu + \sigma \|\bmx\|_2^{p-2}
 \sqrt{\| \bmA \bmx - \bmb \|_2^2 + \mu \|\bmx\|_2^2}.}
...
\eqn{4.1}{ \min
\sqrt{\| \bmB_k \bmy - \|\bmb\| \bme_1 \|_2^2 + \mu \|\bmy\|_2^2}
+ \sfrac{1}{p} \sigma \|  \bmy \|_2^p.}

To solve \req{4.1}, the optimality conditions
\eqn{4.2}{ ( \bmB_k^T ( \bmB_k^{} \bmy(\lambda) - \|\bmb\| \bme_1^{} ) + \lambda
\bmy(\lambda) = 0,}
where
\disp{\lambda = \mu + \sigma \|\bmy(\lambda)\|_2^{p-2}
 \sqrt{ \| \bmB_k \bmy(\lambda) - \|\bmb\| \bme_1 \|_2^2
+ \mu \|\bmy(\lambda)\|_2^2},}
...
\sigma \|\bmy(\lambda)\|_2^{p-2}
 \sqrt{ \| \bmB_k \bmy(\lambda) - \|\bmb\| \bme_1 \|_2^2 +
 \mu \|\bmy(\lambda)\|_2^2} + \mu - \lambda = 0.}
...
equation \req{4.4} significantly simplifies.

\galreferences
A complete description of the underlying quadratically-regularized
...
\tim{and}
\bmb  =  \vect{ 1 \\ . \\ . \\ . \\ . \\ . \\ . \\ . \\ 1 },}
with regularization weight $\sigma = 1$, shift $\mu = 0$ and order $p = 3$.
