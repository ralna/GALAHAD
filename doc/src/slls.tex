...
whose first {\tt lp} components on output contain the 
nonzeros of the {\tt index}-th column of $\bmA_o$.

\ittf{IP} is an \intentinout\ rank-one array of dimension at least
{\tt lp} and type \integer, that must be set to record the list
of indices of nonzero components of the {\tt index}-th column of $\bmA_o$,
corresponding to the values stored in {\tt P}.

\ittf{lp} is an \intentinout\ scalar variable of type \integer,
