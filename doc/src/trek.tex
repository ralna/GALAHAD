...
the current radius and $\|\bmx\|$ at the output $\bmx$.
...
is give, and \false\ if those from Algorithm B.3 should be used instead.
...
for each new member of the extended Krylov space. Such checks incur some
cost, and experience shows that testing every second member is sufficient, 
using {\tt stop\_check\_all\_orders = \true}.
