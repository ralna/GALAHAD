...
\newcommand{\package}{bqpb}
\newcommand{\packagename}{BQPB}
...
to solve the {\bf bound-constrained convex quadratic programming problem}
\eqn{qp}{\mbox{minimize}\;\; q(\bmx) = \half \bmx^T \bmH \bmx + \bmg^T \bmx + f}
or the {\bf bound-constrained shifted least-distance problem}
\eqn{lsqp}{\mbox{minimize}\;\;
 \half \sum_{j=1}^n w_j^2 ( x_j^{ } - x_j^0 )^2 + \bmg^T \bmx + f}
...
vectors $\bmg$, $\bmw$, $\bmx^{0}$, $\bmx^{l}$,
$\bmx^{u}$ and the scalar $f$ are given. Any of the constraint bounds 
$x_{j}^{l}$ and $x_{j}^{u}$ may be infinite.
Full advantage is taken of any zero coefficients in the matrix $\bmH$.

If general linear constraints are present, the package 
{\tt GALAHAD\_CQP} should be used instead.
...
The matrix $\bmH$ is stored as a compact dense matrix by rows,
...
\hspace{8mm}
{\tt CALL \packagename\_solve( prob, data, control, inform[, X\_stat] )}
...
\restrictions {\tt prob\%n} $> 0$, {\tt prob\%m} $\geq 0$,
  If $\bmH$ is provided, {\tt prob\%H\%ne} $\geq -2$.
 {\tt prob\%H\_type} $\in \{${\tt 'DENSE'},
 {\tt 'COORDINATE'}, {\tt 'SPARSE\_BY\_\-ROWS'}, {\tt 'DIAGONAL'},
 {\tt 'SCALED\_IDENTITY'}, {\tt 'IDENTITY'}, {\tt 'ZERO'} $\}$.
...
\itt{\galerrrestrictions.} One of the restrictions
 {\tt prob\%n} $> 0$ ,  {\tt prob\%m} $\geq 0$
    ...
    {\tt 'DIAGONAL'}  {\tt SCALED\_IDENTITY}, {\tt IDENTITY} or {\tt ZERO},
    when $\bmH$ is available, has been violated.
...
\eqn{4.2a}{
 ...
 \bmz \;\; \mbox{for the least-distance type objective})}
where
\eqn{4.2b}{
 \bmz = \bmz^{l} + \bmz^{u}, \,\,
...
$j = 1, \ldots , n$, where the vector $\bmz$ are
...
The package is actually just a front-end to the 
more-sophisticated \galahad\ package {\tt CQP} that saves users
from setting unnecessary arguments.
