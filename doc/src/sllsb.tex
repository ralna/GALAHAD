...
\newcommand{\package}{sllsb}
\newcommand{\packagename}{SLLSB}
...
{\bf simplex-constrained regularized linear least-squares problem}
...
Full advantage is taken of any zero coefficients in the design matrix $\bmA_o$.
...
should be allocated to have length {\tt o}, and used to hold
the observations $\bmb$; its $i$-th component should be
...
\ittf{W} is a rank-one allocatable array of type \realdp, that should
be allocated to have length {\tt o} if any of the weights $\bmw$
in the objective function is not one, and in this case
filled with the value $w_{i}$ for $i = 1, \ldots , o$.
If all the weights are one, {\tt W} need not be allocated.

\ittf{X\_s} is a rank-one allocatable array of type \realdp, that should
be allocated to have length {\tt n} if any of the shifts $\bmx_s$
in the objective function is nonzero, and in this case
filled with the value $(\bmx_s)_{i}$ for $i = 1, \ldots , n$.
If all the shifts are zero, {\tt X\_s} need not be allocated.
...
corresponding to the simplex equality constraints (see Section~\ref{galmethod}).
...
and type \integer, that indicates which of the non-negativity
constraints are in the current active set. Possible values for
{\tt X\_status(j)}, {\tt j}$=1, \ldots ,$ {\tt n}, and their meanings are
\begin{description}
\itt{<0} the $j$-th non-negativity bound constraint
is in the active set with value zero.,
\itt{0}  the $j$-th non-negativity bound constraint is not in the active set,
of the variable is unconstrained.
\end{description}
When {\tt control\%crossover} is \true, more specific values are
\begin{description}
\itt{-1} the $j$-th simple bound constraint
is both independent and active with value zero.
\itt{-2} the $j$-th simple bound constraint
is active with value zero but linearly dependent on others,
\itt{0} the $j$-th non-negativity bound constraint is not in the active set.
\end{description}

\end{description}

In addition, the following components are used internally, but need not be
set by the user.
\begin{description}
\ittf{A} is scalar variable of type {\tt SMT\_TYPE}
that holds the constraint Jacobian matrix $\bmA$ of the simplex constraints.
This need not be set by the user, but is set internally.
The following components are used:
...
on the simplex constraints. The $i$-th component of
...
\realdp, that holds
the vector of zero bounds $\bmx^{l}$ on the the variables.
...
\ifthenelse{\equal{\package}{sllsb}}{
...
{\tt 'ma86'}, {\tt 'ma97'}, {\tt slblt}, {\tt 'pardiso'}
and {\tt 'wsmp'},
although only {\tt 'sils'} and {\tt 'slblt'} are installed by default.
...
\itt{SLS\_control} and {\tt SLS\_pounce\_control}
...
\itt{SLS\_inform} and {\tt SLS\_pounce\_inform}
...
The simplex-constrained least-squares solution algorithm is called as follows:
\vspace*{1mm}

\hspace{8mm}
{\tt CALL \packagename\_solve( prob, data, control, inform )}
...
and, if they are not required, {\tt prob\%W} and {\tt prob\%X\_s}.

The components {\tt prob\%X}, {\tt prob\%Y} and {\tt prob\%Z}
must be set to initial estimates of the primal variables, $\bmx$,
Lagrange multipliers, $\bmy$, for the simplex constraints
...
On exit, the components {\tt prob\%X} , {\tt prob\%Y},
{\tt prob\%Z} amd {\tt prob\%R}, 
will contain the best estimates of the primal variables $\bmx$,
Lagrange multipliers, $\bmy$, for the general constraints
dual variables for the bound constraints $\bmz$, 
and the residuals $\bmr = \bmA_o \bmx - \bmb$, respectively.
\restrictions {\tt prob\%n} $> 0$, {\tt prob\%m} $\geq 0$, and
...
\begin{verbatim}
  ( .. lines ignored by SLLSB_read_specfile .. )
    BEGIN SLLSB
       keyword    value
       .......    .....
       keyword    value
...
\begin{verbatim}
    BEGIN SLLSB SPECIFICATION
\end{verbatim}
and
\begin{verbatim}
    END SLLSB SPECIFICATION
\end{verbatim}
...
{\tt
\begin{verbatim}
     CALL SLLSB_read_specfile( control, device )
\end{verbatim}
}
...
\eqn{4.1a}{\bme_{\cal C_i}^T \bmx_{\cal C_i} = 1}
and 
\eqn{4.1b}{\bmx_{\cal C_i}  \geq 0 \tim{for} i = 1,\ldots,m,}
...
\req{4.1b}, and \req{4.2b} and \req{4.3} are satisfied at each stage.
...
! dense storage format
   ...
   p%Ao%val = (/ 1.0_wp, 0.0_wp, 0.0_wp, 1.0_wp, 1.0_wp, 0.0_wp, 0.0_wp, &
...
! dense storage format
   ...
   ALLOCATE( p%Ao%val( o * n  )
   p%Ao%val = (/ 1.0_wp, 1.0_wp, 0.0_wp, 0.0_wp, 0.0_wp, 1.0_wp, 0.0_wp, &
