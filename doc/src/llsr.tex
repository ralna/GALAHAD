...
$\|\bmx\|_S -  \Delta| \leq$ {\tt stop\_normal} $\ast \;
\max(1,\Delta)$.
...
\itt{power} is a scalar \intentin\ variable of type default \realdp,
that must be set on initial entry
to the value of the regularization power, $p$.
{\bf Restriction: } $\power \geq 2$.

\itt{weight} is a scalar \intentin\ variable of type default \realdp,
that must be set on initial entry
