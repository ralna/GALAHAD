...
\newcommand{\package}{bnls}
\newcommand{\packagename}{BNLS}
...
where the variables $\bmx$ are required to satisfy the 
{\bf simple-bound constraints}
\eqn{c}{\bmx^l \leq \bmx \leq \bmx^u,}
and the positive weights $w_i$, $i=1,\ldots,m_c$ are given.
...
simple-bound constraint set $\bmx^l \leq \bmx \leq \bmx^u$.
...
\galmatrix The $m_r$ by $n$ unsymmetric matrix $\bmJ_r(\bmx)$ and 
the symmetric $n$ by $n$ matrix $\bmH(\bmx,\bmy)$
...
the value ${\bmJ_r}_{i,j}$ for $i = 1, \ldots , m_r$, $j = 1, \ldots , n$. 
...
Component $m_r \ast (j-1) + i$ of the storage array {\tt  Jr\%val} will 
hold the value ${\bmJ_r}_{i,j}$ for $i = 1, \ldots , m_r$, $j = 1, \ldots , n$.
...
The derived data type {\tt SMT\_TYPE} is used to hold any 
unsymmetric matrix $\bmA$ or symmetric one $\bmH$ if these are available. 
...
If the matrix is symmetric, each pair of off-diagonal entries $h_{ij} = h_{ji}$ 
is represented as a single entry (see \S\ref{dense}--\ref{diagonal}).
...
the first entry in each row, or dimension at least {\tt n + 1}, that may hold 
the pointers to the first entry in each column, as required
(see \S\ref{rowwise}--\ref{columnwise}).
...
\character, that
is used to indicate the storage scheme used. If the dense-by-row storage scheme
(see Section~\ref{dense}) is used,
the first five components of {\tt Jr\%type} must contain the
string {\tt DENSE} or (or the first thirteen contain {\tt DENSE\_BY\_ROWS}), 
while if a  dense-by-column storage scheme (see Section~\ref{dense-column}) 
is desired, the first sixteen components must contain the string 
{\tt DENSE\_BY\_COLUMNS}.
For the sparse co-ordinate scheme (see Section~\ref{coordinate}),
the first ten components of {\tt Jr\%type} must contain the
string {\tt COORDINATE},
for the sparse row-wise storage scheme (see Section~\ref{rowwise}),
its first fourteen components must contain the
string {\tt SPARSE\_BY\_ROWS}.
and for the sparse column-wise storage scheme (see Section~\ref{columnwise}),
the first seventeen components of {\tt Jr\%type} must contain the
...
scheme (see Section~\ref{coordinate}), or the sparse column-wise
(see Section~\ref{columnwise}) storage scheme.
It need not be allocated for the other dense sparse schemes.
...
\itt{Jr\%ptr} is a rank-one allocatable array of dimension {\tt m\_r+1} and type
\integer, that holds the starting position of each row of $\bmJ_r$, as well
as the total number of entries plus one, in the sparse row-wise storage
scheme (see Section~\ref{rowwise}). It should be of dimension {\tt n+1} and type
\integer to hold the starting position of each column of $\bmJ_r$, as well
as the total number of entries plus one, in the sparse column-wise scheme
(see Section~\ref{columnwise}). It need not be allocated when the
...
\ittf{W} is a rank-one allocatable array of dimension {\tt m\_r} and 
type default \realdp, that holds the values of the weights, $\bmw$.
The $i$-th component of {\tt W}, $i = 1, \ldots , m_r$, contains $w_{i}$.
...
\begin{description}
\itt{$\leq$ 1} A projected-gradient method using {\tt GALAHAD\_BLLS} 
will be used.
\itt{2} An interior-point method using {\tt GALAHAD\_BLLSB} will be used.
\itt{$\geq$ 3} An interior-point method will initially be used, but a
switch to a projected-gradient method will occur when sufficient progress
has occurred (see {\tt \%stop\_pg\_switch}).
\end{description}
...
\itt{BLLS\_control} is a scalar variable of type
{\tt BLLS\_control\_type}
whose components are used to control
the projected-gradient step calculation using
{\tt \libraryname\_BLLS} has been selected (see {\tt subproblem\_solver}.
See the specification sheet for the package
{\tt \libraryname\_BLLS}
for details, and appropriate default values.

\itt{BLLSB\_control} is a scalar variable of type
{\tt BLLSB\_control\_type}
whose components are used to control
...
\itt{blls} is a scalar variable of type \realdp, that gives
the total CPU time spent solving the step-determination subproblem
using the projected-gradient method from the {\tt GALAHAD\_BLLS} package.

\itt{bllsb} is a scalar variable of type \realdp, that gives
the total CPU time spent solving the step-determination subproblem
using the interior-point method from the {\tt GALAHAD\_BLLSB} package.

\itt{clock\_total} is a scalar variable of type default \realdp, that gives
 the total elapsed system clock time spent in the package.

\itt{clock\_blls} is a scalar variable of type \realdp, that gives the total 
elapsed system clock time spent solving the step-determination subproblem
using the projected-gradient method from the {\tt GALAHAD\_BLLS} package.

\itt{clock\_bllsb} is a scalar variable of type \realdp, that gives the total 
elapsed system clock time spent solving the step-determination subproblem
using the interior-point method from the {\tt GALAHAD\_BLLSB} package.
...
\itt{BLLS\_inform} is a scalar variable of type
{\tt BLLS\_inform\_type}
...
\itt{BLLSB\_inform} is a scalar variable of type
{\tt BLLSB\_inform\_type}
whose components give information about the progress and needs
of the integer-point subproblem package
{\tt \libraryname\_BLLSB}.
See the specification sheet for the package
{\tt \libraryname\_BLLSB}
for details.
...
\hspace{37mm}
{\tt eval\_Jr\_prod, eval\_Jr\_prods,  eval\_Jr\_sprod ] )}
...
The component {\tt nlp\%W} needs only be allocated and set if
non-unit weights are required.
...
{\tt nlp\%Jr\%type} $\in \{
  \mbox{\tt 'DENSE'}, \mbox{\tt 'DENSE\_BY\_ROWS'}, 
  \mbox{\tt 'DENSE\_BY\_COLUMNS'},$ $
  \mbox{\tt 'COORDINATE'}, \mbox{\tt 'SPARSE\_BY\_ROWS'},
  \mbox{\tt 'SPARSE\_BY\_COLUMNS'} \}$ 
%and
%{\tt nlp\%H\%type} $\in \{
...
{\tt eval\_sprod}, {\tt eval\_jprods} and {\tt eval\_SCALE}
...
to and from the \optional\ subroutines
 ...
 {\tt eval\_Jr\_prod}, {\tt eval\_Jr\_prods} and {\tt eval\_Jr\_sprod} 
(see below).
...
{\tt eval\_Jr\_prod} or {\tt eval\_Jr\_prods} are absent.
...
\itt{eval\_Jr\_prods} is an \optional\
...
If {\tt eval\_Jr\_prods} is present,
it must be declared {\tt EXTERNAL} in the calling program.
If {\tt eval\_Jr\_prods} is absent, \solver\ will use reverse communication to
...
If the argument {\tt eval\_Jr\_prods} is present when calling \solver, the
user is expected to provide a subroutine of that name to evaluate the
product of the Jacobian $\bmJ_r(\bmx)$ with a given sparse vector $\bmv$.
The routine must be specified as

\def\baselinestretch{0.8}
{\tt \begin{verbatim}
       SUBROUTINE eval_Jr_prods( status, X, userdata, V, P[, IV, lvl, lvu, IP, lp, got_Jr] )
\end{verbatim} }
\def\baselinestretch{1.0}
\noindent whose arguments are as follows:
...
only components {\tt IV(lvl:lvu)} of {\tt V}
...
whose components on output contain the required components of $\bmJ_r(\bmx) \bmv$.
If components
{\tt lp}
and
{\tt IP}
(see below) are \present,
only the nonzero components {\tt IP(1:lp)} of {\tt P}
need be assigned.
Otherwise, all components of {\tt P} must be set.

\itt{lvl} is an \optional\ scalar variable of type \integer,
that, if \present, holds the starting position in {\tt IV}
of the list of indices of nonzero components of $\bmv$.

\itt{lvu} is an \optional\ scalar variable of type \integer,
that, if \present, holds the finishing position in {\tt IV}
of the list of indices of nonzero components of $\bmv$.

\itt{IV} is an \optional\  rank-one array of dimension $n$
...
\itt{lp} is an \optional\ scalar variable of type \integer,
that, if \present, must be set to record the number of non-zeros in
$\bmJ_r(\bmx) \bmv$.

\itt{IP} is an \optional\ rank-one array of dimension $o$
and type \integer, that, if \present, must be set to record the list
of indices of nonzero components of $\bmJ_r(\bmx) \bmv$.
If either of
{\tt lp}
and
{\tt IP}
...
\ittf{2.} The user should compute the vector of residual functions
     ...
     {\tt reverse\%eval\_status} should be set to 0. If the user is
     ...
     {\tt nlp\%R}, but should then set {\tt reverse\%eval\_status}
     to a non-zero value.
...
\ittf{3.} The user should compute the Jacobian matrix $\bmJ_r(\bmx)$ of the
     ...
     {\tt  nlp\%Jr\%ne} and {\tt reverse\%eval\_status} should be set to 0.
...
\ittf{6.} The user should compute the product $\bmp = \bmJ_r(\bmx) \bmv$
     involving the residual Jacobian $\bmJ_r(\bmx)$ at the point $\bmx$, given
...
\ittf{7.} The user should compute the nonzero components of the product 
     $\bmp = \bmJ_r(\bmx) \bmv$
     involving the residual Jacobian $\bmJ_r(\bmx)$ at the point $\bmx$, given
     in {\tt nlp\%X}, and a given sparse vector $\bmv$, whose nonzeros
     are in positions {\tt reverse\%iv(\-reverse\%lvl:reverse\%lvu)}
     of {\tt reverse\%V}. The resulting nonzero components of $\bmp$ should be 
     occupy positions {\tt reverse\%IP(1:reverse\%lp)} of {\tt reverse\%P},
     and the components {\tt reverse\%IP} and {\tt reverse\%lp} must be set.
     The component {\tt reverse\%eval\_status} should set to 0 unless the
     user is unable to evaluate the product---for instance, if a component of 
     the Jacobian is undefined at $\bmx$---in which case 
...
\itt{\galerrrestrictions.}
  ...
  \mbox{\tt 'DENSE'}, 
  \mbox{\tt 'DENSE\_BY\_ROWS'}, 
  \mbox{\tt 'DENSE\_BY\_COLUMNS'}, 
  \mbox{\tt 'SPARSE\_BY\_ROWS'}, 
  \mbox{\tt 'SPARSE\_BY\_COLUMNS'}, 
  \mbox{\tt 'COORDINATE'} 
  has been violated.
...
\begin{verbatim}
  ( .. lines ignored by BNLS_read_specfile .. )
    BEGIN BNLS
       keyword    value
       .......    .....
       keyword    value
...
\begin{verbatim}
    BEGIN BNLS SPECIFICATION
\end{verbatim}
and
\begin{verbatim}
    END BNLS SPECIFICATION
\end{verbatim}
...
\begin{verbatim}
     CALL BNLS_read_specfile( control, device )
\end{verbatim}
}
\def\baselinestretch{1.0}
...
{\tt control\%BLLS\_control\%print\_level}
and
{\tt control\%BLLSB\_control\%print\_level}.
See the specification sheets for the packages
{\tt \libraryname\_BLLS} and {\tt \libraryname\_BLLSB}
for details.
...
\galmethod
The primal optimality conditions \req{c} and dual optimality conditions
\[\bmJ_r^T(\bmx) \bmW \bmr(\bmx) = \bmz\]
necessarily hold at an optimal point $\bmx$ for some 
dual variables $\bmz \geq 0$, where additionally $\bmx$ and $\bmz$ 
...
using the \galahad\ module {\tt BLLS}, or by
applying the interior-point method available in the module {\tt BLLSB}
...
$\bmw = 1$ and parameter value $p = 4$, where each variable is required
